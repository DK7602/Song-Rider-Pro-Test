...
function parseFullTextToSectionCards(fullText){
  ...
  function extractInlineChords(line){
    ...
    cleaned = cleaned.replace(/[\[\(\{]\s*([A-G](?:#|b)?(?:m|maj|min|dim|aug|sus|add)\d*(?:\d+)?(?:\/[A-G](?:#|b)?)?)\s*[\]\)\}]/g, (m, g1) => {
  ...
  function lineIsMostlyChords(line){
    ...
    const tokens = raw
      ...
      .map(s => s.trim())
      .filter(Boolean)
      .map(t => t.replace(/^[^A-Za-z0-9#b\/\.]+/,"").replace(/[^A-Za-z0-9#b\/\.]+$/,""));
  ...
  function buildSlotsFromAlignedChordLine(chordLineRaw, lyricLineRaw){
    ...
    // find chord tokens + their starting column positions
    const toks = [];
    let i = 0;
    while(i < chordLine.length){
      ...
      const start = i;
      while(i < chordLine.length && chordLine[i] !== " ") i++;
      const tok = chordLine.slice(start, i).trim();
      if(isChordToken(tok)){
    ...
    toks.forEach((item, idx) => {
      // map chord column to 0..7 (or compress if too many chords)
      const slot = n > 8
...
function pianoNote(ctx, freq, durMs, vel=0.9){
  ...
  for(const p of partials){
    ...
    const inharm = 1.0;// keep harmonics in tune
    o.type = "sine";
    o.frequency.value = freq * p.h * inharm;
    o.detune.value = 0; // keep piano in-tune (no random detune)
...
function buildAlignedLine(line, semis=0){
  ...
  const chordLine = chordGroups.map((g,i)=> padTo(g || "-", widths[i])).join(" | ");
  const lyricLine = segs.map((s,i)=> padTo(String(s ?? ""), widths[i])).join(" | ");
...
function buildChordSheetLines(noteRows, lyric){
  ...
  const slots = Array.isArray(notes) ? notes.slice(0,8) : [];
...
async function extractTextFromPdfFile(file){
  ...
  async function ensurePdfJs(){
    // Loads PDF.js in the most reliable way for GitHub Pages + Android Chrome/PWA.
    // Supports BOTH classic script builds (pdf.min.js) and ESM builds (pdf.mjs).
    ...
    // 1) Prefer LOCAL ESM if you have it (your repo screenshot shows `pdf.mjs`)
    const localEsmCandidates = [
      asUrl("pdf.mjs?v="+stamp),
      asUrl("./pdf.mjs?v="+stamp),
      asUrl("../pdf.mjs?v="+stamp),
    ];

    // 2) Then try LOCAL classic build
    ...
    // 3) CDN fallbacks (only if local missing)
    ...
    async function tryImport(url){
      try{
        // some servers send wrong content-type; import will still work on same-origin
        const mod = await withTimeout(import(/* @vite-ignore */ url), 15000, "Failed to import: " + url);
        const lib = (mod && (mod.getDocument ? mod : (mod.default && mod.default.getDocument ? mod.default : null)));
    ...
    // Try classic <script> load
    async function tryScript(url){
      try{
    ...
    // ESM local
    for(const url of localEsmCandidates){
      if(await tryImport(url)) return;
    }

    // Classic local (verify exists fast)
    for(const url of localClassicCandidates){
      const ok = await fetchOk(url);
      if(!ok) continue;
      if(await tryScript(url)) return;
    }

    // CDN classic
    for(const url of cdnClassicCandidates){
      const ok = await fetchOk(url);
      if(!ok) continue;
      if(await tryScript(url)) return;
    }

    throw new Error("PDF.js failed to load (local or CDN). Make sure `pdf.mjs` or `pdf.min.js` is present in your repo root.");
  }
await ensurePdfJs();
