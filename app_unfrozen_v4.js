...
function sectionHasFullTextContent(sec){
  if(!state.project) return false;
  const full = String(state.project.fullText || "");
  const lines = normalizeLineBreaks(full).split("\n");
  const target = String(sec||"").trim().toUpperCase();

  // Map of all heading labels (built-ins + extras) to detect next heading boundary
  const headings = new Set(getFullEditSections().map(s => String(s).trim().toUpperCase()));
  for(const ex of getExtraSections()){
    headings.add(String(ex).trim().toUpperCase());
  }
  ...
  for(let i=0;i<lines.length;i++){
    ...
    // consider chords in [ ] as content too
    const stripped = line.replace(/\[[^\]]*\]/g, "").trim();
    if(stripped) return true;
    // If line is only chord boxes, treat as content too
    const chordOnly = line.replace(/\s+/g,"");
    if(chordOnly && /^\[[^\]]*\]+$/.test(chordOnly)) return true;
  }
  return false;
}
...
function refreshEnabledSectionsFromContent(){
  ...
  // Keep extras that user explicitly created
  const extra = new Set(getExtraSections().map(s => String(s||"").trim()).filter(Boolean));
  const keep = new Set();
  for(const s of (state.project.enabledSections||[])){
    const name = String(s||"").trim();
    if(!name) continue;
    if(extra.has(name)) keep.add(name);
  }

  // Built-in pages ONLY appear if there is text under the heading on FULL page
  for(const sec of getFullEditSections()){
    if(sectionHasFullTextContent(sec)) keep.add(sec);
  }

  // Always keep at least one working page
  if(!keep.size) keep.add("VERSE 1");

  state.project.enabledSections = Array.from(keep);
}
...
function getSectionPages(){
  ...
  const pages = ["Full"];
  for(const sec of getFullEditSections()){
    if(en.has(sec)) pages.push(sec);
  }
  // extras come after built-ins
  for(const ex of getExtraSections()){
    if(en.has(ex)) pages.push(ex);
  }
  return pages;
}
  ...
  if(state.currentSection === "Full"){
    ...
    addBtn.addEventListener("click", (e) => {
      ...
      for(let i=0;i<seq.length;i++){
        const sec = seq[i];
        if(enabled.includes(sec.toUpperCase()) || sectionHasFullTextContent(sec)){
          lastIdx = i;
        }
      }
      ...
      if(lastIdx < seq.length - 1){
        ...
        // ✅ Only re-create a deleted built-in page if FULL page has text under its heading.
        // Otherwise, + creates an EXTRA page.
        if(sectionHasFullTextContent(candidate)){
          nextName = candidate;
          ...
          // No Full-page content under the next built-in heading -> create a new extra page instead
