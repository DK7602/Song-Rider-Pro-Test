...
function refreshEnabledSectionsFromContent(){
  ...
  for(const sec of getFullEditSections()){
    if(sectionHasContent(sec)){
      en.add(sec);
      // ✅ if user types under the heading on Full, the page can come back
      if(Array.isArray(state.project.deletedSections)){
        state.project.deletedSections = state.project.deletedSections.filter(x => String(x).toUpperCase() !== String(sec).toUpperCase());
      }
    }
  }
...
function getSectionPages(){
  ...
  const pages = ["Full"];
  for(const sec of getFullEditSections()){
    if(en.has(sec) || sectionHasContent(sec)) pages.push(sec);
  }
  return pages;
}
function nextNewExtraName(){
  const base = "NEW PAGE";
  const existing = new Set(getExtraSections().map(s => s.toUpperCase()));
  let i = 1;
...
function defaultProject(name="New Song"){
  ...
  return {
    ...
    deletedSections: [], // ✅ built-in pages the user deleted (don't auto-readd)
    sections
  };
}
...
function normalizeProject(p){
  ...
  if(!Array.isArray(p.deletedSections)) p.deletedSections = [];
  ...
  const doDeletePage = (sec, gotoAfter="Full") => {
    ...
    editProject("deletePage", () => {
      ...
      if(!SECTIONS.includes(sec)){
        ...
        // ✅ remember that this built-in page was deleted (so + won\'t re-add it unless Full has content)
        state.project.deletedSections = (state.project.deletedSections||[]);
        if(!state.project.deletedSections.includes(sec)) state.project.deletedSections.push(sec);
      }
  ...
  if(state.currentSection === "Full"){
    ...
    addBtn.addEventListener("click", (e) => {
      ...
      if(lastIdx < seq.length - 1){
        ...
        const deleted = (state.project.deletedSections || []).map(s => String(s||"").toUpperCase());
        const wasDeleted = deleted.includes(String(candidate).toUpperCase());
        const hasContent = sectionHasContent(candidate);

        // ✅ If the user previously deleted this built-in page, do NOT re-add it
        // unless there is text under that heading on Full (content).
        if(wasDeleted && !hasContent){
      ...
      state.project.enabledSections = (state.project.enabledSections||[]);
      state.project.deletedSections = (state.project.deletedSections||[])
        .filter(x => String(x).toUpperCase() !== String(nextName).toUpperCase());
            state.project.deletedSections = (state.project.deletedSections||[])
              .filter(x => String(x).toUpperCase() !== String(nextName).toUpperCase());
        ...
        // After CHORUS 3 -> create a new extra page
        nextName = nextNewExtraName();
        editProject("addExtraPage", () => {
          state.project.sections = state.project.sections || {};
          state.project.extraSections = (state.project.extraSections||[]);
...
// If this is a custom (extra) page, show an editable title input in the header
  ...
  if(isExtra && el.sheetTitle){
    ...
    inp.placeholder = "Page title (ex. CHORUS 4)";
    ...
    const commitRename = () => {
      ...
      if(!raw) { inp.value = state.currentSection; return; }
  ...
  addBtn.addEventListener("click", (e) => {
    ...
    if(i >= 0 && i < order.length - 1){
      ...
      // After CHORUS 3 (or unknown) -> create a new extra page
      nextName = nextNewExtraName();
      editProject("addExtraPage", () => {
        state.project.sections = state.project.sections || {};
        state.project.extraSections = (state.project.extraSections||[]);
...
***********************/
function wire(){
  if(el.togglePanelBtn) el.togglePanelBtn.addEventListener("click", () => {
...
if(el.capoInput){
  ...
  if(el.capoInput) el.capoInput.addEventListener("change", commitCapoStep_CAPTURE, true);
  if(el.capoInput) el.capoInput.addEventListener("blur", commitCapoStep_CAPTURE, true);
}
...
if(el.capoInput){
  if(el.capoInput) el.capoInput.addEventListener("keydown", (e) => {
    if(e.key === "Enter"){
      e.preventDefault();
      e.stopPropagation();
      el.capoInput.blur();
  ...
  if(el.capoInput) el.capoInput.addEventListener("change", () => {
    editProject("capoOrStep", () => commitCapoOrStepFromInput());
  });

  if(el.capoInput) el.capoInput.addEventListener("blur", () => {
    editProject("capoOrStep", () => commitCapoOrStepFromInput());
  });
}
  ...
  if(el.exportBtn){
    if(el.exportBtn) el.exportBtn.addEventListener("click", exportFullPreview);
  }
  ...
  if(el.bpmInput) el.bpmInput.addEventListener("change", commitBpm);
  if(el.bpmInput) el.bpmInput.addEventListener("blur", commitBpm);
...
const v = Number.isFinite(raw) ? raw : 0; 
  ...
  if(el.capoInput) el.capoInput.addEventListener("change", commitCapo);
  if(el.capoInput) el.capoInput.addEventListener("blur", commitCapo);
  ...
  if(el.instDots){
    if(el.instDots) el.instDots.addEventListener("click", () => {
  ...
  if(el.instTieBar){
    if(el.instTieBar) el.instTieBar.addEventListener("click", () => {
...
if(el.uploadAudioBtn){
  if(el.uploadAudioBtn) el.uploadAudioBtn.addEventListener("click", async () => {
...
if(el.beat1Btn){
  if(el.beat1Btn) el.beat1Btn.addEventListener("click", () => {
    markBeat1Now();
  });
}
