/* app.js (FULL REPLACE MAIN v45_pdfPasteAlign) */
(() => {
...
***********************/
const SECTIONS = ["Full","INTRO","VERSE 1","CHORUS 1","VERSE 2","CHORUS 2","VERSE 3","CHORUS 3","BRIDGE","VERSE 4","CHORUS 4","OUTRO"];
...
function parseFullTextToSectionCards(fullText){
  ...
  const buckets = {};
  const titles = {};
  // ensure buckets exist for all known sections except Full
  FULL_EDIT_SECTIONS.forEach(s => buckets[s] = []);

  // allow creating buckets for headings not in FULL_EDIT_SECTIONS (e.g., VERSE 4, INTRO, OUTRO)
  ...
  function isChordToken(tok){
    ...
    // normalize common unicode accidentals
    t = t.replace(/♯/g,"#").replace(/♭/g,"b");

    // strip trailing punctuation commonly produced by PDF extraction
    t = t.replace(/[\.,;:]+$/,"").trim();

    // remove internal spaces (rare)
    t = t.replace(/\s+/g,"");

    return CHORD_TOKEN_RE.test(t);
  }

  // Extract inline chords like [C] or (G) and return cleaned lyric + chord list
  function extractInlineChords(line){
  // ✅ Capture bracketed/parenthesized tokens and validate via isChordToken
  // Supports: [G], (D/F#), {F#m7b5}, [E7#9], etc.
  const chords = [];
  let cleaned = String(line || "");

  cleaned = cleaned.replace(/[\[\(\{]\s*([^\]\)\}]+?)\s*[\]\)\}]/g, (m, inner) => {
    const tok = String(inner || "").trim()
      .replace(/[|]+/g,"")                 // strip barlines
...
// Treat a line as a "mostly chords" line if tokens are chords and there is little else
function lineIsMostlyChords(line){
    const raw0 = String(line || "");
    const raw = raw0.trim();
    if(!raw) return false;

    // quick allow: single-token chord lines (very common above short lyric fragments)
    ...
    // If the line contains many letters outside A-G (after removing spaces/punct), it's probably lyrics.
    const alpha = raw.replace(/[^A-Za-z]/g,"");
    const nonChordLetters = alpha.replace(/[A-Ga-g]/g,"");
    if(nonChordLetters.length >= 4) return false;

    // tokenize (keep slashes for bass notes, strip obvious separators)
    const tokens = raw
      .replace(/[|\t]+/g, " ")
      .replace(/[-–—]+/g, " ")
      .split(/\s+/)
    ...
    // chord line if at least 2 chord tokens AND at least half the tokens are chords
    if(chordish >= 2 && chordish >= Math.ceil(tokens.length * 0.5)) return true;

    return chordish >= Math.max(1, Math.ceil(tokens.length * 0.7));
  }

  // Build 8 note slots from a "visually aligned" chord line above the lyric line.
  // We do NOT drop chords if there are >8: multiple chords can share a slot.
  function buildSlotsFromAlignedChordLine(chordLineRaw, lyricLineRaw){
    // ✅ More resilient alignment for copy/paste + PDF text
    // - treats tabs + NBSP as spaces
    // - strips barlines/punctuation around chord tokens
    // - supports single-chord lines
    ...
    // collect chord tokens and their "visual" start column from the chord line
    const toks = [];
    let i = 0;
    while(i < chordLine.length){
      ...
      // ✅ sanitize around token (common in UG / PDF extraction)
      tok = tok
        .replace(/^[^A-Za-z0-9#b\/\.]+/,"")
        .replace(/[^A-Za-z0-9#b\/\.]+$/,"")
        .replace(/[|]+/g,"")
    ...
    // If chord line likely collapsed (no multi-space runs), fall back to word-anchoring against the lyric line.
    let looksCollapsed = !/  +/.test(chordLine) && toks.length >= 2;

    const denom = Math.max(1, chordLine.length - 1);
    const n = toks.length;

    // Precompute lyric word start columns (for collapsed chord lines)
    let wordStarts = [];
    if(looksCollapsed){
      const s = String(lyricLine || "");
      const reWord = /\S+/g;
      let m;
    ...
    toks.forEach((item, idx) => {
      ...
      if(looksCollapsed){
        // Map chord idx to a lyric word start (left-to-right), then to slot.
        ...
        // Normal case: preserve column-based alignment.
        slot = n > 8
    ...
    // Heuristic: some clipboard/paste sources (Ultimate Guitar, etc.) collapse multiple spaces,
    // destroying the original "column" alignment. If the chord line has no multi-space runs,
    // fall back to word-anchoring against the lyric line.
    const looksCollapsed = !/  +/.test(chordLine) && toks.length >= 2;
  ...
  // Accept headings like "VERSE 1", "[Verse 2]", "Chorus", "INTRO", "OUTRO", etc.
  // Track bare "VERSE"/"CHORUS" headings (common in PDFs)
  let verseAuto = 0;
  let chorusAuto = 0;
  ...
  function isSectionHeadingLine(line){
    ...
    // Fix PDF "T u r n" letter-spacing within a single heading line
    raw = raw.replace(/(?:\b[A-Za-z]\s){2,}[A-Za-z]\b/g, (m)=>m.replace(/\s+/g,""));

    // remove trailing colon
    raw = raw.replace(/\s*:\s*$/,"").trim();

    // keep slashes/hyphens (C/G, PRE-CHORUS), drop other punctuation
    const t = raw.toUpperCase()
      .replace(/[^\w\s\/-]+/g," ")
      .replace(/\s+/g," ")
      .trim();

    // normalize: "VERSE1" -> "VERSE 1"
    let mVerse = t.match(/^VERSE\s*([0-9]+)$/);
    if(mVerse){
      const n = clamp(parseInt(mVerse[1],10)||1, 1, 3);
      verseAuto = Math.max(verseAuto, n);
      return `VERSE ${n}`;
    }

    let mCh = t.match(/^CHORUS\s*([0-9]+)$/);
    if(mCh){
      const n = clamp(parseInt(mCh[1],10)||1, 1, 3);
      chorusAuto = Math.max(chorusAuto, n);
      return `CHORUS ${n}`;
    }

    // bare VERSE / CHORUS (no number) -> auto-increment
    if(t === "VERSE"){
      verseAuto = clamp(verseAuto + 1, 1, 3);
      return `VERSE ${verseAuto}`;
    }
    if(t === "CHORUS"){
      chorusAuto = clamp(chorusAuto + 1, 1, 3);
      return `CHORUS ${chorusAuto}`;
    }

    if(t === "INTRO" || t === "INTRODUCTION") return "INTRO";
    if(t === "OUTRO" || t === "CODA") return "OUTRO";
    if(t === "BRIDGE") return "BRIDGE";
    if(t === "PRECHORUS" || t === "PRE-CHORUS" || t === "PRE CHORUS") return "BRIDGE"; // fallback bucket
    if(t === "INTERLUDE" || t === "TAG") return "BRIDGE"; // fallback bucket

    return null;
  }

  let pendingChordLine = null; // raw chord line awaiting lyric line

  function pushCard(lyricsRaw, slots){
  ...
  // ✅ flushCard existed in earlier drafts when blank lines created visual separators.
  // In current parser we do NOT create blank cards, so this is intentionally a no-op.
  function flushCard(){
    /* no-op */
  }
  ...
  for(let i=0; i<lines.length; i++){
    ...
    // @@TITLE: ... (metadata line, not a card)
    const titleMatch = line.match(/^\s*@@TITLE\s*:\s*(.*)\s*$/i);
    if(titleMatch){
      const ttl = String(titleMatch[1] || "").trim();
      if(ttl) titles[cur] = ttl;
      else { try{ delete titles[cur]; }catch{} }
      continue;
    }

    const heading = isSectionHeadingLine(line);
    if(heading){
      // flush any dangling chord line as its own card (rare, but keeps info)
      if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
        pushCard("", buildSlotsFromAlignedChordLine(pendingChordLine, ""));
      }
    ...
    if(line.trim() === ""){
      // If we have a chord-only line waiting and the next non-empty line is lyrics,
      // swallow this blank line so the chord line can pair with the lyric line.
      if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
        let k = i + 1;
        while(k < lines.length && String(lines[k] || "").trim() === "") k++;
        const nxt = k < lines.length ? String(lines[k] || "") : "";
        const nxtHeading = isSectionHeadingLine(nxt);
        // next non-empty is a lyric line -> keep chord line pending and skip this blank
        if(nxt && !nxtHeading && !lineIsMostlyChords(nxt)){
          continue;
        }
      }

      // blank line ends any pending chord line
      if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
        pushCard("", buildSlotsFromAlignedChordLine(pendingChordLine, ""));
      }
      pendingChordLine = null;
    ...
    // if current line is a chord-only line, store as pending
    if(lineIsMostlyChords(line)){
      // if we already had a pending chord line, treat previous as its own card
      if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
        pushCard("", buildSlotsFromAlignedChordLine(pendingChordLine, ""));
      }
    ...
    if(pendingChordLine){
      ...
      // inline chords: distribute left-to-right across slots, no drops
    ...
    pushCard(cleaned, slots);
  }

  // flush leftover chord line
  if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
    pushCard("", buildSlotsFromAlignedChordLine(pendingChordLine, ""));
  }
...
function buildFullPreviewHtmlDoc(title){
  ...
  SECTIONS.filter(s => s !== "Full").forEach(sec => {
    ...
    // Section heading
    blocks.push(
      `<div style="font-family:Arial,Helvetica,sans-serif;font-weight:900;margin:20px 0 8px;letter-spacing:.3px;">${escapeHtml(sec.toUpperCase())}</div>`
    );
...
function renderSheetTitleBar(){
  ...
  name.textContent = state.currentSection;
  ...
  // Title input only for non-Full pages
  let titleInput = null;
  if(state.currentSection !== "Full"){
...
async function importPdfToFull(file){
  try{
    const textRaw = await extractTextFromPdfFile(file);
    const text = normalizePdfHeadings(textRaw);
    if(!String(text||"").trim()){
...
function normalizePdfHeadings(txt){
  // Turns PDF lines like "Chorus" into SRP headings "CHORUS 1", "CHORUS 2", etc.
  // If the PDF has no headings, we leave it as-is (it will live in VERSE 1 by default).
  ...
  const normalizeLine = (s) => {
    ...
    // undo PDF "l e t t e r   s p a c i n g" for headings too
  ...
  for(const raw of lines){
    ...
    // Accept common variants: "Chorus", "Chorus 2", "CHORUS:", etc.
    let m;

    m = t.match(/^chorus\b\s*(\d+)?\s*:?\s*$/i);
    if(m){
      const n = m[1] ? parseInt(m[1],10) : (++chorusN);
      if(!m[1]) chorusN = n;
      out.push(`CHORUS ${n}`);
      continue;
    }

    m = t.match(/^verse\b\s*(\d+)?\s*:?\s*$/i);
    if(m){
      const n = m[1] ? parseInt(m[1],10) : (++verseN);
      if(!m[1]) verseN = n;
      out.push(`VERSE ${n}`);
      continue;
    }

    m = t.match(/^bridge\b\s*(\d+)?\s*:?\s*$/i);
    if(m){
      out.push("BRIDGE");
      continue;
    }

    m = t.match(/^intro\b\s*:?\s*$/i);
    if(m){
      out.push("INTRO");
      continue;
    }

    m = t.match(/^outro\b\s*:?\s*$/i);
    if(m){
      out.push("OUTRO");
      continue;
    }
...
try{
  ...
  console.error("SRP init crash", e);
  try{ alert("Song Rider Pro failed to start. Open DevTools console for details.\n\n" + (e?.message||e)); }catch{}
}
})();
