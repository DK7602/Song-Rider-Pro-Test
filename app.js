...
function isSectionHeadingLine(line){
    ...
    // strip surrounding [ ] and trailing colon
    ...
    // Common aliases
    if(t === "INTRO") t = "INTRO 1";
    if(t === "OUTRO") t = "OUTRO 1";

    // Allow: VERSE 4, CHORUS 4, etc. We map anything beyond the UI's last slot
    // into the last available section, but we keep the original label in the text (handled in parser).
    const mVerse = t.match(/^VERSE\s*(\d+)$/);
    if(mVerse){
      const n = parseInt(mVerse[1],10);
      if(!isFinite(n) || n<=0) return null;
      if(n >= 3) return "VERSE 3";
      return "VERSE 1";
    }
    const mCh = t.match(/^CHORUS\s*(\d+)$/);
    if(mCh){
      const n = parseInt(mCh[1],10);
      if(!isFinite(n) || n<=0) return null;
      if(n >= 3) return "CHORUS 3";
      return "CHORUS 1";
    }

    // Handle INTRO/OUTRO: map to first / last practical page
    if(/^INTRO\b/.test(t)) return "VERSE 1";
    if(/^OUTRO\b/.test(t)) return "CHORUS 3";

    // Bridge stays bridge if available
    if(t === "BRIDGE") return "BRIDGE";

    // If user pastes exact section label we support, accept it
...
let pendingChordLine = null;
  ...
  function lineIsMostlyChords(line){
    ...
    // if it contains letters outside chord grammar, it's not a chord line
  ...
  function buildChordsFromAlignedLine(chordLineRaw){
    const chordLine = String(chordLineRaw || "");
    const denom = Math.max(1, chordLine.length - 1);

    const slots = Array(8).fill("");

    function placeChordAtSlot(ch, slot){
      const c = String(ch || "").trim();
      if(!c) return;
      let s = clamp(Number(slot)||0, 0, 7);

      // If the slot is already occupied, DO NOT slide the chord elsewhere.
      // Append so we keep the original alignment and don't "invent" extra chord positions.
      if(slots[s]){
        slots[s] = String(slots[s]).trim() ? (String(slots[s]).trim() + " " + c) : c;
        return;
      }
    ...
    for(const m of chordLine.matchAll(/\S+/g)){
      const tok = String(m[0] || "")
        ...
        .trim();
      if(!tok) continue;
      if(isChordToken(tok)) toks.push({ chord: tok, idx: m.index || 0 });
    }

    if(!toks.length) return slots;

    for(const t of toks){
      const ratio = t.idx / denom;
      const slot = Math.round(ratio * 7);
      placeChordAtSlot(t.chord, slot);
    }
    return slots;
  }

  function flushCard(){
    // Keep original spacing inside each line; just trim ends.
    const blockLines = acc.map(x => String(x || "").replace(/\s+$/,"")).filter(l => l.trim() !== "");
    acc = [];
    if(!blockLines.length) return;

    const chordsSlots = Array(8).fill("");
    const seqFallback = [];
    const lyricParts = [];

    function absorbSeqChordsFromLine(line){
      const chordLine = String(line || "")
        .replace(/[\[\]\(\)\{\}]/g," ")
        .replace(/[|]/g," ")
        .split(/\s+/)
        .filter(Boolean)
        .map(x => x.trim())
        .filter(isChordToken);
      chordLine.forEach(c => seqFallback.push(c));
    }
    ...
    for(let i=0;i<blockLines.length;i++){
      ...
      if(lineIsMostlyChords(L) && i+1 < blockLines.length && !lineIsMostlyChords(blockLines[i+1])){
        const lyricLine = String(blockLines[i+1] || "");
        const aligned = buildChordsFromAlignedLine(L);

        // merge aligned into chordsSlots (prefer aligned)
        for(let k=0;k<8;k++){
          if(aligned[k] && !chordsSlots[k]) chordsSlots[k] = aligned[k];
        }

        // any remaining chords (if alignment failed) -> sequential fallback
        if(aligned.every(x => !String(x||"").trim())){
          absorbSeqChordsFromLine(L);
        }

        const { cleaned, chords } = extractInlineChords(lyricLine);
        chords.forEach(c => seqFallback.push(c));
      ...
      // standalone chord line (no lyric following) => treat as sequence
      if(lineIsMostlyChords(L)){
        absorbSeqChordsFromLine(L);
        continue;
      }

      // inline chords inside lyric line
      const { cleaned, chords } = extractInlineChords(L);
      chords.forEach(c => seqFallback.push(c));
      if(cleaned) lyricParts.push(cleaned);
    }

    const lyric = lyricParts.join(" ").replace(/\s+/g," ").trim();

    // Fill remaining empty chord slots left->right from seqFallback.
    if(seqFallback.length){
      for(const c of seqFallback){
        const idx = chordsSlots.findIndex(x => !String(x||"").trim());
        if(idx === -1) break;
        chordsSlots[idx] = String(c||"").trim();
      }
    }

    const anyChords = chordsSlots.some(x => String(x||"").trim());
    if(!lyric && !anyChords) return;

    const item = { lyrics: lyric };
    if(anyChords) item.chords = chordsSlots.slice(0,8);
 ...
 for(const rawLine of lines){
  ...
  if(heading){
    // Keep original heading label for headings beyond the UI's fixed pages (e.g., VERSE 4, CHORUS 4, INTRO/OUTRO)
    let rawH = String(trimmed||"").trim()
      .replace(/^\[\s*/,"").replace(/\s*\]$/,"")
      .replace(/:$/,"").trim()
      .replace(/\s+/g," ")
      .toUpperCase();
    ...
    // If the original heading isn't one of the built-in pages, insert it as a marker card
    // so the user still sees "VERSE 4" / "OUTRO" in the rendered cards.
    if(rawH && !FULL_EDIT_SECTIONS.includes(rawH)){
      // Normalize INTRO/OUTRO numbering for display
      if(rawH === "INTRO") rawH = "INTRO";
      if(rawH === "OUTRO") rawH = "OUTRO";
      emitCard({ lyrics: `[${rawH}]`, chords: Array(8).fill("") });
    }
...
function applyFullTextToProjectSections(fullText){
  ...
  FULL_EDIT_SECTIONS.forEach(sec => {
    ...
    for(let i=0; i<want.length; i++){
      ...
      if(Array.isArray(w.chords) && w.chords.some(x => String(x||"").trim())){
        base.notes = Array(8).fill("");
        for(let k=0;k<8;k++){
          base.notes[k] = String(w.chords[k] || "").trim();
        }
      }
...
function buildAlignedLine(line, semis=0){
  ...
  // Helper: join up to 2 chord tokens without adding placeholders
  const join2 = (a,b)=>{
  ...
  // 8 note slots -> 4 "cells" (two 8ths each). No '-' fillers.
  const noteGroups = [
    join2(n[0], n[1]),
    join2(n[2], n[3]),
    join2(n[4], n[5]),
  ...
  // build a 4-cell beat/lyric row (already split by your autosplit)
  const beatRow = beats.map(b => String(b ?? "").trim());

  // Determine column widths so chords sit above the same cell as import.
  const widths = [0,0,0,0].map((_,i)=>Math.max(
    noteGroups[i].length,
    beatRow[i].length,
    1
  ));
  ...
  const notesLine = noteGroups.map((g,i)=>pad(g, widths[i])).join(" | ").trimEnd();

  // If you have no beatRow text (empty lyric card), keep line simple
  if(!beatRow.some(x=>x)){
    return notesLine.trim() ? notesLine : lyric;
  }

  const beatsLine = beatRow.map((g,i)=>pad(g, widths[i])).join(" | ").trimEnd();

  // If lyric exists but beatRow is empty, just return lyric (fallback)
  if(lyric && !beatsLine.trim()) return lyric;

  return (notesLine.trim() ? (notesLine + "
") : "") + beatsLine;
}
...
async function extractTextFromPdfFile(file){
  ...
  for(let p=1; p<=pdf.numPages; p++){
    ...
    // Estimate average character width for this page (used to preserve chord spacing)
    const cw = [];
    for(const it of items){
      const txt = String(it.str || "");
      if(it.w > 0 && txt.trim() && txt.length > 0){
        cw.push(it.w / txt.length);
      }
    }
    const charW = (() => {
      if(!cw.length) return 5;
      cw.sort((a,b)=>a-b);
      return cw[Math.floor(cw.length/2)] || 5;
    })();
    ...
    const pushLine = () => {
      ...
      for(const it of line){
        ...
        if(cursorX === null){
          ...
          const gap = it.x - cursorX;
          if(gap > charW * 0.75){
            const spaces = Math.max(1, Math.min(40, Math.round(gap / charW)));
            s += " ".repeat(spaces);
          }else{
            s += " ";
          }
        ...
        s += txt;
        cursorX = it.x + (it.w || (charW * txt.length));
      }

      // keep internal spacing (important for chord alignment), but trim line end
