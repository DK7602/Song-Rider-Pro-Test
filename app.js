...
function setAutoScroll(on){
  ...
  if(state.autoScrollOn){
    ...
    // ✅ Always restart from beat 1 + card 1 when AutoScroll is engaged
    state.tick8 = 0;
    state.lastAutoBar  = -1;
    state.lastHorseBar = -1;
    state.playCardIndex = 0;

    // If MP3 sync is active, rewind to Beat 1 start so tick8 doesn't jump ahead
    if(state.audioSyncOn && state.audioSyncAudio){
      try{
        state.audioSyncAudio.currentTime = Math.max(0, Number(state.audioSyncOffsetSec) || 0);
      }catch{}
      state.lastAudioTick8 = -1;
    }else{
      // Restart internal clock immediately from beat 1
      stopBeatClock();
    }
