...
function insertWordIntoNotes(word){
  ...
  try{
    let marker = el.notesEditor.querySelector(`[${NOTES_RHYME_MARKER_ATTR}="1"]`);
    if(!marker){
      restoreNotesSelection();
      syncNotesCaretMarker();
      marker = el.notesEditor.querySelector(`[${NOTES_RHYME_MARKER_ATTR}="1"]`);
    }

    if(marker){
      const prevNode = marker.previousSibling;
      const nextNode = marker.nextSibling;
      const beforeChar = (prevNode && prevNode.nodeType === Node.TEXT_NODE) ? (prevNode.nodeValue || "").slice(-1) : "";
      const afterChar = (nextNode && nextNode.nodeType === Node.TEXT_NODE) ? (nextNode.nodeValue || "").slice(0,1) : "";
      const needsSpaceBefore = !!beforeChar && !/\s/.test(beforeChar);
      const needsSpaceAfter = !afterChar || !/\s/.test(afterChar);
      const textNode = document.createTextNode(`${needsSpaceBefore ? " " : ""}${insertWord}${needsSpaceAfter ? " " : ""}`);
  ...
  if(!range && state.notesLastRange){
    try{
      range = state.notesLastRange.cloneRange();
      if(!el.notesEditor.contains(range.commonAncestorContainer)) range = null;
    }catch{}
  }
  ...
  const beforeChar = (() => {
    try{
      if(startContainer && startContainer.nodeType === Node.TEXT_NODE){
        return (startContainer.nodeValue || "").slice(Math.max(0, startOffset - 1), startOffset);
      }
    }catch{}
    return "";
  })();
  ...
  const afterChar = (() => {
    try{
      if(startContainer && startContainer.nodeType === Node.TEXT_NODE){
        return (startContainer.nodeValue || "").slice(startOffset, startOffset + 1);
      }
    }catch{}
    return "";
  })();
  ...
  const needsSpaceBefore = !!beforeChar && !/\s/.test(beforeChar);
  const needsSpaceAfter = !afterChar || !/\s/.test(afterChar);
  const insertText = `${needsSpaceBefore ? " " : ""}${insertWord}${needsSpaceAfter ? " " : ""}`;
...
function insertWordIntoLyrics(word){
  ...
  const notesHasStoredCaret = !!(state.notesOpen && el.notesEditor && state.notesLastRange);
  const useNotesEditor = !!(el.notesEditor && state.notesOpen && (active === el.notesEditor || notesHasStoredCaret));
  if(useNotesEditor && insertWordIntoNotes(word)) return;
...
async function renderRhymes(seed){
  ...
  list.forEach(w => {
    ...
    b.addEventListener("mousedown", (e) => {
      if(state.notesOpen && el.notesEditor) e.preventDefault();
    });
...
function clearNotesRhymeMarker(){
  try{
    ...
    el.notesEditor.querySelectorAll(`[${NOTES_RHYME_MARKER_ATTR}="1"]`).forEach(n => n.remove());
  }catch{}
}
...
function placeNotesRhymeMarker(){
  try{
    ...
    const range = sel.getRangeAt(0);
    if(!el.notesEditor.contains(range.commonAncestorContainer)) return false;
    clearNotesRhymeMarker();
    const marker = document.createElement("span");
    marker.setAttribute(NOTES_RHYME_MARKER_ATTR, "1");
    ...
    sel.removeAllRanges();
    sel.addRange(after);
    state.notesLastRange = after.cloneRange();
    return true;
  }catch{
    return false;
  }
...
function restoreNotesSelection(){
  try{
    ...
    if(state.notesLastRange){
      sel.removeAllRanges();
      sel.addRange(state.notesLastRange);
      return true;
    }
    placeCaretAtEnd(el.notesEditor);
    return true;
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ["keyup","mouseup","touchend","focus","click"].forEach(evt => {
