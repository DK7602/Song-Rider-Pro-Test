...
function readNotesBackup(projectId){
  try{
    ...
    const raw = localStorage.getItem(notesBackupKey(projectId));
    if(!raw) return null;
...
function writeNotesBackup(project){
  try{
    ...
    localStorage.setItem(notesBackupKey(project.id), JSON.stringify(payload));
  }catch{}
}
...
function deleteNotesBackup(projectId){
  try{
    if(!projectId) return;
    localStorage.removeItem(notesBackupKey(projectId));
  }catch{}
}
