...
function parseFullTextToSectionCards(fullText){
  ...
  const hasBreaks = lines.some(l => String(l||"").trim() === BREAK_LINE);
  ...
  let nextPageIdx = 0;
  function allocateNextSection(optionalTitle){
    ensurePageMeta();
  ...
  function maybeConsumeTitleLine(i){
    ...
    if(cand === BREAK_LINE) return { nextIndex: k, title: "" };
  ...
  if(hasBreaks){
    ...
    for(let i=0; i<lines.length; i++){
      ...
      if(String(line).trim() === BREAK_LINE){
        ...
        pageIdx++;
        cur = (pageIdx < FULL_EDIT_SECTIONS.length) ? FULL_EDIT_SECTIONS[pageIdx] : allocateNextSection(null);
        ensureBucket(cur);
      ...
      if(line.trim() === ""){
        ...
        if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
          ...
          if(nxt && String(nxt).trim() !== BREAK_LINE && !lineIsMostlyChords(nxt)){
            continue;
          }
        }
    ...
    for(let i=0; i<lines.length; i++){
      ...
      if(heading){
        ...
        cur = allocateNextSection(heading);
        ensureBucket(cur);
        continue;
      }
...
if(state.currentSection === "Full"){
  el.sheetHint.textContent = 'Paste + edit. Use break lines (__________) to separate pages. Optional: put the page name on the line right after a break. Blank line = next card.';
...
function isSectionVisible(sec){
  ...
  // Base pages (PAGE 1..PAGE 10) are always visible
  if(BASE_SECTION_ORDER.includes(sec)) return true;

  // Extra pages appear when they have content or were explicitly added
  if(sectionHasAnyContent(sec)) return true;
  return state.project.enabledSections.includes(sec);
}
...
function findNextSectionToEnable(fromSec){
  // New behavior: "+" always creates a new page (EXTRA) after the 10 base pages.
