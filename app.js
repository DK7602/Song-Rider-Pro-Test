...
function isSectionHeadingLine(line){
  // Accept: "VERSE 1", "Verse 1:", "[Verse 1]", "(CHORUS 2)", "CHORUS", "BRIDGE", etc.
  let t = String(line || "").trim();

  // strip common wrappers/brackets
  t = t.replace(/^\s*[\[\(\{]+\s*/,"").replace(/\s*[\]\)\}]+\s*$/,"");

  // normalize punctuation / spacing
  t = t.replace(/:$/,"").replace(/\s+/g," ").trim().toUpperCase();

  // normalize "VERSE1" -> "VERSE 1", "CHORUS2" -> "CHORUS 2"
  t = t.replace(/^VERSE\s*(\d+)$/,"VERSE $1");
  t = t.replace(/^CHORUS\s*(\d+)$/,"CHORUS $1");
  t = t.replace(/^BRIDGE\s*(\d+)$/,"BRIDGE"); // ignore numbering
  if(t === "INTRO") t = "VERSE 1";

  // Common aliases
  if(t === "CHORUS") t = "CHORUS 1";
  if(t === "VERSE") t = "VERSE 1";
...
let pendingChordLine = null;
  ...
  function buildChordsFromAlignedLine(chordLineRaw){
    ...
    function placeChordAtSlot(ch, slot){
      ...
      if(!slots[s]){ slots[s] = c; return; }
      for(let k=s+1;k<8;k++){ if(!slots[k]){ slots[k]=c; return; } }
      for(let k=s-1;k>=0;k--){ if(!slots[k]){ slots[k]=c; return; } }
    }
