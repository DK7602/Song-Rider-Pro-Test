...
function insertWordIntoLyrics(word){
  ...
  if(useNotesEditor){
    ...
    const marker = el.notesEditor?.querySelector(`[${NOTES_RHYME_MARKER_ATTR}="1"]`);
    if(marker){
      const textNode = document.createTextNode(insert);
      const nextMarker = document.createElement("span");
      nextMarker.setAttribute(NOTES_RHYME_MARKER_ATTR, "1");
      nextMarker.appendChild(document.createTextNode("​"));
      marker.replaceWith(textNode, nextMarker);
      try{
        const sel = window.getSelection?.();
        if(sel){
          const range = document.createRange();
          range.setStartAfter(nextMarker);
          range.collapse(true);
          sel.removeAllRanges();
          sel.addRange(range);
