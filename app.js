...
// transpose a visible chord label (handles slash bass + preserves surrounding brackets)
function transposeChordLabel(label, semis){
  const s0 = String(label ?? "");
  const s = s0.trim();
  if(!s) return s0;

  // Preserve wrapping brackets like [Am], (Am), {Am}
  const wrap = s.match(/^([\[(\{<]+)\s*(.+?)\s*([\])\}>]+)$/);
  const core = wrap ? wrap[2] : s;
  const pre  = wrap ? wrap[1] : "";
  const post = wrap ? wrap[3] : "";

  // Split on slash bass e.g. D/F#
  const parts = core.split("/");
  const main = parts[0] ?? "";
  const bass = parts[1] ?? "";

  const mainT = semis ? transposeChordName(main, semis) : main;

  let out = mainT;
  if(bass){
    // bass is usually just a note name; transpose root only
    const bassT = semis ? transposeChordName(bass, semis) : bass;
    out = `${mainT}/${bassT}`;
  }
  return (pre + out + post);
}
...
function buildAlignedLine(line, semis=0){
  ...
  if(!segs.some(s 
function buildChordSheetLines(notes, lyric){
  const L = String(lyric || "");
  const slots = Array.isArray(notes) ? notes.slice(0,8) : [];
  if(!L.trim() || !slots.some(x => String(x||"").trim())){
    return { chordLine: "", lyricLine: L };
  }

  const len = Math.max(1, L.length);
  const arr = Array(len).fill(" ");

  // Place each chord at an even column position across the lyric width
  for(let i=0;i<8;i++){
...
function buildFullPreviewText  const chordLine = chordGroups.map((g,i)=> padTo(g || "-", widths[i])).join(" | ");
