...
async function exportFullPreview(){
  ...
  try{
    ...
    // ✅ Best option (when supported): File System Access API (works on many desktop Chromes and some Android builds)
    try{
    ...
    // ✅ Classic: download via <a download>
    try{
    ...
    // ✅ Last resort: open in a new tab so user can use Chrome "Download" / "Share"
    try{
      const url = URL.createObjectURL(htmlBlob);
...
async function extractTextFromPdfFile(file){
  ...
  async function ensurePdfJs(){
    ...
    // ✅ Local candidates (try multiple path styles)
    const localCandidates = [
      new URL("pdf.min.js?v="+stamp, base).toString(),            // same folder as index.html
      new URL("./pdf.min.js?v="+stamp, base).toString(),          // explicit same-folder
      new URL("../pdf.min.js?v="+stamp, base).toString(),         // one folder up (covers common repo layouts)
    ];

    // ✅ CDN candidates (fallback only)
    const cdnCandidates = [
    ...
    const candidates = [...localCandidates, ...cdnCandidates];

    let lastErr = null;
    for(const url of candidates){
      try{
        // If it’s a local file (same origin), verify it exists first to avoid false positives.
        const sameOrigin = (()=>{ try{ return (new URL(url)).origin === location.origin; }catch{ return false; } })();
        if(sameOrigin){
          const ok = await fetchOk(url);
          if(!ok) throw new Error("Local PDF.js not found / not served correctly: " + url);
        }

        await loadScript(url);
        if(window.pdfjsLib?.getDocument) return;
        throw new Error("Loaded script but pdfjsLib.getDocument missing: " + url);
      }catch(e){
        lastErr = e;
      }
    }
    throw lastErr || new Error("pdfjsLib not available after loading scripts");
  }
await ensurePdfJs();
