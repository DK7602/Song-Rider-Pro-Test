...
function getSavedProjectSort(){
  try{
    const v = String(localStorage.getItem(LS_SORT) || "").trim().toLowerCase();
...
function saveProjectSort(v){
  try{
    localStorage.setItem(LS_SORT, (String(v || "").trim().toLowerCase() === "recent") ? "recent" : "az");
  }catch{}
}

function notesBackupKey(projectId){
...
const CLOUD_PROJECTS_TABLE = "srp_projects";
...
function setCloudStatus(kind, extra=""){
  ...
  if(kind === "syncing"){
    btn.textContent = "☁︎";
    btn.title = "Cloud syncing…";
    btn.style.opacity = "1";
    return;
  }
  if(kind === "ok"){
    btn.textContent = "☁︎";
    btn.title = "Cloud connected";
    btn.style.opacity = "1";
    return;
  }
  if(kind === "error"){
    btn.textContent = "☁︎!";
    btn.title = "Cloud sync error" + (extra ? ": " + extra : "");
    btn.style.opacity = "1";
    return;
  }

  btn.textContent = "☁︎";
  btn.title = "Local only";
...
async function fetchCloudProjects(){
  ...
  const { data, error } = await cloud.client
    ...
    .select("project_id,name,data,updated_at,deleted,deleted_at")
...
async function pushProjectToCloud(project){
  if(!cloud.client || !project || !project.id) return;
...
async function deleteProjectFromCloud(projectId){
  if(!cloud.client || !projectId) return;
  const row = {
  ...
  if(localChanged){
    ...
    try{
      ...
      if(curId && !merged.some(p => p.id === curId)){
        localStorage.setItem(LS_CUR, merged[0]?.id || "");
      }
    }finally{
...
async function initCloudSync(){
  if(cloud.initTried) return;
  cloud.initTried = true;
  ...
  if(cloudSyncBtn){
    cloudSyncBtn.addEventListener("click", async () => {
      ...
      if(cloud.ready){
        ...
        alert("Cloud is not ready yet. Check your Supabase SQL/table setup.");
      }
    });
  }
