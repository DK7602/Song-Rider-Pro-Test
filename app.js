...
function pianoNote(ctx, freq, durMs, vel=0.9){
  ...
  scheduleCleanup([o1,o2,g1,g2,mix,ns,nf,ng,hp,body,lp,env,trem,lfo,lfoGain], (durMs + tail*1000 + 1200));
  return { out: trem, nodes:[o1,o2,g1,g2,mix,ns,nf,ng,hp,body,lp,env,trem,lfo,lfoGain] };
}
/***********************
...
function grandPianoNote(ctx, freq, durMs, vel=0.9){
  ...
  // clean pop-key stack: body + glass + shimmer
  const o1 = ctx.createOscillator();
  o1.type = "triangle";
  o1.frequency.value = f0;
  ...
  o3.type = "triangle";
  o3.frequency.value = Math.min(f0 * 3, 5200);

  const g1 = ctx.createGain(); g1.gain.value = 0.66;
  const g2 = ctx.createGain(); g2.gain.value = 0.24;
  const g3 = ctx.createGain(); g3.gain.value = 0.06;
  const mix = ctx.createGain();

  // smooth attack with controlled sustain so ... and _ still feel good
  const env = ctx.createGain();
  const peak = 0.050 * v;
  env.gain.setValueAtTime(0.0001, t0);
  env.gain.exponentialRampToValueAtTime(peak, t0 + 0.008);
  env.gain.setValueAtTime(peak * 0.84, t0 + 0.060);

  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.72, endTime);
  const tail = clamp(0.95 + dur * 0.62, 1.00, 4.20);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  // soft key click for definition, lighter than piano/Rhodes
  const nLen = 0.008;
  const bs = Math.max(256, Math.floor(ctx.sampleRate * nLen));
  const buf = ctx.createBuffer(1, bs, ctx.sampleRate);
  const data = buf.getChannelData(0);
  for(let i=0;i<data.length;i++){
    data[i] = (Math.random()*2 - 1) * (1 - i/data.length) * 0.14;
  }
  const ns = ctx.createBufferSource();
  ns.buffer = buf;
  ...
  clickBP.frequency.value = Math.min(2000, f0 * 4);
  clickBP.Q.value = 0.9;

  const clickG = ctx.createGain();
  clickG.gain.setValueAtTime(0.0001, t0);
  clickG.gain.exponentialRampToValueAtTime(0.0065 * v, t0 + 0.002);
  clickG.gain.exponentialRampToValueAtTime(0.0001, t0 + nLen);

  // polished pop EQ curve
  const hp = ctx.createBiquadFilter();
  hp.type = "highpass";
  ...
  // very subtle chorus width, different from the Piano button tone
  const chorus = ctx.createDelay();
  chorus.delayTime.value = 0.012;

  const chorusMix = ctx.createGain();
  chorusMix.gain.value = 0.16;
  ...
  scheduleCleanup([o1,o2,o3,g1,g2,g3,mix,ns,clickBP,clickG,hp,env,presence,chorus,chorusMix,dry,lp,outTrim,lfo,lfoGain], (durMs + tail*1000 + 1200));
  return { out: outTrim, nodes:[o1,o2,o3,g1,g2,g3,mix,ns,clickBP,clickG,hp,env,presence,chorus,chorusMix,dry,lp,outTrim,lfo,lfoGain] };
}
...
function playSingleNoteForInstrument(rawChord, durMs){
  ...
  if(state.instrument === "acoustic"){
    const n = grandPianoNote(ctx, f, durMs, 0.84);
