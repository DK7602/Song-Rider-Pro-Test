...
const CLOUD_SYNC_RETRY_KEY = "songrider_v25_cloud_sync_retry";
const CLOUD_SYNC_MAX_REASONS = 24;
const CLOUD_SYNC_BASE_DELAY_MS = 2500;
const CLOUD_SYNC_MAX_DELAY_MS = 45000;
...
function getCloudRetryStorageKey(scope){
  return scopedStorageKey(CLOUD_SYNC_RETRY_KEY, String(scope || getAuthUserId() || "guest"));
}

function hasPendingCloudSync(){
  return Array.isArray(cloud.pendingReasons) && cloud.pendingReasons.length > 0;
}

function getPendingCloudReasons(){
  return Array.isArray(cloud.pendingReasons) ? cloud.pendingReasons.slice() : [];
}

function persistCloudRetryState(scope){
  try{
    const key = getCloudRetryStorageKey(scope);
    const reasons = getPendingCloudReasons().filter(Boolean).slice(-CLOUD_SYNC_MAX_REASONS);
    if(!reasons.length && !cloud.retryAttempt && !cloud.lastError){
      localStorage.removeItem(key);
      return;
    }
    localStorage.setItem(key, JSON.stringify({
      reasons,
      retryAttempt: clamp(Number(cloud.retryAttempt) || 0, 0, 99),
...
function clearPersistedCloudRetryState(scope){
  try{ localStorage.removeItem(getCloudRetryStorageKey(scope)); }catch{}
}

function restoreCloudRetryState(scope){
  try{
    const raw = localStorage.getItem(getCloudRetryStorageKey(scope));
    if(!raw) return;
    const parsed = JSON.parse(raw);
    const reasons = Array.isArray(parsed?.reasons) ? parsed.reasons.map(x => String(x || "").trim()).filter(Boolean) : [];
    cloud.pendingReasons = [];
    cloud.pendingReasonSet = new Set();
    reasons.slice(-CLOUD_SYNC_MAX_REASONS).forEach(reason => {
      if(!cloud.pendingReasonSet.has(reason)){
        cloud.pendingReasonSet.add(reason);
        cloud.pendingReasons.push(reason);
      }
    });
    cloud.retryAttempt = clamp(Number(parsed?.retryAttempt) || 0, 0, 99);
    cloud.lastError = String(parsed?.lastError || "");
  }catch{}
}
...
function enqueueCloudReason(reason=""){
  const r = String(reason || "").trim() || "pending";
  if(!(cloud.pendingReasonSet instanceof Set)) cloud.pendingReasonSet = new Set();
  if(!Array.isArray(cloud.pendingReasons)) cloud.pendingReasons = [];
  if(!cloud.pendingReasonSet.has(r)){
    cloud.pendingReasonSet.add(r);
    cloud.pendingReasons.push(r);
    while(cloud.pendingReasons.length > CLOUD_SYNC_MAX_REASONS){
      const dropped = cloud.pendingReasons.shift();
      cloud.pendingReasonSet.delete(dropped);
    }
  }
  cloud.lastQueuedAt = now();
  persistCloudRetryState();
}

function consumePendingCloudReasons(fallbackReason=""){
  const reasons = getPendingCloudReasons();
  cloud.pendingReasons = [];
  cloud.pendingReasonSet = new Set();
  const fallback = String(fallbackReason || "").trim();
  if(fallback && !reasons.includes(fallback)) reasons.push(fallback);
  persistCloudRetryState();
  return reasons;
}
...
function scheduleCloudRetry(reason="", immediate=false){
  if(reason) enqueueCloudReason(reason);
  ...
  if(immediate){
    cloud.retryTimer = setTimeout(() => {
      cloud.retryTimer = null;
      const reasons = consumePendingCloudReasons("retry-immediate");
      syncAllProjectsToCloud(reasons.join(", "), reasons).catch(err => {
        console.error("Immediate retry cloud sync failed", err);
      });
    }, 120);
    return;
  }

  const attempt = Math.max(0, Number(cloud.retryAttempt) || 0);
  const delay = Math.min(CLOUD_SYNC_MAX_DELAY_MS, CLOUD_SYNC_BASE_DELAY_MS * Math.pow(2, Math.min(attempt, 4)));
  cloud.retryTimer = setTimeout(() => {
    cloud.retryTimer = null;
    const reasons = consumePendingCloudReasons("retry");
    syncAllProjectsToCloud(reasons.join(", "), reasons).catch(err => {
      console.error("Retry cloud sync failed", err);
    });
  }, delay);
}
...
function setCloudStatus(kind, extra=""){
  ...
  if(kind === "ok"){
    const pending = hasPendingCloudSync() ? ` • ${getPendingCloudReasons().length} pending` : "";
    btn.textContent = "☁︎";
    btn.title = cloud.user ? `Signed in: ${cloud.user.email || cloud.user.id}${pending}` : "Cloud ready";
    btn.style.opacity = "1";
    return;
  }
  if(kind === "error"){
    const pending = hasPendingCloudSync() ? ` • retrying ${getPendingCloudReasons().length} pending` : "";
    btn.textContent = "☁︎!";
    btn.title = "Cloud error" + (extra ? ": " + extra : "") + pending;
    btn.style.opacity = "1";
    return;
  }
...
function queueCloudSync(reason=""){
  if(cloud.suspendQueue) return;
  enqueueCloudReason(reason || "debounced");
  if(!cloud.ready || !cloud.client || !getAuthUserId()) return;
  ...
  cloud.queueTimer = setTimeout(() => {
    ...
    const reasons = consumePendingCloudReasons(reason || "debounced");
    syncAllProjectsToCloud(reasons.join(", "), reasons).catch(err => {
...
function triggerImmediateCloudSync(reason=""){
  if(cloud.suspendQueue) return;
  enqueueCloudReason(reason || "immediate");
  if(!cloud.ready || !cloud.client || !getAuthUserId()) return;
  if(cloud.queueTimer){
    clearTimeout(cloud.queueTimer);
    cloud.queueTimer = null;
  }
  scheduleCloudRetry("", true);
}

async function syncAllProjectsToCloud(reason="", explicitReasons){
  if(cloud.suspendQueue) return;
  const reasons = (Array.isArray(explicitReasons) ? explicitReasons : [reason]).map(x => String(x || "").trim()).filter(Boolean);
  if(cloud.syncing){
    reasons.forEach(enqueueCloudReason);
    cloud.syncQueued = true;
    return;
  }
  ...
  if(typeof navigator !== "undefined" && "onLine" in navigator && navigator.onLine === false){
    reasons.forEach(enqueueCloudReason);
    setCloudStatus("error", "Offline. Waiting to retry.");
    scheduleCloudRetry();
    return;
  }
  ...
  try{
    ...
    reasons.forEach(enqueueCloudReason);
    cloud.retryAttempt = clamp((Number(cloud.retryAttempt) || 0) + 1, 0, 99);
    cloud.lastError = msg;
    persistCloudRetryState();
    ...
    cloud.syncing = false;
    if((cloud.syncQueued || hasPendingCloudSync()) && !cloud.suspendQueue && cloud.ready && cloud.client && getAuthUserId()){
      cloud.syncQueued = false;
      clearCloudRetryTimer();
      cloud.retryTimer = setTimeout(() => {
        cloud.retryTimer = null;
        const followupReasons = consumePendingCloudReasons("queued-followup");
        syncAllProjectsToCloud(followupReasons.join(", "), followupReasons).catch(err => {
...
async function handleAuthSessionChange(session, reason=""){
  ...
  if(cloud.user){
    ...
    restoreCloudRetryState(getAuthUserId());
    cloud.ready = true;
    setCloudStatus("syncing");
    try{
      await mergeCloudIntoLocal();
      ...
      if(hasPendingCloudSync()) scheduleCloudRetry("", true);
      updateAuthModalUI();
    }catch(err){
      if(seq !== cloud.authChangeSeq) return;
  ...
  try{
    ...
    if(!cloud.onlineListenerWired){
      ...
      window.addEventListener("online", () => {
        if(cloud.user){
          scheduleCloudRetry("online", true);
        }
      });

      window.addEventListener("focus", () => {
        if(cloud.user && (hasPendingCloudSync() || cloud.lastError)){
          scheduleCloudRetry("focus", true);
        }
      });
      ...
      document.addEventListener("visibilitychange", () => {
        ...
        if(document.visibilityState === "visible"){
          if(hasPendingCloudSync() || cloud.lastError) scheduleCloudRetry("visible", true);
          return;
        }
        if(document.visibilityState === "hidden" && hasPendingCloudSync()){
          triggerImmediateCloudSync("background");
        }
      });

      window.addEventListener("pagehide", () => {
        if(cloud.user && hasPendingCloudSync()){
          triggerImmediateCloudSync("pagehide");
        }
      }, { capture:true });
    }
  }catch(err){
...
async function tombstoneRecordingInProject(projectId, rec){
  ...
  if(cloud.ready && cloud.user) triggerImmediateCloudSync("recording-delete");
}
...
function normalizeProject(p){
  ...
  if(notesBackup){
    if(!String(p.notesDoc || "").trim() && String(notesBackup.notesDoc || "").trim()){
      p.notesDoc = String(notesBackup.notesDoc || "");
    }
    if((!String(p.notesColor || "").trim() || p.notesColor === "#151515") && String(notesBackup.notesColor || "").trim()){
      p.notesColor = String(notesBackup.notesColor || "");
    }
  }
...
applyTick();
  ...
  try{
    ...
    if(rec){
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      if(cloud.ready && cloud.user) triggerImmediateCloudSync("recording-offset");
      renderRecordings();
    }
  }catch{}
}
...
async function uploadAudioFile(){
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      ...
      await dbPut(item);
      if(cloud.ready && cloud.user) triggerImmediateCloudSync("recording-upload");
...
async function renderRecordings(){
  ...
  mine.forEach(rec => {
    ...
    edit.addEventListener("click", async () => {
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      if(cloud.ready && cloud.user) triggerImmediateCloudSync("recording-rename");
      renderRecordings();
    });
...
await dbPut(item);
if(cloud.ready && cloud.user) triggerImmediateCloudSync("recording-new");
await renderRecordings();
...
if(el.capoInput){
  ...
  if(cloudSyncBtn){
    cloudSyncBtn.addEventListener("click", async () => {
      await initCloudSync();
      if(cloud.user){
        triggerImmediateCloudSync("manual-cloud-button");
      }
      openAuthModal();
    });
  }
