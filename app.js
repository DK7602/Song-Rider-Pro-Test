...
function setRecordUI(){
  ...
  const useMiniRecordIcon = panelHidden && !!state.autoScrollOn;

  // Mini bar: keep text normally, but switch to compact round record icon while AutoScroll is active.
  if(el.mRecordBtn){
    el.mRecordBtn.classList.toggle("iconCircle", useMiniRecordIcon);
    el.mRecordBtn.classList.toggle("recCircle", useMiniRecordIcon);
    el.mRecordBtn.classList.toggle("btn", !useMiniRecordIcon);
    el.mRecordBtn.classList.toggle("rec", !useMiniRecordIcon);
    el.mRecordBtn.classList.toggle("recording", state.isRecording);

    if(useMiniRecordIcon){
      el.mRecordBtn.textContent = "";
      el.mRecordBtn.style.width = "34px";
      el.mRecordBtn.style.minWidth = "34px";
      el.mRecordBtn.style.height = "34px";
      el.mRecordBtn.style.padding = "0";
      el.mRecordBtn.style.fontSize = "0";
      el.mRecordBtn.style.flex = "0 0 auto";
    }else{
      el.mRecordBtn.textContent = state.isRecording ? "Stop" : "Record";
      el.mRecordBtn.style.width = "";
      el.mRecordBtn.style.minWidth = "";
      el.mRecordBtn.style.height = "";
      el.mRecordBtn.style.padding = "";
      el.mRecordBtn.style.fontSize = "";
      el.mRecordBtn.style.flex = "";
    }
...
function renderProjectsDropdown(){
  const all = loadAllProjects().map(normalizeProject).filter(Boolean);
  const sort = (el.sortSelect && el.sortSelect.value) ? el.sortSelect.value : getSavedProjectSort();
