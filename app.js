...
function parseFullTextToSectionCards(fullText){
  ...
  const CHORD_TOKEN_RE =
    /^([A-G](?:#|b)?)(?:(?:maj|min|m|dim|aug|\+|sus|add)?(?:2|4|5|6|7|9|11|13)?(?:maj7|M7|m7|m9|m11|m13|sus2|sus4|add9|add11|add13|dim7|hdim7|m7b5)?(?:b5|#5|b9|#9|b11|#11|b13|#13)?)?(?:\/[A-G](?:#|b)?)?$|^N\.?C\.?$/i;
  ...
  // Extract inline chords like [C] or (G) or {Dm7} and return cleaned lyric + chord list
  ...
  function lineIsMostlyChords(line){
    ...
    // ✅ chord line if at least 1 chord token and the majority of tokens are chords
    if(chordish >= 1 && chordish >= Math.ceil(tokens.length * 0.6)) return true;

    // ✅ also allow 1-token chord line (very common)
    if(tokens.length === 1 && chordish === 1) return true;
  ...
  function pushCard(lyricsRaw, slots){
    ...
    if(!lyrics && !chords.some(x => String(x||"").trim())) return;

    ensureBucket(cur);
    buckets[cur].push({ lyrics, chords });
  }

  function flushCard(){ /* no-op */ }
