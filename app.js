...
async function extractTextFromPdfFile(file){
  // ensure pdf.js is available (local first, CDN fallback)
  let pdfjsLib = window.pdfjsLib;
  let usingCdn = false;

  async function tryLoadPdfJs(){
    // 1) already present (from index module preload)
    if(pdfjsLib && pdfjsLib.getDocument) return;

    // 2) local bundled pdf.mjs
    try{
    ...
    // 3) CDN fallback (works even if local files are missing)
    try{
  ...
  // PDF.js worker (local first, CDN fallback)
  try{
    const workerLocal = "./pdf.worker.mjs";
    const workerCdn = "https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/build/pdf.worker.mjs";
    if(pdfjsLib.GlobalWorkerOptions){
      pdfjsLib.GlobalWorkerOptions.workerSrc = usingCdn ? workerCdn : workerLocal;
    }
  }catch(_e){}
