/* app.js (FULL REPLACE MAIN v46_unfrozen_pdfHeadings_glowfix) */
(() => {
...
let pendingFullBlockScroll = null;
...
// BOOT-SAFE STUB: some deployed builds have hit a startup ReferenceError on
// updateFullIfVisible during init. Define a harmless stub early; the real
// implementation later in the file will override this function.
...
function _typingViewportBottom(pad = 18){
  try{
    const vv = window.visualViewport;
    if(vv && Number.isFinite(vv.height)){
      return Math.min(window.innerHeight || 0, (vv.offsetTop || 0) + vv.height) - pad;
    }
  }catch{}
  return (window.innerHeight || document.documentElement.clientHeight || 0) - pad;
}
function _getSheetScroller(){
  return document.getElementById("sheetBody") || document.scrollingElement || document.documentElement;
}
function keepFullTextareaSteady(ta, snap){
  if(!ta) return;
  const scroller = _getSheetScroller();
  if(!scroller) return;

  const lockedTop  = Number(snap && snap.scrollTop);
  const prevTop    = Number(snap && snap.top);
  const prevBottom = Number(snap && snap.bottom);

  const apply = () => {
    // ✅ Strongly preserve the editor viewport while typing.
    // Android/WebView can auto-scroll after textarea autosize,
    // which makes Full Song View bounce while typing.
    if(Number.isFinite(lockedTop)){
      scroller.scrollTop = lockedTop;
    }

    // If the textarea itself drifted in the viewport, cancel that drift too.
    if(Number.isFinite(prevTop)){
      const rectNow = ta.getBoundingClientRect();
      const drift = rectNow.top - prevTop;
      if(Math.abs(drift) > 0.5){
        scroller.scrollTop += Math.round(drift);
      }
    }

    // Minimal rescue only if the textarea bottom actually drops below the visible typing area.
    const rect = ta.getBoundingClientRect();
    const limit = _typingViewportBottom(18);
    if(rect.bottom > limit && (!Number.isFinite(prevBottom) || prevBottom <= limit)){
      scroller.scrollTop += Math.ceil(rect.bottom - limit);
    }
  };
...
function ensureFullTextareaVisible(ta){
  if(!ta) return;
  const scroller = _getSheetScroller();
  if(!scroller) return;
  const rect = ta.getBoundingClientRect();
  const limit = _typingViewportBottom(22);

  if(rect.bottom > limit){
    scroller.scrollTop += Math.ceil(rect.bottom - limit);
  }else if(rect.top < 92){
    scroller.scrollTop -= Math.ceil(92 - rect.top);
  }
}
function scrollFullSectionCardIntoView(card, opts={}){
  if(!card) return;
  const scroller = _getSheetScroller();
  if(!scroller) return;
  ...
  const topPad = 92;
  const bottomPad = 34;
  const desiredTop = currentTop + (cardRect.top - hostRect.top) - topPad;
  const desiredBottom = currentTop + (cardRect.bottom - hostRect.bottom) + bottomPad;

  let targetTop = currentTop;
  if(cardRect.top < hostRect.top + topPad){
    targetTop = desiredTop;
  }else if(cardRect.bottom > hostRect.bottom - bottomPad){
    targetTop = desiredBottom;
  }else if(opts.forceAlignTop){
    targetTop = desiredTop;
  }
  ...
  if(focusTextarea){
    const ta = card.querySelector('textarea.fullSectionText');
    if(ta){
      ...
      requestAnimationFrame(() => ensureFullTextareaVisible(ta));
      setTimeout(() => ensureFullTextareaVisible(ta), 60);
    }
  }
}
...
saveBtn: $("saveBtn"),
  ...
  mInstrumentSelect: $("mInstrumentSelect"),
  noteLenSelect: $("noteLenSelect"),
  mNoteLenSelect: $("mNoteLenSelect"),

  exportBtn: $("exportBtn"),
  notesBtn: $("notesBtn"),
  archiveBtn: $("archiveBtn"),
  archiveLoadedName: $("archiveLoadedName"),
  archiveModal: $("archiveModal"),
  archiveCloseBtn: $("archiveCloseBtn"),
  archiveSortSelect: $("archiveSortSelect"),
  archiveAddBtn: $("archiveAddBtn"),
  archiveList: $("archiveList"),
  archiveAddPanel: $("archiveAddPanel"),
  archiveAddList: $("archiveAddList"),
  archiveMoveModal: $("archiveMoveModal"),
  archiveMoveText: $("archiveMoveText"),
  archiveMoveConfirmBtn: $("archiveMoveConfirmBtn"),
  archiveMoveCancelBtn: $("archiveMoveCancelBtn"),
  projectPickerBtn: $("projectPickerBtn"),
  projectPickerLabel: $("projectPickerLabel"),
  projectModal: $("projectModal"),
  projectCloseBtn: $("projectCloseBtn"),
  projectSortSelect: $("projectSortSelect"),
...
style.textContent = `
  ...
  /* ===== BPM: compact on mobile, roomier on desktop ===== */
  #bpmInput{
    width:34px !important;
...
function injectChordCopyModeStyle(){
  const old = document.getElementById("srpChordCopyModeStyle");
  if(old) old.remove();

  const style = document.createElement("style");
  style.id = "srpChordCopyModeStyle";
  style.textContent = `
    body.chordCopyMode .noteCell{
      transition: box-shadow .14s ease, transform .14s ease, filter .14s ease;
    }
    body.chordCopyMode .noteCell.chordCopyTarget{
      box-shadow: 0 0 0 2px rgba(37,99,235,.22), 0 0 0 5px rgba(37,99,235,.10);
    }
    body.chordCopyMode .noteCell.chordCopySource{
      box-shadow: 0 0 0 2px rgba(22,163,74,.28), 0 0 0 6px rgba(22,163,74,.14);
      filter: brightness(1.04);
    }
    body.chordCopyMode .card.chordCopySourceCard{
...
function clearChordCopyTimer(){
  if(chordDragCopy.timer){
    clearTimeout(chordDragCopy.timer);
    chordDragCopy.timer = null;
  }
}

function clearChordCopyClasses(){
  try{
    document.body.classList.remove("chordCopyMode");
    document.querySelectorAll('.noteCell.chordCopyTarget, .noteCell.chordCopySource').forEach(elm => {
      elm.classList.remove('chordCopyTarget','chordCopySource');
    });
    document.querySelectorAll('.card.chordCopySourceCard').forEach(elm => {
      elm.classList.remove('chordCopySourceCard');
    });
  }catch{}
}

function cancelChordDragCopy(){
  clearChordCopyTimer();
  chordDragCopy.active = false;
  chordDragCopy.sourceLineId = "";
  chordDragCopy.sourceNotes = Array(8).fill("");
  chordDragCopy.sourceInput = null;
  chordDragCopy.sourceCard = null;
  chordDragCopy.pointerId = null;
  try{ document.querySelectorAll('.noteCell[data-pending-tap-focus="1"]').forEach(elm => delete elm.dataset.pendingTapFocus); }catch{}
  clearChordCopyClasses();
}

function forceCollapseMobileKeyboard(sourceInput){
  try{
...
function activateChordDragCopy(sourceInput, line, card){
  if(!line) return;
  cancelChordDragCopy();
  const notes = getLineNotes8(line);
  if(!notes.some(Boolean)) return;

  chordDragCopy.active = true;
  chordDragCopy.sourceLineId = String(line.id || "");
  chordDragCopy.sourceNotes = notes.slice(0,8);
  chordDragCopy.sourceInput = sourceInput || null;
  chordDragCopy.sourceCard = card || null;

  try{
    forceCollapseMobileKeyboard(sourceInput);
    document.body.classList.add('chordCopyMode');
    document.querySelectorAll('.noteCell').forEach(elm => {
      elm.classList.add('chordCopyTarget');
    });
    if(sourceInput) sourceInput.classList.add('chordCopySource');
    if(card) card.classList.add('chordCopySourceCard');
  }catch{}
}

function scheduleChordDragCopyHold(sourceInput, line, card, e){
  clearChordCopyTimer();
  chordDragCopy.startX = Number(e?.clientX || 0);
  chordDragCopy.startY = Number(e?.clientY || 0);
  chordDragCopy.pointerId = (e && Number.isFinite(e.pointerId)) ? e.pointerId : null;
  chordDragCopy.timer = setTimeout(() => {
    chordDragCopy.timer = null;
    activateChordDragCopy(sourceInput, line, card);
  }, 380);
}

function maybeCancelChordCopyHoldFromMove(e){
  if(!chordDragCopy.timer) return;
  if(chordDragCopy.pointerId != null && e && Number.isFinite(e.pointerId) && e.pointerId !== chordDragCopy.pointerId) return;
  const x = Number(e?.clientX || 0);
  const y = Number(e?.clientY || 0);
  if(Math.abs(x - chordDragCopy.startX) > 12 || Math.abs(y - chordDragCopy.startY) > 12){
    clearChordCopyTimer();
  }
}

function commitChordDragCopyToLine(targetLine, targetCard){
  if(!chordDragCopy.active || !targetLine) return false;
  const src = Array.isArray(chordDragCopy.sourceNotes) ? chordDragCopy.sourceNotes.slice(0,8) : Array(8).fill('');
  if(!src.some(Boolean)) return false;

  editProject('dragChordCopy', () => {
    setLineNotes8(targetLine, src);
    syncFullTextFromSections();
  });

  const targetInputs = Array.from((targetCard || document).querySelectorAll?.('.noteCell') || []);
  applyChordRowUiRefresh(targetCard, targetLine, targetInputs);
  cancelChordDragCopy();
  return true;
}

function wireChordDragCopyGlobalCancel(){
  if(document.body && document.body.dataset.chordCopyGlobalWired === '1') return;
  if(document.body) document.body.dataset.chordCopyGlobalWired = '1';

  document.addEventListener('click', (e) => {
    if(!chordDragCopy.active) return;
    const inNoteCell = !!(e.target && e.target.closest && e.target.closest('.noteCell'));
    if(!inNoteCell) cancelChordDragCopy();
  }, true);
  ...
  window.addEventListener('scroll', cancelOnScroll, { passive:true, capture:true });
  document.getElementById('sheetBody')?.addEventListener('scroll', cancelOnScroll, { passive:true });
}
...
function applyFullTextToProjectSections(fullText){
  ...
  secsToApply.forEach(sec => {
    ...
    for(let i=0; i<want.length; i++){
      ...
      // ✅ Full view is the source of truth when editing there.
      // Always replace the card chord row from parsed Full text so removed/moved
      // chords do not linger on cards and appear as duplicates/autofill.
...
const LS_SORT = "songrider_v25_projectSort";
const LS_ARCHIVE_SORT = "songrider_v25_archiveSort";
const LS_NOTES_BACKUP_PREFIX = "songrider_v25_notes_backup::";

function getAuthUserId(){
  return String(cloud?.user?.id || "").trim();
}
...
function scopedStorageKey(base, scope){
  return `${String(base || "") }::${String(scope || getStorageScope())}`;
}

function getProjectsStorageKey(scope){
  return scopedStorageKey(LS_KEY, scope);
}

function getCurrentProjectStorageKey(scope){
  return scopedStorageKey(LS_CUR, scope);
}

function getSortStorageKey(scope){
  return scopedStorageKey(LS_SORT, scope);
}

function getSavedProjectSort(scope){
  try{
    const v = String(localStorage.getItem(getSortStorageKey(scope)) || "").trim().toLowerCase();
    return (v === "recent") ? "recent" : "az";
  }catch{
    return "az";
  }
}

function saveProjectSort(v, scope){
  try{
    localStorage.setItem(getSortStorageKey(scope), (String(v || "").trim().toLowerCase() === "recent") ? "recent" : "az");
  }catch{}
}

function getArchiveSortStorageKey(scope){
  return scopedStorageKey(LS_ARCHIVE_SORT, scope);
}

function getSavedArchiveSort(scope){
  try{
    const v = String(localStorage.getItem(getArchiveSortStorageKey(scope)) || "").trim().toLowerCase();
    return (v === "recent") ? "recent" : "az";
  }catch{
    return "recent";
  }
...
function saveArchiveSort(v, scope){
  try{
    localStorage.setItem(getArchiveSortStorageKey(scope), (String(v || "").trim().toLowerCase() === "az") ? "az" : "recent");
  }catch{}
}

function notesBackupKey(projectId, scope){
  return `${LS_NOTES_BACKUP_PREFIX}${String(scope || getStorageScope())}::${projectId || ""}`;
}

function legacyNotesBackupKey(projectId){
  return `${LS_NOTES_BACKUP_PREFIX}${projectId || ""}`;
}

function parseNotesBackupRaw(raw){
  try{
    if(!raw) return null;
    const parsed = JSON.parse(raw);
    if(!parsed || typeof parsed !== "object") return null;
    return {
      notesDoc: (typeof parsed.notesDoc === "string") ? parsed.notesDoc : "",
      notesColor: (typeof parsed.notesColor === "string") ? parsed.notesColor : "",
...
function pickBestNotesBackup(items){
  ...
  for(const item of list){
    const doc = String(item?.notesDoc || "");
    const hasDoc = !!doc.trim();
    const score = (Number(item?.updatedAt) || 0) + (hasDoc ? 1e15 : 0);
    if(score > bestScore){
      best = item;
      bestScore = score;
    }
...
function readNotesBackup(projectId){
  try{
    if(!projectId) return null;
    const candidates = [];
    const seen = new Set();
    const scopes = [getStorageScope(), "guest", getAuthUserId()].filter(Boolean);

    for(const scope of scopes){
      const key = notesBackupKey(projectId, scope);
      if(seen.has(key)) continue;
      seen.add(key);
      const parsed = parseNotesBackupRaw(localStorage.getItem(key));
      if(parsed) candidates.push(parsed);
    }

    const legacy = parseNotesBackupRaw(localStorage.getItem(legacyNotesBackupKey(projectId)));
    if(legacy) candidates.push(legacy);

    return pickBestNotesBackup(candidates);
  }catch{
    return null;
  }
}

function writeNotesBackup(project){
  try{
    if(!project || !project.id) return;
    const payload = {
      notesDoc: String(project.notesDoc || ""),
      notesColor: String(project.notesColor || ""),
    ...
    const seen = new Set();
    for(const scope of scopes){
      const key = notesBackupKey(project.id, scope);
      if(seen.has(key)) continue;
      seen.add(key);
      localStorage.setItem(key, json);
    }
    localStorage.setItem(legacyNotesBackupKey(project.id), json);
  }catch{}
}

function deleteNotesBackup(projectId){
  try{
    if(!projectId) return;
    const scopes = [getStorageScope(), "guest", getAuthUserId()].filter(Boolean);
    const seen = new Set();
    ...
    localStorage.removeItem(legacyNotesBackupKey(projectId));
  }catch{}
}
...
/***********************
CLOUD SYNC (Supabase)
- Local-first app with automatic cloud backup/restore
- Keeps existing local behavior intact
- Sync target: public.srp_projects
***********************/
...
const CLOUD_PROJECTS_TABLE = "srp_user_projects";
const CLOUD_AUDIO_BUCKET = "audio-files";
...
function setCloudStatus(kind, extra=""){
  cloud.status = kind || "local";
  cloud.lastError = extra || cloud.lastError || "";
  ...
  if(kind === "syncing"){
    btn.textContent = "☁︎";
    btn.title = cloud.user ? `Cloud syncing for ${cloud.user.email || "account"}` : "Cloud syncing…";
    btn.style.opacity = "1";
    return;
  }
  if(kind === "ok"){
    btn.textContent = "☁︎";
    btn.title = cloud.user ? `Signed in: ${cloud.user.email || cloud.user.id}` : "Cloud ready";
    btn.style.opacity = "1";
    return;
  }
  if(kind === "error"){
    btn.textContent = "☁︎!";
    btn.title = "Cloud error" + (extra ? ": " + extra : "");
    btn.style.opacity = "1";
    return;
  }

  btn.textContent = "☁︎";
  btn.title = cloud.user ? `Signed in: ${cloud.user.email || cloud.user.id}` : "Sign in for private cloud sync";
  btn.style.opacity = cloud.user ? "1" : ".72";
}

function serializeProjectForCloud(project){
  const normalized = normalizeProject(deepClone(project));
  return {
    user_id: getAuthUserId(),
    project_id: normalized.id,
    name: String(normalized.name || "Untitled"),
    data: normalized,
    updated_at: Number(normalized.updatedAt) || now(),
    deleted: false,
    deleted_at: null,
...
function projectFromCloudRow(row){
  ...
  const data = (row.data && typeof row.data === "object") ? deepClone(row.data) : {};
  data.id = String(data.id || row.project_id || "");
  if(!data.id) return null;
  if(typeof row.name === "string" && row.name.trim()) data.name = row.name.trim();
  if(Number.isFinite(Number(row.updated_at))){
    data.updatedAt = Number(row.updated_at);
  }
  return normalizeProject(data);
}

async function fetchCloudProjects(){
  if(!cloud.client || !getAuthUserId()) return [];
  const { data, error } = await cloud.client
    .from(CLOUD_PROJECTS_TABLE)
    .select("user_id,project_id,name,data,updated_at,deleted,deleted_at")
...
async function pushProjectToCloud(project){
  if(!cloud.client || !getAuthUserId() || !project || !project.id) return;
  const row = serializeProjectForCloud(project);
...
async function deleteProjectFromCloud(projectId){
  if(!cloud.client || !getAuthUserId() || !projectId) return;
  const row = {
    user_id: getAuthUserId(),
    project_id: String(projectId),
    name: "",
    data: { id: String(projectId), deleted: true },
    updated_at: now(),
    deleted: true,
...
async function mergeCloudIntoLocal(){
  ...
  const remoteRows = await fetchCloudProjects();

  const remoteMap = new Map();
  for(const row of remoteRows){
    if(!row || !row.project_id) continue;
    remoteMap.set(String(row.project_id), row);
  }

  const localList = loadAllProjects().map(normalizeProject).filter(Boolean);
  const localMap = new Map(localList.map(p => [String(p.id), p]));

  let localChanged = false;
  let remoteNeedsPush = false;

  // apply cloud deletions / newer cloud versions
  for(const [id, row] of remoteMap.entries()){
    const local = localMap.get(id);
    const rowUpdated = Number(row.updated_at) || 0;

    if(row.deleted){
      if(local && (rowUpdated >= (Number(local.updatedAt) || 0))){
        localMap.delete(id);
        localChanged = true;
      }else if(local){
        remoteNeedsPush = true;
      }
      continue;
    }

    const remoteProject = projectFromCloudRow(row);
    if(!remoteProject) continue;

    if(!local){
      localMap.set(id, remoteProject);
      localChanged = true;
      continue;
    }

    const localUpdated = Number(local.updatedAt) || 0;
    if(rowUpdated > localUpdated){
      const bestNotes = pickBestNotesBackup([
        { notesDoc: remoteProject.notesDoc, notesColor: remoteProject.notesColor, updatedAt: Number(remoteProject.updatedAt) || rowUpdated },
        { notesDoc: local.notesDoc, notesColor: local.notesColor, updatedAt: localUpdated },
        readNotesBackup(id)
      ]);
      if(bestNotes){
        if(String(bestNotes.notesDoc || "").trim()) remoteProject.notesDoc = String(bestNotes.notesDoc || "");
        if(String(bestNotes.notesColor || "").trim()) remoteProject.notesColor = String(bestNotes.notesColor || remoteProject.notesColor || "");
      }
      localMap.set(id, remoteProject);
      localChanged = true;
    }else if(localUpdated > rowUpdated){
      remoteNeedsPush = true;
    }
  }

  // local-only projects should get pushed up
  for(const [id] of localMap.entries()){
    if(!remoteMap.has(id)) remoteNeedsPush = true;
  }

  if(localChanged){
    const merged = Array.from(localMap.values()).sort((a,b)=>(Number(b.updatedAt)||0) - (Number(a.updatedAt)||0));
    cloud.suspendQueue = true;
    try{
      saveAllProjects(merged);
      const curId = getCurrentProjectIdForScope(getStorageScope());
      if(curId && !merged.some(p => p.id === curId)){
        setCurrentProjectIdForScope(getStorageScope(), merged[0]?.id || "");
      }
    }finally{
      cloud.suspendQueue = false;
    }
  }

  scheduleDeferredCloudAudioSync("merge");
  scheduleDeferredCloudNoteImageSync("merge");
...
function queueCloudSync(reason=""){
  if(cloud.suspendQueue) return;
  if(!cloud.ready || !cloud.client || !getAuthUserId()) return;

  if(cloud.queueTimer) clearTimeout(cloud.queueTimer);
  cloud.syncQueued = true;
  cloud.queueTimer = setTimeout(() => {
    cloud.queueTimer = null;
    syncAllProjectsToCloud(reason).catch(err => {
      console.error("Cloud sync failed", err);
    });
  }, 700);
}

function triggerImmediateCloudSync(reason=""){
  // Recording changes can enqueue sync even before auth/cloud is fully ready.
  // triggerImmediateCloudSync() persists the reason first, then syncs immediately when possible.
  if(cloud.suspendQueue) return;
  if(!cloud.ready || !cloud.client || !getAuthUserId()) return;
  if(cloud.queueTimer){
    clearTimeout(cloud.queueTimer);
    cloud.queueTimer = null;
  }
  cloud.syncQueued = true;
  setTimeout(() => {
    syncAllProjectsToCloud(reason || "immediate").catch(err => {
      console.error("Immediate cloud sync failed", err);
    });
  }, 120);
}

async function syncAllProjectsToCloud(reason=""){
  if(cloud.suspendQueue) return;
  if(cloud.syncing){
    cloud.syncQueued = true;
    return;
  ...
  cloud.syncing = true;
  cloud.syncQueued = false;
  setCloudStatus("syncing");

  try{
    // ✅ Always start by pulling latest cloud state down first.
    // This makes "Sync Now" truly bidirectional so a second device can fetch
    // newly uploaded recordings / manifests instead of only pushing its own state.
    await mergeCloudIntoLocal();

    // ✅ Upload any local audio blobs and refresh per-project manifests.
    await syncCloudAudioForAllProjects(reason || "projects");
    await syncCloudNoteImagesForAllProjects(reason || "projects");

    // ✅ Re-read projects AFTER audio sync so freshly-written manifests get pushed
    // in the same sync pass (important for new recordings/uploads).
    const all = loadAllProjects().map(normalizeProject).filter(Boolean);
    for(const project of all){
      await pushProjectToCloud(project);
    }

    // ✅ Pull once more so this device also picks up anything that landed remotely
    // during the same sync window.
    await mergeCloudIntoLocal();

    setCloudStatus("ok");
  }catch(err){
    console.error("Cloud sync failed", reason, err);
    setCloudStatus("error", err?.message || String(err || "Unknown error"));
    throw err;
  }finally{
    cloud.syncing = false;
    if(cloud.syncQueued && !cloud.suspendQueue && cloud.ready && cloud.client && getAuthUserId()){
      cloud.syncQueued = false;
      setTimeout(() => {
        syncAllProjectsToCloud("queued-followup").catch(err => {
          console.error("Queued cloud sync failed", err);
        });
      }, 180);
    }
  }
...
function setAuthMode(mode="signin"){
  cloud.authMode = mode || "signin";
  cloud.recoveryMode = (cloud.authMode === "recovery");

  const modal = document.getElementById("authModal");
  if(modal) modal.classList.toggle("resetting", cloud.authMode === "forgot" || cloud.authMode === "recovery");

  const resetEmail = document.getElementById("authResetEmailInput");
  const newPass = document.getElementById("authNewPasswordInput");
  const sendBtn = document.getElementById("authSendResetBtn");
  const updateBtn = document.getElementById("authUpdatePasswordBtn");
  ...
  if(resetEmail){
    resetEmail.style.display = (cloud.authMode === "forgot") ? "block" : "none";
    if(emailInput && !resetEmail.value) resetEmail.value = String(emailInput.value || "").trim();
  }
  if(newPass) newPass.style.display = (cloud.authMode === "recovery") ? "block" : "none";
  if(sendBtn) sendBtn.style.display = (cloud.authMode === "forgot") ? "inline-flex" : "none";
  if(updateBtn) updateBtn.style.display = (cloud.authMode === "recovery") ? "inline-flex" : "none";
  if(backBtn) backBtn.textContent = (cloud.authMode === "recovery") ? "Back to Sign In" : "Back to Sign In";
  if(forgotRow) forgotRow.style.display = (cloud.authMode === "signin") ? "flex" : "none";
}

function updateAuthModalUI(message=""){
  const modal = document.getElementById("authModal");
  if(!modal) return;
  const signedIn = !!cloud.user;
  modal.classList.toggle("signed-in", signedIn && cloud.authMode !== "forgot" && cloud.authMode !== "recovery");
  modal.classList.toggle("signed-out", !signedIn && cloud.authMode !== "forgot" && cloud.authMode !== "recovery");

  const status = document.getElementById("authStatus");
  const userEmail = document.getElementById("authUserEmail");
  const syncBtn = document.getElementById("authSyncNowBtn");
  ...
  const resetEmailInput = document.getElementById("authResetEmailInput");
  ...
  const updatePasswordBtn = document.getElementById("authUpdatePasswordBtn");
  const backBtn = document.getElementById("authBackToSignInBtn");

  setAuthMode(cloud.authMode || "signin");

  if(status){
    if(message) status.textContent = message;
    else if(cloud.authMode === "forgot") status.textContent = "Enter your email and we’ll send you a password reset link.";
    else if(cloud.authMode === "recovery") status.textContent = "Enter your new password below.";
    else if(cloud.user) status.textContent = "Projects sync privately to your signed-in account.";
    else status.textContent = "Sign in so your projects follow you across devices.";
  }
  if(userEmail) userEmail.textContent = cloud.user ? (cloud.user.email || cloud.user.id || "Signed in") : "";

  [syncBtn, signOutBtn, signInBtn, signUpBtn, forgotBtn, sendResetBtn, updatePasswordBtn, backBtn].forEach(btn => {
    if(btn) btn.disabled = !!cloud.authBusy;
  });
  [emailInput, passInput, resetEmailInput, newPasswordInput].forEach(inp => {
    if(inp) inp.disabled = !!cloud.authBusy;
  });
}
...
function openAuthModal(message=""){
  const modal = document.getElementById("authModal");
  if(!modal) return;
  if(cloud.authMode !== "recovery") setAuthMode(cloud.user ? "signin" : "signin");
  updateAuthModalUI(message);
...
function setAuthBusy(v, message=""){
  cloud.authBusy = !!v;
  updateAuthModalUI(message);
}

async function reloadProjectForCurrentScope(){
  const prevProject = state.project ? deepClone(state.project) : null;
  state.project = getCurrentProject();
  if(prevProject && state.project && prevProject.id === state.project.id){
    const bestNotes = pickBestNotesBackup([
      { notesDoc: state.project.notesDoc, notesColor: state.project.notesColor, updatedAt: Number(state.project.updatedAt) || 0 },
      { notesDoc: prevProject.notesDoc, notesColor: prevProject.notesColor, updatedAt: Number(prevProject.updatedAt) || 0 },
      readNotesBackup(state.project.id)
    ]);
    if(bestNotes){
      if(String(bestNotes.notesDoc || "").trim()) state.project.notesDoc = String(bestNotes.notesDoc || "");
      if(String(bestNotes.notesColor || "").trim()) state.project.notesColor = String(bestNotes.notesColor || state.project.notesColor || "");
    }
    writeNotesBackup(state.project);
  }
...
async function handleAuthSessionChange(session, reason=""){
  const seq = ++cloud.authChangeSeq;
  const prevScope = getStorageScope();

  cloud.session = session || null;
  cloud.user = session?.user || null;

  if(cloud.user){
    maybeSeedSignedInScopeFromGuest();
    cloud.ready = true;
    setCloudStatus("syncing");
    try{
      await mergeCloudIntoLocal();
      if(seq !== cloud.authChangeSeq) return;
      setCloudStatus("ok");
      queueCloudSync(reason || "auth");
      updateAuthModalUI();
    }catch(err){
      if(seq !== cloud.authChangeSeq) return;
      console.error("Cloud auth sync failed", err);
      setCloudStatus("error", err?.message || String(err || "Unknown error"));
      updateAuthModalUI(err?.message || "Cloud sync failed.");
    }
  }else{
    cloud.ready = false;
  ...
  if(cloud.appMounted && prevScope !== nextScope){
    await reloadProjectForCurrentScope();
  }
}

async function initCloudSync(){
  if(cloud.authInitialized) return;
  cloud.authInitialized = true;
  ...
  try{
    ...
    cloud.client.auth.onAuthStateChange((event, session) => {
      if(event === "PASSWORD_RECOVERY") { setAuthMode("recovery"); openAuthModal("Enter your new password below."); }
      handleAuthSessionChange(session, event).catch(err => {
    ...
    if(error) throw error;
    await handleAuthSessionChange(data?.session || null, "init");

    if(!cloud.onlineListenerWired){
      cloud.onlineListenerWired = true;
      window.addEventListener("online", () => {
        if(cloud.user){
          syncAllProjectsToCloud("online").catch(err => console.error(err));
        }
      });
    }
  }catch(err){
...
function scheduleDeferredCloudAudioSync(reason="deferred"){
  try{
    if(!cloud.client || !cloud.user || !getAuthUserId()) return;
    clearTimeout(cloud._audioDeferredTimer);
    cloud._audioDeferredTimer = setTimeout(() => {
      syncCloudAudioForAllProjects(reason).catch(err => {
        console.error("Deferred cloud audio sync failed", err);
      });
    }, 1200);
  }catch{}
}

function scheduleDeferredCloudNoteImageSync(reason="deferred"){
  try{
    if(!cloud.client || !cloud.user || !getAuthUserId()) return;
    clearTimeout(cloud._noteImageDeferredTimer);
    cloud._noteImageDeferredTimer = setTimeout(() => {
      syncCloudNoteImagesForAllProjects(reason).catch(err => {
        console.error("Deferred cloud note image sync failed", err);
      });
    }, 1200);
  }catch{}
}
...
async function submitAuthSignIn(){
  if(!cloud.client) await initCloudSync();
  ...
  if(!email || !password){
    updateAuthModalUI("Enter your email and password.");
    return;
  }
  try{
    ...
    if(error) throw error;
    updateAuthModalUI("Signed in.");
    closeAuthModal();
  }catch(err){
    updateAuthModalUI(err?.message || "Sign-in failed.");
  }finally{
    setAuthBusy(false);
  }
}
...
async function submitAuthSignUp(){
  ...
  if(password.length < 6){
    updateAuthModalUI("Use a password with at least 6 characters.");
    return;
  }
  try{
    ...
    if(error) throw error;
    if(data?.session){
      updateAuthModalUI("Account created and signed in.");
      closeAuthModal();
    }else{
      updateAuthModalUI("Account created. Check your email if Supabase asks for confirmation, then sign in.");
    }
  }catch(err){
    updateAuthModalUI(err?.message || "Sign-up failed.");
  }finally{
    setAuthBusy(false);
  }
}

async function submitAuthSendReset(){
  if(!cloud.client) await initCloudSync();
  const email = String(document.getElementById("authResetEmailInput")?.value || document.getElementById("authEmailInput")?.value || "").trim();
  if(!email){
    updateAuthModalUI("Enter your email first.");
    return;
  }
  try{
    ...
    if(error) throw error;
    updateAuthModalUI("Password reset email sent. Check your inbox.");
  }catch(err){
    updateAuthModalUI(err?.message || "Could not send reset email.");
  }finally{
    setAuthBusy(false);
  }
}

async function submitAuthUpdatePassword(){
  if(!cloud.client) await initCloudSync();
  const password = String(document.getElementById("authNewPasswordInput")?.value || "");
  if(password.length < 6){
    updateAuthModalUI("Use a password with at least 6 characters.");
    return;
  }
  try{
    ...
    setAuthMode("signin");
    updateAuthModalUI("Password updated. You can sign in now.");
  }catch(err){
    updateAuthModalUI(err?.message || "Could not update password.");
  }finally{
    setAuthBusy(false);
  }
}

async function submitAuthSignOut(){
  if(!cloud.client) return;
  try{
    ...
    updateAuthModalUI(err?.message || "Sign-out failed.");
  }finally{
    setAuthBusy(false);
  }
}
...
function wireAuthModal(){
  ...
  document.getElementById("authSignInBtn")?.addEventListener("click", submitAuthSignIn);
  document.getElementById("authSignUpBtn")?.addEventListener("click", submitAuthSignUp);
  document.getElementById("authForgotPasswordBtn")?.addEventListener("click", () => {
    setAuthMode("forgot");
    updateAuthModalUI();
  });
  document.getElementById("authBackToSignInBtn")?.addEventListener("click", () => {
    setAuthMode("signin");
    updateAuthModalUI();
  });
  document.getElementById("authSendResetBtn")?.addEventListener("click", submitAuthSendReset);
  document.getElementById("authUpdatePasswordBtn")?.addEventListener("click", submitAuthUpdatePassword);
  document.getElementById("authSignOutBtn")?.addEventListener("click", submitAuthSignOut);
  document.getElementById("authSyncNowBtn")?.addEventListener("click", async () => {
    try{
      updateAuthModalUI("Syncing now…");
      await syncAllProjectsToCloud("manual");
      updateAuthModalUI("Synced.");
    }catch(err){
      updateAuthModalUI(err?.message || "Sync failed.");
    }
  });

  ["authEmailInput", "authPasswordInput"].forEach(id => {
    document.getElementById(id)?.addEventListener("keydown", (e) => {
      if(e.key === "Enter") submitAuthSignIn();
    });
  });

  document.getElementById("authResetEmailInput")?.addEventListener("keydown", (e) => {
    if(e.key === "Enter") submitAuthSendReset();
  });
  document.getElementById("authNewPasswordInput")?.addEventListener("keydown", (e) => {
    if(e.key === "Enter") submitAuthUpdatePassword();
  });
...
function pruneLegacyTrailingBlankFullBlocks(fullText){
  const txt = normalizeLineBreaks(fullText || "");
  ...
  const legacyTitles = new Set([
    "INTRO","VERSE 1","CHORUS 1","VERSE 2","CHORUS 2","VERSE 3","BRIDGE","CHORUS 3","OUTRO"
  ]);

  const hasMeaningfulBody = (blk) => {
    const body = Array.isArray(blk?.body) ? blk.body : [];
    return body.some(line => String(line || "").trim() !== "");
  };

  let lastKeep = blocks.length - 1;
  while(lastKeep >= 0){
    const blk = blocks[lastKeep];
    const title = String(blk?.title || "").trim().toUpperCase().replace(/\s+/g, " ");
    if(!hasMeaningfulBody(blk) && legacyTitles.has(title)){
      lastKeep -= 1;
      continue;
    }
    break;
  ...
  const kept = blocks.slice(0, Math.max(1, lastKeep + 1));
  const out = [];
  for(const blk of kept){
    const title = String(blk.title || "").trim();
    out.push(BREAK_LINE + (title ? title : ""));
    out.push("");
    for(const ln of (blk.body || [])) out.push(String(ln ?? ""));
    out.push("");
    out.push("");
  }
...
function loadAllProjectsForScope(scope){
  try{
    const raw = localStorage.getItem(getProjectsStorageKey(scope));
...
function saveAllProjectsForScope(scope, projects){
  localStorage.setItem(getProjectsStorageKey(scope), JSON.stringify(projects));
}

function getCurrentProjectIdForScope(scope){
  try{ return localStorage.getItem(getCurrentProjectStorageKey(scope)) || ""; }catch{ return ""; }
}

function setCurrentProjectIdForScope(scope, id){
  try{ localStorage.setItem(getCurrentProjectStorageKey(scope), String(id || "")); }catch{}
}

function clearCurrentProjectIdForScope(scope){
  try{ localStorage.removeItem(getCurrentProjectStorageKey(scope)); }catch{}
}

function maybeSeedSignedInScopeFromGuest(){
  const userId = getAuthUserId();
  if(!userId) return;
  const userProjects = loadAllProjectsForScope(userId);
  if(Array.isArray(userProjects) && userProjects.length) return;
  const guestProjects = loadAllProjectsForScope("guest");
  if(!Array.isArray(guestProjects) || !guestProjects.length) return;
  saveAllProjectsForScope(userId, deepClone(guestProjects));
  const guestCur = getCurrentProjectIdForScope("guest");
  if(guestCur) setCurrentProjectIdForScope(userId, guestCur);
}


function loadAllProjects(){
  return loadAllProjectsForScope(getStorageScope());
}

function saveAllProjects(projects){
  saveAllProjectsForScope(getStorageScope(), projects);
  if(!cloud.suspendQueue) queueCloudSync("saveAllProjects");
}
...
function upsertProject(p){
  ...
  saveAllProjects(all);
  writeNotesBackup(p);
  setCurrentProjectIdForScope(getStorageScope(), p.id);
  if(!cloud.suspendQueue) queueCloudSync("upsertProject");
}
...
function deleteProjectById(id){
  ...
  saveAllProjects(all);
  deleteNotesBackup(id);
  clearCurrentProjectIdForScope(getStorageScope());
  if(!cloud.suspendQueue && getAuthUserId()){
    deleteProjectFromCloud(id).then(() => {
      setCloudStatus("ok");
    }).catch(err => {
...
function getProjectByIdLocal(projectId){
  const all = loadAllProjects().map(normalizeProject).filter(Boolean);
  return all.find(p => String(p.id || "") === String(projectId || "")) || null;
}

function getCloudAudioManifest(project){
  return Array.isArray(project?.cloudAudioManifest) ? project.cloudAudioManifest.map(x => ({ ...(x || {}) })) : [];
}

function setProjectCloudAudioManifest(project, manifest){
  if(!project || typeof project !== "object") return project;
  project.cloudAudioManifest = Array.isArray(manifest)
    ? manifest
        .filter(Boolean)
        .map(entry => ({
          id: String(entry.id || ""),
          projectId: String(entry.projectId || project.id || ""),
          title: String(entry.title || ""),
          kind: String(entry.kind || "upload"),
          createdAt: Number(entry.createdAt) || now(),
          updatedAt: Number(entry.updatedAt) || Number(entry.createdAt) || now(),
          offsetSec: Number(entry.offsetSec) || 0,
          path: String(entry.path || ""),
          contentType: String(entry.contentType || "audio/mpeg"),
...
function getCloudAudioPath(projectId, recId, blobOrType){
  const userId = String(getAuthUserId() || "").trim();
  const pid = String(projectId || "").trim();
  const rid = String(recId || "").trim();
  const type = String(blobOrType?.type || blobOrType || "").toLowerCase();
  let ext = ".mp3";
  if(type.includes("wav")) ext = ".wav";
  else if(type.includes("ogg")) ext = ".ogg";
  else if(type.includes("webm")) ext = ".webm";
  else if(type.includes("m4a") || type.includes("mp4")) ext = ".m4a";
  return `${userId}/${pid}/${rid}${ext}`;
}

function recordingToManifestEntry(rec){
  if(!rec || !rec.id) return null;
  const path = String(rec.cloudPath || getCloudAudioPath(rec.projectId, rec.id, rec.blob || rec.contentType || "audio/mpeg"));
  return {
    id: String(rec.id),
    projectId: String(rec.projectId || ""),
    title: String(rec.title || ""),
    kind: String(rec.kind || "upload"),
    createdAt: Number(rec.createdAt) || now(),
    updatedAt: Number(rec.updatedAt) || Number(rec.createdAt) || now(),
    offsetSec: Number(rec.offsetSec) || 0,
    path,
    contentType: String(rec.blob?.type || rec.contentType || "audio/mpeg"),
    size: Number(rec.blob?.size || rec.size || 0),
    deleted: false
  };
}

async function uploadRecordingToCloud(rec){
  if(!cloud.client || !cloud.user || !rec || !rec.id || !rec.blob) return null;
  const entry = recordingToManifestEntry(rec);
  ...
  const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).upload(path, rec.blob, options);
  if(error) throw error;
  rec.cloudPath = path;
  rec.contentType = entry.contentType;
  rec.size = entry.size;
  return { ...entry, path };
}

async function downloadRecordingFromCloud(entry){
  if(!cloud.client || !cloud.user || !entry?.path) return null;
  const { data, error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).download(entry.path);
  if(error) throw error;
  const blob = data instanceof Blob ? data : new Blob([data], { type: entry.contentType || "audio/mpeg" });
  return {
    id: String(entry.id),
    ...
    blob,
    offsetSec: Number(entry.offsetSec) || 0,
    cloudPath: String(entry.path || ""),
    contentType: String(entry.contentType || blob.type || "audio/mpeg"),
    size: Number(entry.size) || Number(blob.size) || 0
  };
}

async function removeRecordingFromCloud(entry){
  if(!cloud.client || !cloud.user || !entry?.path) return;
  try{
    const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).remove([entry.path]);
    if(error) throw error;
  }catch(err){
    console.warn("Cloud audio remove failed", err);
  }
}

async function saveProjectAudioManifest(project, manifest){
  if(!project || !project.id) return;
  const next = normalizeProject(deepClone(project));
  setProjectCloudAudioManifest(next, manifest);

  if(state.project && String(state.project.id || "") === String(next.id || "")) state.project = next;
...
async function tombstoneRecordingInProject(projectId, rec){
  if(!projectId || !rec) return;
  const project = getProjectByIdLocal(projectId) || (state.project && String(state.project.id||"")===String(projectId) ? normalizeProject(deepClone(state.project)) : null);
  if(!project) return;
  const manifest = getCloudAudioManifest(project);
  const path = String(rec.cloudPath || getCloudAudioPath(projectId, rec.id, rec.blob || rec.contentType || "audio/mpeg"));
  const tomb = {
    id: String(rec.id),
  ...
  const map = new Map(manifest.map(e => [String(e.id || ""), { ...(e || {}) }]));
  map.set(tomb.id, tomb);
  const nextManifest = Array.from(map.values());
  await saveProjectAudioManifest(project, nextManifest);
  triggerImmediateCloudSync("recording-delete");
}

async function syncCloudAudioForAllProjects(reason=""){
  if(!cloud.client || !cloud.user || !getAuthUserId()) return;

  const allProjects = loadAllProjects().map(normalizeProject).filter(Boolean);
  const allRecs = await dbGetAll().catch(() => []);
  const recMap = new Map((Array.isArray(allRecs) ? allRecs : []).map(r => [String(r.id || ""), r]));
  let anyManifestChanged = false;
  let currentProjectTouched = false;

  for(const project of allProjects){
    const projectId = String(project.id || "");
    if(!projectId) continue;

    const manifestMap = new Map(getCloudAudioManifest(project).map(e => [String(e.id || ""), { ...(e || {}) }]));
    const localProjectRecs = (Array.isArray(allRecs) ? allRecs : []).filter(r => String(r.projectId || "") === projectId);
    let manifestChanged = false;
    let projectTouched = false;

    for(const rec of localProjectRecs){
      if(!rec || !rec.id || !rec.blob) continue;
      const localUpdated = Number(rec.updatedAt) || Number(rec.createdAt) || now();
      const remoteEntry = manifestMap.get(String(rec.id));
      const remoteUpdated = Number(remoteEntry?.updatedAt) || 0;

      if(!remoteEntry || remoteEntry.deleted || localUpdated > remoteUpdated){
        const uploadedEntry = await uploadRecordingToCloud(rec);
        uploadedEntry.updatedAt = localUpdated;
        uploadedEntry.createdAt = Number(rec.createdAt) || uploadedEntry.createdAt;
        uploadedEntry.offsetSec = Number(rec.offsetSec) || 0;
        uploadedEntry.title = String(rec.title || "");
        manifestMap.set(String(rec.id), uploadedEntry);
        manifestChanged = true;
      }else if(remoteUpdated > localUpdated){
        const remoteRec = await downloadRecordingFromCloud(remoteEntry);
        await dbPut(remoteRec);
        recMap.set(String(remoteRec.id), remoteRec);
        projectTouched = true;
      }
    }

    for(const [recId, entry] of manifestMap.entries()){
      const localRec = recMap.get(String(recId));
      const localUpdated = Number(localRec?.updatedAt) || Number(localRec?.createdAt) || 0;
      const remoteUpdated = Number(entry?.updatedAt) || 0;

      if(entry?.deleted){
        if(localRec && remoteUpdated >= localUpdated){
          await dbDelete(recId);
          recMap.delete(recId);
          projectTouched = true;
        }
        await removeRecordingFromCloud(entry);
        continue;
      }

      if(!localRec){
        const remoteRec = await downloadRecordingFromCloud(entry);
        await dbPut(remoteRec);
        recMap.set(String(remoteRec.id), remoteRec);
        projectTouched = true;
      }
    ...
    if(JSON.stringify(getCloudAudioManifest(project)) !== JSON.stringify(nextManifest)){
      await saveProjectAudioManifest(project, nextManifest);
      manifestChanged = true;
    }

    if(manifestChanged) anyManifestChanged = true;
    if(projectTouched) currentProjectTouched = currentProjectTouched || (state.project && String(state.project.id||"") === projectId);
  }

  if(anyManifestChanged) queueCloudSync(`audio-manifest-${reason || "sync"}`);
  if(currentProjectTouched) await renderRecordings();
}

async function deleteRecordingWithSync(rec){
  if(!rec || !rec.id) return;
  const projectId = String(rec.projectId || "");
  if(cloud.ready && cloud.user && projectId){
    await tombstoneRecordingInProject(projectId, rec);
  }
  await dbDelete(rec.id);
  if(state.audioSyncRecId === rec.id) stopAudioSync();
}
...
function normalizeProject(p){
  ...
  p.fullText = pruneLegacyTrailingBlankFullBlocks(p.fullText);
  if(typeof p.notesDoc !== "string") p.notesDoc = "";
  if(typeof p.notesColor !== "string" || !p.notesColor) p.notesColor = "#151515";
  if(!Array.isArray(p.notesImages)) p.notesImages = [];
  p.notesImages = p.notesImages.filter(Boolean).map((entry) => ({
    id: String(entry.id || ""),
    projectId: String(entry.projectId || p.id || ""),
    title: String(entry.title || ""),
    createdAt: Number(entry.createdAt) || now(),
    updatedAt: Number(entry.updatedAt) || Number(entry.createdAt) || now(),
    path: String(entry.path || ""),
  ...
  const notesBackup = readNotesBackup(p.id);
  if(notesBackup){
    const bestNotes = pickBestNotesBackup([
      {
        notesDoc: String(p.notesDoc || ""),
        notesColor: String(p.notesColor || ""),
        updatedAt: Number(p.updatedAt) || 0
      },
      notesBackup
    ]);
    if(bestNotes){
      p.notesDoc = String(bestNotes.notesDoc || p.notesDoc || "");
      if(String(bestNotes.notesColor || "").trim()){
        p.notesColor = String(bestNotes.notesColor || p.notesColor || "#151515");
      }
    }
  }
...
function getCurrentProject(){
  ...
  const curId = getCurrentProjectIdForScope(getStorageScope());
  return (curId && active.find(p => p.id === curId)) || active[0];
}
...
const NOTE_IMAGE_STORE = "note_images";
...
function openDB(){
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if(!db.objectStoreNames.contains(NOTE_IMAGE_STORE)){
        db.createObjectStore(NOTE_IMAGE_STORE, { keyPath: "id" });
      }
    };
...
async function dbPut(rec){
  if(rec && typeof rec === "object"){
    if(!Number.isFinite(Number(rec.createdAt))) rec.createdAt = now();
    if(!Number.isFinite(Number(rec.updatedAt))) rec.updatedAt = Number(rec.createdAt) || now();
  }
  const db = await openDB();
  return new Promise((resolve, reject) => {
...
async function dbPutNoteImage(rec){
  if(rec && typeof rec === "object"){
    if(!Number.isFinite(Number(rec.createdAt))) rec.createdAt = now();
    if(!Number.isFinite(Number(rec.updatedAt))) rec.updatedAt = Number(rec.createdAt) || now();
  }
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(NOTE_IMAGE_STORE, "readwrite");
    tx.objectStore(NOTE_IMAGE_STORE).put(rec);
    tx.oncomplete = () => resolve(true);
    tx.onerror = () => reject(tx.error);
  });
}

async function dbDeleteNoteImage(id){
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(NOTE_IMAGE_STORE, "readwrite");
    tx.objectStore(NOTE_IMAGE_STORE).delete(id);
    tx.oncomplete = () => resolve(true);
    tx.onerror = () => reject(tx.error);
  });
}

async function dbGetNoteImage(id){
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(NOTE_IMAGE_STORE, "readonly");
    const req = tx.objectStore(NOTE_IMAGE_STORE).get(id);
    req.onsuccess = () => resolve(req.result || null);
    req.onerror = () => reject(req.error);
  });
}

async function dbGetAllNoteImages(){
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(NOTE_IMAGE_STORE, "readonly");
    const req = tx.objectStore(NOTE_IMAGE_STORE).getAll();
    req.onsuccess = () => resolve(req.result || []);
    req.onerror = () => reject(req.error);
  });
}
...
lastChordRaw: "",
  ...
  autoScrollPausedUiTick8: 0,
  autoScrollPausedAt: 0,
  playCardIndex: null,
...
function shouldTickRun(){
  if(state.autoScrollPaused) return false;
...
function uiTick8(){
  if(state.autoScrollPaused){
    return Number(state.autoScrollPausedUiTick8) || 0;
  }
...
function transposeMaybePositionalChordToken(token, semis){
  const raw = String(token || "");
  if(!raw || !semis) return raw;

  const m = raw.match(/^(\s*[\[\(\{]?)([1-8]?)([A-Ga-g][#b]?[^\s\]\)\}\|,;:]*)([\]\)\}]?\s*)$/);
  if(!m) return raw;
  ...
  if(!parseChordToken(chord)) return raw;
  return `${lead}${pos}${transposeChordName(chord, semis)}${trail}`;
}

function lineLooksMostlyLikeChordsGlobal(line){
  const raw = String(line || "").replace(/\u00A0/g, " ").trim();
  if(!raw) return false;
  ...
  for(const tok0 of toks){
    ...
    const posMatch = tok.match(/^([1-8])(.+)$/);
    const bare = posMatch ? String(posMatch[2] || "") : tok;

    if(parseChordToken(bare)) chordish++;
    else other++;
  }

  if(chordish === 0) return false;
  return (other === 0) || (chordish >= 2 && other <= 1) || (chordish >= 3 && (chordish / Math.max(1, chordish + other)) >= 0.65);
}

function transposeChordsInDisplayLine(line, semis){
  const raw = String(line || "");
  if(!raw || !semis) return raw;

  let out = raw.replace(/[\[\(]([^\]\)]+)[\]\)]/g, (m, inner) => {
    const innerText = String(inner || "").trim();
    if(!innerText) return m;

    const posMatch = innerText.match(/^([1-8])(.+)$/);
    const bare = posMatch ? String(posMatch[2] || "") : innerText;
    if(!parseChordToken(bare)) return m;

    const next = posMatch ? `${posMatch[1]}${transposeChordName(bare, semis)}` : transposeChordName(bare, semis);
    return `${m[0]}${next}${m[m.length - 1]}`;
  });

  if(!lineLooksMostlyLikeChordsGlobal(raw)) return out;

  return out.replace(/(^|\s)([^\s|]+)(?=\s|\||$)/g, (m, lead, tok) => {
    const t = transposeMaybePositionalChordToken(tok, semis);
    return `${lead}${t}`;
  });
}

function transposeFullSectionBodyForDisplay(text, semis){
  const s = String(text || "");
  if(!s || !semis) return s;
  return s.replace(/\r\n/g, "\n").replace(/\r/g, "\n").split("\n")
    .map(line => transposeChordsInDisplayLine(line, semis))
    .join("\n");
}
...
function buildPianoVoicing(ch){
  ...
  // Build the keyboard stack from explicit chord degrees instead of relying on
  // a generic sorted pitch-class pass. This makes the 3rd speak clearly so
  // major chords like D stay unmistakably major instead of sounding minor-ish.
  const has = (iv) => ints.includes(iv);
  const pick = (choices, fallback=null) => {
    for(const iv of choices){
      if(has(iv)) return iv;
    }
    return fallback;
  };

  const thirdLike = pick([4,3,5,2], 4);
  const fifthLike = pick([7,6,8], 7);
  const seventhLike = pick([11,10], null);
  const colorLike = pick([14,9], null);

  const bass = nearestMidiForPC(bassPC, 38);

  const voiceIntervals = [0, thirdLike, fifthLike];
  if(seventhLike !== null) voiceIntervals.push(seventhLike);
  if(colorLike !== null) voiceIntervals.push(colorLike);
  voiceIntervals.push(12); // top root

  // For major/minor/dim/aug chords, double the 3rd up top so the chord quality
  // is obvious even on small phone speakers.
  if(![5,2].includes(thirdLike)) voiceIntervals.push(thirdLike + 12);

  const mids = [];
  let target = 50;
  for(const iv of voiceIntervals){
    const pc = (root + iv) % 12;
...
/***********************
SYNTH KEYS NOTE
Stable-pitch keyboard synth for block chords.
***********************/
function synthKeysNote(ctx, freq, durMs, vel=0.88){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.10, durMs/1000);
  const f0 = clamp(freq, 45, 2600);
  const v = clamp(vel, 0.2, 1.0);
  ...
  env.gain.setValueAtTime(0.0001, t0);
  env.gain.exponentialRampToValueAtTime(peak, t0 + 0.006);
  env.gain.setValueAtTime(peak * 0.82, t0 + 0.040);
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.68, endTime);
  const tail = clamp(0.55 + dur * 0.45, 0.75, 3.00);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);
  ...
  const outTrim = ctx.createGain(); outTrim.gain.value = 0.58;

  o1.connect(g1); g1.connect(mix);
  o2.connect(g2); g2.connect(mix);
  o3.connect(g3); g3.connect(mix);
  mix.connect(hp); hp.connect(body); body.connect(presence); presence.connect(lp); lp.connect(env);
  env.connect(dry); dry.connect(outTrim);
  env.connect(room.in); room.wet.connect(wet); wet.connect(outTrim);

  o1.start(t0); o2.start(t0); o3.start(t0);
  const stopAt = endTime + tail + 0.12;
  o1.stop(stopAt); o2.stop(stopAt); o3.stop(stopAt);

  scheduleCleanup([o1,o2,o3,g1,g2,g3,mix,hp,body,presence,lp,env,dry,wet,outTrim, ...room.nodes], (durMs + tail*1000 + 1200));
  return { out: outTrim, nodes:[o1,o2,o3,g1,g2,g3,mix,hp,body,presence,lp,env,dry,wet,outTrim, ...room.nodes] };
}
...
function playSingleNoteForInstrument(rawChord, durMs){
  ...
  const singleMidi = nearestMidiForPC(singlePc, 57);
  const f = midiToFreq(singleMidi) * fracMul;

  if(state.instrument === "acoustic"){
    const n = grandPianoNote(ctx, f, durMs, 0.94);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 5200);
  }else if(state.instrument === "electric"){
    const n = pianoNote(ctx, f, durMs, 0.84);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 6400);
  }else{
    const n = synthKeysNote(ctx, f, durMs, 0.86);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 4200);
  }
}

function playKeyboardBlockChord(ch, durMs, noteMaker, opts={}){
  const ctx = ensureCtx();
  const room = makeSoftRoom(ctx);
  const dryBus = ctx.createGain();
  dryBus.gain.value = Number.isFinite(opts.dryGain) ? opts.dryGain : 0.34;
  const wet = ctx.createGain();
  wet.gain.value = Number.isFinite(opts.wetGain) ? opts.wetGain : 0.08;
  ...
  freqs.forEach((f, i) => {
    const centerIndex = (freqs.length - 1) / 2;
    const dist = Math.abs(i - centerIndex);
    const velBase = Number.isFinite(opts.baseVel) ? opts.baseVel : 0.84;
    const vel = clamp(velBase - dist * 0.045 + ((Math.random()*2-1)*0.012), 0.52, 0.98);
    const n = noteMaker(ctx, f, durMs, vel);
    n.out.connect(dryBus);
    scheduleCleanup([n.out], durMs + (Number(opts.cleanupMs) || 7000));
  });

  scheduleCleanup([dryBus, wet, ...room.nodes], durMs + (Number(opts.cleanupMs) || 7000) + 800);
}

function playAcousticChord(ch, durMs, fracMul=1){
  playKeyboardBlockChord(ch, durMs, (ctx,f,d,v)=>grandPianoNote(ctx, f * (fracMul || 1), d, v), {
    dryGain: 0.36,
    wetGain: 0.075,
    baseVel: 0.90,
    cleanupMs: 8200
...
function playElectricChord(ch, durMs, fracMul=1){
  playKeyboardBlockChord(ch, durMs, (ctx,f,d,v)=>pianoNote(ctx, f * (fracMul || 1), d, v), {
    dryGain: 0.30,
    wetGain: 0.060,
    baseVel: 0.82,
    cleanupMs: 9200
...
function playPianoChord(ch, durMs, fracMul=1){
  playKeyboardBlockChord(ch, durMs, (ctx,f,d,v)=>synthKeysNote(ctx, f * (fracMul || 1), d, v), {
    dryGain: 0.34,
    wetGain: 0.10,
    baseVel: 0.86,
    cleanupMs: 6200
...
function getTopVisibleCard(){
  const cards = getCards();
  if(cards.length === 0) return null;

  const sb = el.sheetBody;
  const sbRect = sb ? sb.getBoundingClientRect() : null;
  const stickyHeader = sb ? sb.querySelector('.sheetHeader') : null;
  const stickyH = stickyHeader ? stickyHeader.getBoundingClientRect().height : 0;
  const topLine = sbRect ? (sbRect.top + stickyH + 6) : getHeaderBottomY();
  const bottomLine = sbRect ? (sbRect.bottom - 6) : window.innerHeight;
  ...
  return best || getCardAtPlayLine() || getNearestVisibleCard() || cards[0];
}
...
applyTick();
  ...
  try{
    ...
    if(rec){
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      triggerImmediateCloudSync("recording-offset");
      renderRecordings();
    }
  }catch{}
}
...
function resetAutoScrollToFirstCardOfActivePage(){
  ...
  // ✅ Start on the TOP-MOST card that is currently visible in the card viewport.
  // If nothing visible can be resolved, fall back safely.
  const visibleCard = getTopVisibleCard() || getCardAtPlayLine() || getNearestVisibleCard() || cards[0];
  const idx = Math.max(0, cards.indexOf(visibleCard));
...
function stopAllMusic(){
  ...
  if(el.drumSelect) el.drumSelect.value = "off";
  if(el.mDrumSelect) el.mDrumSelect.value = "off";

  // ✅ reset instrument selectors
  if(el.instrumentSelect) el.instrumentSelect.value = "off";
  if(el.mInstrumentSelect) el.mInstrumentSelect.value = "off";
...
function renderNoteLenUI(){
  const mode = String(state.noteLenMode || "half");
  if(el.noteLenSelect) el.noteLenSelect.value = mode;
  if(el.mNoteLenSelect) el.mNoteLenSelect.value = mode;
}

function renderInstrumentUI(){
  const value = state.instrumentOn ? String(state.instrument || "piano") : "off";
  if(el.instrumentSelect) el.instrumentSelect.value = value;
  if(el.mInstrumentSelect) el.mInstrumentSelect.value = value;
  renderNoteLenUI();
}

function renderDrumUI(){
  const value = state.drumsOn ? String(state.drumStyle || "rap") : "off";
  if(el.drumSelect) el.drumSelect.value = value;
  if(el.mDrumSelect) el.mDrumSelect.value = value;
}

function updateAutoScrollButtons(){
  const isOn = !!state.autoScrollOn;
  const isPaused = !!state.autoScrollPaused;

  // Idle keeps the original SRP up/down AutoScroll icon.
  // Running becomes Pause, and Paused becomes Resume.
  const mainLabel = !isOn ? "⇅" : (isPaused ? "▶" : "❚❚");
  const mainTitle = !isOn
    ? "Start AutoScroll"
    : (isPaused ? "Resume AutoScroll" : "Pause AutoScroll");

  [el.autoPlayBtn, el.mScrollBtn].forEach(btn => {
    if(!btn) return;
    btn.textContent = mainLabel;
    btn.title = mainTitle;
    btn.setAttribute("aria-label", mainTitle);
    btn.classList.toggle("on", isOn);
  });

  [el.autoStopBtn, el.mAutoStopBtn].forEach(btn => {
    if(!btn) return;
    btn.style.display = isOn ? "inline-flex" : "none";
    btn.disabled = !isOn;
    btn.title = "Stop AutoScroll";
    btn.setAttribute("aria-label", "Stop AutoScroll");
  });
...
function pauseAutoScroll(){
  if(!state.autoScrollOn || state.autoScrollPaused) return;

  state.autoScrollPaused = true;
  state.autoScrollPausedUiTick8 = uiTick8();
  state.autoScrollPausedAt = performance.now();

  if(state.currentSection === "Full" && state.fullPerfMode){
    if(state.fullPerfClockStartMs){
      state.fullPerfPausedElapsedMs = Math.max(0, performance.now() - state.fullPerfClockStartMs);
    }
  }
...
function resumeAutoScroll(){
  ...
  const frozenUiTick8 = Number(state.autoScrollPausedUiTick8) || 0;

  if(state.currentSection === "Full" && state.fullPerfMode){
    const elapsed = Number(state.fullPerfPausedElapsedMs) || 0;
    state.fullPerfClockStartMs = performance.now() - elapsed;
    state.fullPerfPausedElapsedMs = 0;
  }else{
    state.autoScrollTickOffset = state.tick8 - frozenUiTick8;
    state.lastAutoBar = Math.floor(Math.max(0, frozenUiTick8) / 8);
  }

  state.autoScrollPaused = false;
  state.autoScrollPausedAt = 0;
  updateAutoScrollButtons();
...
function handleAutoScrollPrimaryAction(){
  if(!state.autoScrollOn){
    setAutoScroll(true);
    return;
  }
...
function setAutoScroll(on){
  ...
  state.autoScrollPaused = false;
  state.autoScrollPausedUiTick8 = 0;
  state.autoScrollPausedAt = 0;
...
function setRecordUI(){
  if(el.mRecordBtn){
    el.mRecordBtn.classList.add("iconCircle","recCircle");
    el.mRecordBtn.classList.remove("btn","rec");
    el.mRecordBtn.classList.toggle("recording", state.isRecording);
    el.mRecordBtn.textContent = "";
    el.mRecordBtn.style.width = "";
    el.mRecordBtn.style.minWidth = "";
    el.mRecordBtn.style.height = "";
    el.mRecordBtn.style.padding = "";
    el.mRecordBtn.style.fontSize = "0";
    el.mRecordBtn.style.flex = "0 0 auto";
    el.mRecordBtn.title = state.isRecording ? "Stop Recording" : "Record";
    el.mRecordBtn.setAttribute("aria-label", el.mRecordBtn.title);
  }
...
const chordRowClipboard = { notes: Array(8).fill("") };

function getLineNotes8(line){
  const src = (line && Array.isArray(line.notes)) ? line.notes : [];
  return Array.from({length:8}, (_,i)=> String(src[i] ?? "").trim());
}

function setLineNotes8(line, notes){
  if(!line) return;
  line.notes = Array.from({length:8}, (_,i)=> String((notes && notes[i]) ?? "").trim());
}

function copyChordRowFromLine(line){
  chordRowClipboard.notes = getLineNotes8(line);
}

function hasChordClipboard(){
  return getLineNotes8({notes: chordRowClipboard.notes}).some(Boolean);
}

function pasteChordRowToLine(line){
  if(!line || !hasChordClipboard()) return false;
  setLineNotes8(line, chordRowClipboard.notes);
  return true;
}

function repeatPreviousChordRow(arr, idx, line){
  if(!Array.isArray(arr) || !line) return false;
  const prev = arr[idx - 1];
  if(!prev) return false;
  const prevNotes = getLineNotes8(prev);
  if(!prevNotes.some(Boolean)) return false;
  setLineNotes8(line, prevNotes);
  return true;
}

function parseInlineChordMapFromLyricsText(text){
  const raw = String(text || "");
  if(!raw.trim()) return null;

  const CHORD_TOKEN_RE = /^([A-G](?:#|b)?)(?:(?:maj|min|m|dim|aug|\+|sus|add)?(?:2|4|5|6|7|9|11|13)?(?:maj7|M7|m7|m9|m11|m13|sus2|sus4|add9|add11|add13|dim7|hdim7|m7b5)?(?:b5|#5|b9|#9|b11|#11|b13|#13)?)?(?:\/[A-G](?:#|b)?)?$|^N\.?C\.?$/i;
  const normalizeTok = (tok) => String(tok || "")
    .trim()
    .replace(/^[\[\(\{]+/, "")
    .replace(/[\]\)\}]+$/, "")
    .replace(/♯/g, "#")
    ...
    .replace(/[\.,;:]+$/g, "")
    .replace(/\s+/g, "");
  const isChordLike = (tok) => CHORD_TOKEN_RE.test(normalizeTok(tok));
  const parsePositional = (tok) => {
    const s = normalizeTok(tok);
    const m = s.match(/^([1-8])(.+)$/);
    if(!m) return null;
    const chord = normalizeTok(m[2] || "");
    if(!isChordLike(chord)) return null;
    return { slot: clamp(parseInt(m[1], 10) - 1, 0, 7), chord };
  };
  ...
  cleaned = cleaned.replace(/[\[\(\{]\s*([^\]\)\}]+?)\s*[\]\)\}]/g, (m, inner) => {
    const tok = normalizeTok(inner);
    if(!tok) return " ";
    if(isChordLike(tok)){
      found = true;
      const firstEmpty = slots.findIndex(v => !v);
      if(firstEmpty >= 0) slots[firstEmpty] = tok;
    }
    return " ";
  });

  cleaned = cleaned.replace(/(^|\s)([1-8])([A-Ga-g](?:#|b)?[^\s]*)/g, (m, lead, pos, body) => {
    const parsed = parsePositional(String(pos || "") + String(body || ""));
    if(parsed){
      found = true;
      slots[parsed.slot] = parsed.chord;
      return lead || " ";
    }
    return m;
  });
  ...
  return found ? { cleaned, slots } : null;
}

function applyChordRowUiRefresh(card, line, noteInputs){
  try{
    const inputs = Array.isArray(noteInputs) && noteInputs.length
      ? noteInputs
      : Array.from((card || document).querySelectorAll?.('.noteCell') || []);
    const notes = getLineNotes8(line);
    for(let i=0;i<8;i++){
      const inp = inputs[i];
      if(!inp) continue;
      inp.dataset.raw = notes[i] || '';
      inp.value = (state.capo ? transposeChordName(notes[i] || '', state.capo) : (notes[i] || ''));
    }
  }catch{}
  updateKeyFromAllNotes();
  updateFullIfVisible();
...
function getExportFullTextSource(){
  // ✅ First choice: export from the currently rendered Full Song editor blocks.
  // This preserves the exact in-app Full view chord placement the user sees.
  try{
    const editor = document.querySelector('.fullSectionEditor');
    if(editor){
      const cards = Array.from(editor.querySelectorAll('.fullSectionCard'));
      if(cards.length){
        const semis = getTransposeSemis() || 0;
        const out = [];
        cards.forEach(card => {
          const pill = card.querySelector('input.songPartPill');
          const ta = card.querySelector('textarea.fullSectionText');
          const ttl = String(pill?.value || '').trim();
          let bodyText = String(ta?.value || '');

          // Textareas show display-transposed text. Convert back to raw song text here,
          // then the export path can apply transpose exactly once in a consistent place.
          if(semis){
            bodyText = transposeFullSectionBodyForDisplay(bodyText, -semis);
          }

          bodyText = normalizeLineBreaks(bodyText);
          out.push(BREAK_LINE + (ttl ? ttl : ''));
          out.push('');
          if(bodyText){
            out.push(...bodyText.split("\n"));
          }
          out.push('');
          out.push('');
        });
        while(out.length && out[out.length - 1] === '') out.pop();
        const domText = out.join("\n") + "\n";
        if(domText.trim()) return domText;
      }
    }
  }catch{}

  let txt = String(state?.project?.fullText || "");
  txt = normalizeLineBreaks(txt);
  if(txt.trim()) return txt;

  // Fallback only when fullText is empty.
  const out = [];
  getVisibleSectionPages().filter(s => s !== "Full").forEach(sec => {
    ...
    const arr = Array.isArray(state.project.sections?.[sec]) ? state.project.sections[sec] : [];
    for(const line of arr){
      const lyrRaw = String(line?.lyrics || "");
      const notesRaw = Array.isArray(line?.notes) ? line.notes.map(n => String(n || "").trim()) : [];
      const hasNotes = notesRaw.some(Boolean);
      const hasLyrics = !!lyrRaw.trim();
      if(!hasNotes && !hasLyrics) continue;

      if(hasLyrics){
        const built = buildChordSheetLines(notesRaw, lyrRaw);
        if(String(built.chordLine || "").trim()) out.push(built.chordLine);
        out.push(String(built.lyricLine || "").trimRight());
      }else if(hasNotes){
        out.push(notesRaw.filter(Boolean).join(" "));
      }
      out.push("");
    }
    out.push("");
...
function transposeFullExportText(text, semis){
  const s = normalizeLineBreaks(text || "");
  if(!s.trim() || !semis) return s;
  return s.split("\n").map(line => {
    const br = _parseBreakLine(line);
    if(br) return line;
    return transposeChordsInDisplayLine(line, semis);
  }).join("\n");
}

function exportTextToHtmlBlocks(fullText){
  const lines = normalizeLineBreaks(fullText || "").split("\n");
  const blocks = [];
  let inPre = false;
  let preLines = [];
  ...
  function flushPre(){
    if(!inPre) return;
    blocks.push(
      `<pre style="margin:0 0 10px 0;font-family:'Courier New',Courier,monospace;font-weight:400;color:#111;white-space:pre;line-height:1.35;">${escapeHtml(preLines.join("\n"))}</pre>`
    );
    inPre = false;
    preLines = [];
  }
  ...
  for(const raw of lines){
    ...
    if(br){
      flushPre();
      const headingText = String(br.title || "").trim();
      if(headingText){
        blocks.push(`<div style="font-family:Arial,Helvetica,sans-serif;font-weight:900;margin:20px 0 8px;letter-spacing:.3px;">${escapeHtml(headingText)}</div>`);
      }
      continue;
    }
...
function buildFullPreviewText(){
  const semis = getTransposeSemis() || 0;
  const src = getExportFullTextSource();
  const body = transposeFullExportText(src, semis).trim();
  return body || "(No lyrics/notes yet - start typing in a section)";
}
...
function buildFullPreviewHtmlDoc(title){
  ...
  const semis = getTransposeSemis() || 0;
  const src = getExportFullTextSource();
  const exportText = transposeFullExportText(src, semis);
  const lineBlocks = exportTextToHtmlBlocks(exportText);
  ...
  if(lineBlocks.length) blocks.push(...lineBlocks);
  else blocks.push(`<div style="font-family:Arial,Helvetica,sans-serif;">(No lyrics/notes yet - start typing in a section)</div>`);
  ...
  try{
    ...
    getVisibleSectionPages().filter(s => s !== "Full").forEach(sec => {
      ...
      for(const line of arr){
        ...
        if(!lyr && hasNotes){
          const notesLine = notesRaw.filter(Boolean).join(" ").trim();
          if(notesLine){
            wroteAny = true;
            out.push(notesLine);
        ...
        const notesLine = notesRaw.map(n => String(n || "").trim());

        const { chordLine, lyricLine } = buildChordSheetLines(notesLine, lyrRaw);
...
function renderSheetTitleBar(){
  ...
  const sheetHeader = el.sheetTitle.closest?.(".sheetHeader") || null;
  if(sheetHeader){
    sheetHeader.style.overflow = "visible";
    sheetHeader.style.alignItems = "stretch";
    sheetHeader.style.paddingTop = "12px";
    sheetHeader.style.paddingBottom = "12px";
    sheetHeader.style.minHeight = (state.currentSection === "Full") ? "46px" : "58px";
  }
  ...
  el.sheetTitle.style.alignSelf = "stretch";
  el.sheetTitle.style.justifyContent = "space-between";
  el.sheetTitle.style.flexWrap = "nowrap";
  el.sheetTitle.style.gap = "10px";
  el.sheetTitle.style.minHeight = (state.currentSection === "Full") ? "22px" : "34px";
  el.sheetTitle.style.lineHeight = "normal";
  el.sheetTitle.style.overflow = "visible";
  ...
  if(state.currentSection !== "Full"){
    ...
    titleInput.style.lineHeight = "34px";
...
if(state.currentSection === "Full"){
  ...
  let blocks = splitBlocks(pruneLegacyTrailingBlankFullBlocks(state.project.fullText || ""));
  if(String(state.project.fullText || "") !== blocksToFullText(blocks)){
  ...
  blocks.forEach((b, idx) => {
    ...
    const displaySemis = getTransposeSemis() || 0;
    ta.value = transposeFullSectionBodyForDisplay(
      Array.isArray(b.body) ? b.body.join("\n") : "",
      displaySemis
    );
    autosizeTextarea(ta);
    ta.addEventListener("focus", () => {
      lastLyricsTextarea = ta;
      refreshRhymesFromActive();
    });
    ...
    ta.addEventListener("input", () => {
      ...
      const snap = {
        ...
        top: ta.getBoundingClientRect().top,
        bottom: ta.getBoundingClientRect().bottom,
      };
      autosizeTextarea(ta);
      requestAnimationFrame(() => keepFullTextareaSteady(ta, snap));
      const displaySemis = getTransposeSemis() || 0;
      const rawBodyText = transposeFullSectionBodyForDisplay(
        String(ta.value || ""),
        -displaySemis
      );
      b.body = rawBodyText.replace(/\r\n/g,"\n").replace(/\r/g,"\n").split("\n");
    ...
    bindTap(addBtn, () => {
      withPillActionLock(() => {
        ...
        blocks.splice(newIndex, 0, { title:"", body:[] });
        pendingFullBlockScroll = { index: newIndex, focusTextarea: true, forceAlignTop: false };
        state.project.fullText = blocksToFullText(blocks);
        quickSaveFullText();
        renderSheet();
        commitDebounced();
  ...
  requestAnimationFrame(() => {
    ...
    try{
      if(pendingFullBlockScroll && Number.isFinite(Number(pendingFullBlockScroll.index))){
        const cards = Array.from(fullWrap.querySelectorAll('.fullSectionCard'));
        const targetCard = cards[clamp(Number(pendingFullBlockScroll.index) || 0, 0, Math.max(0, cards.length - 1))];
        if(targetCard){
          scrollFullSectionCardIntoView(targetCard, pendingFullBlockScroll || {});
        }
      }
    }catch(err){ console.error(err); }
    pendingFullBlockScroll = null;
  });
  return;
...
function makeCardUtilBtn(label, title, onClick){
  const btn = document.createElement("button");
  btn.className = "cardUtilBtn";
  btn.type = "button";
...
const copyBtn = makeCardUtilBtn("⧉", "Copy chord row", () => {
  copyChordRowFromLine(line);
});

const pasteBtn = makeCardUtilBtn("📋", "Paste chord row", () => {
  if(!hasChordClipboard()) return;
  editProject("pasteChordRow", () => {
    pasteChordRowToLine(line);
    syncFullTextFromSections();
  });
  applyChordRowUiRefresh(card, line, noteInputs);
});

const repeatBtn = makeCardUtilBtn("↻", "Repeat previous card chords", () => {
  if(idx <= 0) return;
  editProject("repeatChordRow", () => {
    repeatPreviousChordRow(arr, idx, line);
    syncFullTextFromSections();
  });
  applyChordRowUiRefresh(card, line, noteInputs);
});
...
topRight.appendChild(pasteBtn);
topRight.appendChild(repeatBtn);
...
card.appendChild(top);
    ...
    for(let i=0;i<8;i++){
      ...
      inp.addEventListener("pointerdown", (e) => {
        ...
        if(chordDragCopy.active){
          e.preventDefault();
          commitChordDragCopyToLine(line, card);
          return;
        }

        if((e.pointerType === "mouse" && e.button !== 0) || inp.readOnly || inp.disabled) return;

        // ✅ Tap should open the keyboard normally.
        // ✅ Hold should switch into chord drag/copy mode and collapse the keyboard.
        inp.dataset.pendingTapFocus = "1";
        scheduleChordDragCopyHold(inp, line, card, e);
      });

      inp.addEventListener("pointermove", (e) => {
        maybeCancelChordCopyHoldFromMove(e);
      });

      inp.addEventListener("pointerup", () => {
        clearChordCopyTimer();
        delete inp.dataset.pendingTapFocus;
      });
      ...
      inp.addEventListener("input", () => {
        ...
        const multiTokens = rawNow.split(/[\s,|]+/).map(s => String(s || "").trim()).filter(Boolean);
        if(multiTokens.length > 1){
          editProject("pasteChordTokens", () => {
            for(let j=0; j<multiTokens.length && (i + j) < 8; j++){
              line.notes[i + j] = multiTokens[j];
            }
            syncFullTextFromSections();
          });
          applyChordRowUiRefresh(card, line, noteInputs);
  ...
  let didInlineChordMap = false;
  editProject("lyrics", () => {
    // 1) set lyrics
    line.lyrics = lyr.value;

    const inlineMap = parseInlineChordMapFromLyricsText(line.lyrics);
    if(inlineMap){
      didInlineChordMap = true;
      line.lyrics = inlineMap.cleaned;
      setLineNotes8(line, inlineMap.slots);
      lyr.value = line.lyrics;
    }
...
async function uploadAudioFile(){
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      ...
      await dbPut(item);
      triggerImmediateCloudSync("recording-upload");
...
async function renderRecordings(){
  ...
  mine.forEach(rec => {
    ...
    edit.addEventListener("click", async () => {
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      triggerImmediateCloudSync("recording-rename");
      renderRecordings();
    });
...
stop.addEventListener("click", () => {
  ...
  setTimeout(() => URL.revokeObjectURL(url), 800);
    ...
    del.addEventListener("click", async () => {
      ...
      await deleteRecordingWithSync(rec);
      renderRecordings();
    });
...
await dbPut(item);
triggerImmediateCloudSync("recording-new");
await renderRecordings();
...
Projects dropdown
***********************/
function compareProjectsBySort(a, b, sort){
  if(sort === "az") return (a.name||"").localeCompare(b.name||"");
  return (Number(b.updatedAt)||0) - (Number(a.updatedAt)||0);
}

function getVisibleProjects(){
  return loadAllProjects().map(normalizeProject).filter(p => p && !p.archived);
}
...
function renderArchiveLoadedName(){
  if(!el.archiveLoadedName) return;
  const project = state.project && normalizeProject(deepClone(state.project));
  const show = !!(project && project.archived);
  if(!show){
    el.archiveLoadedName.textContent = "";
    el.archiveLoadedName.style.display = "none";
    return;
  }
  el.archiveLoadedName.textContent = project.name || "Untitled";
  el.archiveLoadedName.style.display = "inline-flex";
  el.archiveLoadedName.title = project.name || "Untitled";
}

function renderProjectsDropdown(){
  const sort = getSavedProjectSort();
  if(el.sortSelect && el.sortSelect.value !== sort) el.sortSelect.value = sort;
  if(el.projectSortSelect && el.projectSortSelect.value !== sort) el.projectSortSelect.value = sort;

  const list = getVisibleProjects().sort((a,b) => compareProjectsBySort(a,b,sort));

  if(el.projectSelect){
    el.projectSelect.innerHTML = "";
    if(state.project && state.project.archived){
      ...
      list.forEach(p => {
        ...
        if(state.project && !state.project.archived && p.id === state.project.id) opt.selected = true;
  ...
  if(el.projectPickerLabel){
    const label = (state.project && !state.project.archived) ? (state.project.name || "Untitled") : "Projects";
    el.projectPickerLabel.textContent = label;
    el.projectPickerLabel.title = label;
  }

  if(el.projectSelect && state.project && !state.project.archived){
    el.projectSelect.value = state.project.id || "";
  }

  renderArchiveLoadedName();
  renderProjectModal();
}

function openProjectModal(){
  if(el.projectModal) el.projectModal.classList.add("show");
  if(el.projectSortSelect) el.projectSortSelect.value = getSavedProjectSort();
  renderProjectModal();
}

function closeProjectModal(){
  if(el.projectModal) el.projectModal.classList.remove("show");
}

function renderProjectModal(){
  if(!el.projectList) return;
  const sort = (el.projectSortSelect && el.projectSortSelect.value) ? el.projectSortSelect.value : getSavedProjectSort();
  if(el.projectSortSelect && el.projectSortSelect.value !== sort) el.projectSortSelect.value = sort;
  if(el.sortSelect && el.sortSelect.value !== sort) el.sortSelect.value = sort;
  ...
  list.forEach(project => {
    ...
    row.innerHTML = `<span class="archiveRowName">${escapeHtml(project.name || "Untitled")}</span><span class="archiveRowMeta">${sort === "recent" ? "Recent" : "A–Z"}</span>`;
...
function clearArchiveLongPress(){
  if(archiveUi.longPressTimer){
    clearTimeout(archiveUi.longPressTimer);
    archiveUi.longPressTimer = null;
  }
  archiveUi.longPressProjectId = "";
}

function closeArchiveMoveModal(){
  archiveUi.moveProjectId = "";
  if(el.archiveMoveModal) el.archiveMoveModal.classList.remove("show");
}

function openArchiveMoveModal(projectId){
  const project = loadAllProjects().map(normalizeProject).filter(Boolean).find(p => p.id === projectId);
  if(!project || !project.archived) return;
  archiveUi.moveProjectId = project.id;
  if(el.archiveMoveText) el.archiveMoveText.textContent = `Move "${project.name || "Untitled"}" to Projects?`;
  if(el.archiveMoveModal) el.archiveMoveModal.classList.add("show");
}

function setArchiveAddPanel(open){
  archiveUi.addOpen = !!open;
  if(el.archiveAddPanel) el.archiveAddPanel.classList.toggle("show", archiveUi.addOpen);
  if(archiveUi.addOpen) renderArchiveAddList();
}

function openArchiveModal(){
  if(el.archiveModal) el.archiveModal.classList.add("show");
  if(el.archiveSortSelect) el.archiveSortSelect.value = getSavedArchiveSort();
  renderArchiveModal();
}

function closeArchiveModal(){
  if(el.archiveModal) el.archiveModal.classList.remove("show");
  setArchiveAddPanel(false);
  closeArchiveMoveModal();
  clearArchiveLongPress();
}

function moveProjectToArchive(projectId){
  const all = loadAllProjects().map(normalizeProject).filter(Boolean);
...
function moveProjectToProjects(projectId){
  const all = loadAllProjects().map(normalizeProject).filter(Boolean);
  const project = all.find(p => p.id === projectId);
  if(!project || !project.archived) return;
...
function renderArchiveAddList(){
  if(!el.archiveAddList) return;
  const sort = (el.archiveSortSelect && el.archiveSortSelect.value) ? el.archiveSortSelect.value : getSavedArchiveSort();
  const active = getVisibleProjects().sort((a,b) => compareProjectsBySort(a,b,sort));
  el.archiveAddList.innerHTML = "";

  if(!active.length){
    const empty = document.createElement("div");
    empty.className = "archiveEmpty";
    empty.textContent = "No projects available to archive.";
    el.archiveAddList.appendChild(empty);
    return;
  }
  ...
  active.forEach(project => {
    const row = document.createElement("button");
    row.type = "button";
    row.className = "archiveRow archiveAddRow";
    row.innerHTML = `<span class="archiveRowName">${escapeHtml(project.name || "Untitled")}</span><span class="archiveRowAction">Add</span>`;
    row.addEventListener("click", () => moveProjectToArchive(project.id));
    el.archiveAddList.appendChild(row);
  });
}
...
function renderArchiveModal(){
  ...
  const sort = (el.archiveSortSelect && el.archiveSortSelect.value) ? el.archiveSortSelect.value : getSavedArchiveSort();
  if(el.archiveSortSelect && el.archiveSortSelect.value !== sort) el.archiveSortSelect.value = sort;

  const archived = getArchivedProjects().sort((a,b) => compareProjectsBySort(a,b,sort));
  el.archiveList.innerHTML = "";

  if(!archived.length){
    const empty = document.createElement("div");
    empty.className = "archiveEmpty";
    empty.textContent = "No completed projects archived yet.";
    ...
    archived.forEach(project => {
      ...
      const startHold = (ev) => {
        clearArchiveLongPress();
        archiveUi.longPressProjectId = project.id;
        archiveUi.longPressTimer = setTimeout(() => {
          archiveUi.longPressTimer = null;
          openArchiveMoveModal(project.id);
        }, 520);
      };
      row.addEventListener("pointerdown", startHold);
      row.addEventListener("pointerup", clearArchiveLongPress);
      row.addEventListener("pointerleave", clearArchiveLongPress);
      row.addEventListener("pointercancel", clearArchiveLongPress);
      row.addEventListener("pointermove", clearArchiveLongPress);
...
function loadProjectById(id){
   persistActiveProjectState({ saveNotes:true });
   closeNotesModal?.();
...
function getCaretCharOffsetFromRange(ed, range){
  try{
...
function getCaretCharOffsetInEditor(ed){
  try{
    const fullLen = getNotesEditorText(ed).length;

    const sel = window.getSelection?.();
    if(sel && sel.rangeCount){
      const liveRange = sel.getRangeAt(0);
      const livePos = getCaretCharOffsetFromRange(ed, liveRange);
      if(typeof livePos === "number") return livePos;
    }

    if(state?.notesLastRange){
      const cachedPos = getCaretCharOffsetFromRange(ed, state.notesLastRange);
      if(typeof cachedPos === "number") return cachedPos;
    }
...
const LYRIC_FRIENDLY_WORDS = new Set([
  "again","air","alive","alone","always","angel","back","band","believe","best","blue","body","broken",
  "burn","call","can","change","close","come","cry","dark","day","desire","dream","drive","dust","end","eyes","face",
  "faith","fall","far","feel","fire","flame","fly","forever","free","friend","game","glory","go","gold","gone",
  "good","grace","ground","hand","hard","heart","heaven","high","higher","hold","home","hope","hurt","inside",
  "kiss","know","land","late","leave","life","light","lights","live","lonely","long","love","lover","low",
  "man","mercy","mind","moon","more","morning","move","name","near","need","never","night","now","pain","peace",
  "plan","pray","praise","rain","ran","real","red","rest","rise","road","run","save","say","sand","shadow",
  "shine","sky","slow","small","snow","somebody","song","soul","sound","space","spirit","stand","stay","still",
  "stone","storm","strong","sun","sweet","take","tell","than","thing","time","tonight","touch","true","understand",
  "turn","wait","wake","walk","want","warm","water","way","when","whole","wild","wind","wish","world","worth",
  "you","your"
]);
const COMMON_SONGWORDS = new Set([
  "again","band","can","fan","friend","hand","land","man","plan","ran","sand","stand","than","understand","van","when"
]);
const OBSCURE_RHYME_PATTERNS = [
  /phobia$/i, /logy$/i, /graphy$/i, /nomy$/i, /esque$/i, /oid$/i,
  /aceous$/i, /arium$/i, /orium$/i, /ette$/i, /ling$/i, /pan$/i,
  /fan$/i, /plan$/i
];
const OBSCURE_RHYME_WORDS = new Set([
  "abye","anigh","briar","brier","clime","deadpan","direful","fen","glade","japan","lisle","mire","nigh","oer","panne","pyre","quire","sere","shan","thee","thine","thy","trepan","turbofan","twyer","ware","whilom","yon","yore"
]);
const DISCOURAGED_RHYME_WORDS = new Set([
  "began","deadpan","japan","lifespan","panne","shan","trepan","turbofan","underplan","overplan","counterplan"
]);

function normalizeDatamuseWord(raw){
  return String(raw || "")
    .trim()
    .toLowerCase()
...
function parseDatamuseFrequency(row){
  const tags = Array.isArray(row && row.tags) ? row.tags : [];
  for(const tag of tags){
    const m = /^f:([0-9]+(?:\.[0-9]+)?)$/i.exec(String(tag || "").trim());
    if(m) return Number(m[1]) || 0;
  }
  return 0;
}

function buildDatamuseWordEntry(row, kind){
  const word = normalizeDatamuseWord(row && row.word);
  if(!word) return null;
  return {
    word,
    kind,
    score: Number(row && row.score) || 0,
    numSyllables: Number(row && row.numSyllables) || 0,
    tags: Array.isArray(row && row.tags) ? row.tags.slice() : [],
    freq: parseDatamuseFrequency(row)
  };
}

async function fetchDatamuseRhymes(word, max = 64){
  const w = normalizeWord(word);
  if(!w) return [];
  const url = `https://api.datamuse.com/words?rel_rhy=${encodeURIComponent(w)}&md=fs&max=${max}`;
  try{
    const res = await fetch(url, { cache: "no-store" });
    if(!res.ok) return [];
    const data = await res.json();
    return (data || []).map(x => buildDatamuseWordEntry(x, "perfect")).filter(Boolean).slice(0, max);
  }catch{
    return [];
  }
}

async function fetchDatamuseNearRhymes(word, max = 64){
  const w = normalizeWord(word);
  if(!w) return [];
  const url = `https://api.datamuse.com/words?rel_nry=${encodeURIComponent(w)}&md=fs&max=${max}`;
  try{
    const res = await fetch(url, { cache: "no-store" });
    if(!res.ok) return [];
    const data = await res.json();
    return (data || []).map(x => buildDatamuseWordEntry(x, "near")).filter(Boolean).slice(0, max);
  }catch{
    return [];
  }
}

function scoreLyricFriendliness(candidate, seedWord){
  const word = normalizeDatamuseWord(candidate && candidate.word);
  const seed = normalizeWord(seedWord);
  if(!word || !seed || word === seed) return -999;

  let score = 0;
  const len = word.length;
  const syll = Number(candidate && candidate.numSyllables) || 0;
  const rawScore = Number(candidate && candidate.score) || 0;
  const freq = Number(candidate && candidate.freq) || 0;
  const seedSyll = estimateSyllables(seed);

  if(candidate && candidate.kind === "perfect") score += 220;
  else if(candidate && candidate.kind === "near") score += 120;

  // Keep Datamuse relevance, but don't let dictionary rarity dominate lyric usefulness.
  score += Math.min(24, rawScore / 55);

  // Word frequency matters a lot for lyric usefulness.
  if(freq >= 20) score += 150;
  else if(freq >= 10) score += 115;
  else if(freq >= 5) score += 80;
  else if(freq >= 2) score += 45;
  else if(freq > 0) score += 12;
  else score -= 95;

  if(LYRIC_FRIENDLY_WORDS.has(word)) score += 120;
  if(COMMON_SONGWORDS.has(word)) score += 180;
  if(/^(love|heart|night|light|fire|rain|home|alone|time|grace|faith|higher|desire|pain|dream|stay|gone|strong|song|soul|man|can|ran|hand|land|plan|stand|band|sand|than|fan|van)$/i.test(word)) score += 90;

  if(len >= 3 && len <= 6) score += 42;
  else if(len <= 8) score += 18;
  else if(len <= 10) score += 6;
  else score -= 28;

  if(syll === 1) score += 38;
  else if(syll === 2) score += 28;
  else if(syll === 3) score += 8;
  else if(syll >= 4) score -= 18;

  if(Math.abs(syll - seedSyll) <= 1) score += 22;
  if(/^[a-z]+(?:'[a-z]+)?$/i.test(word)) score += 14;
  if(/^(?:[bcdfghjklmnpqrstvwxyz]{1,2})?[aeiou][a-z]{1,5}$/i.test(word)) score += 10;
  if(/ing$|ed$|er$|in'$|in$/i.test(word)) score += 6;
  if(/^(?:un|re|pre|post|anti|auto|counter|hyper|micro|multi|neuro|over|psycho|retro|super|tele|trans|turbo|ultra|under)/i.test(word) && !COMMON_SONGWORDS.has(word)) score -= 65;

  // Strongly favor familiar, everyday one-word lyric rhymes.
  if(syll === 1 && len >= 3 && len <= 5) score += 34;
  if(syll >= 3 && !LYRIC_FRIENDLY_WORDS.has(word) && !COMMON_SONGWORDS.has(word)) score -= 42;

  if(/^[A-Z]/.test(String(candidate && candidate.word || ""))) score -= 35;
  if(/[0-9]/.test(word)) score -= 120;
  if(/[\s_-]/.test(word)) score -= 40;
  if(word.length <= 2) score -= 60;
  if(/[^a-z']/i.test(word)) score -= 90;
  if(OBSCURE_RHYME_WORDS.has(word)) score -= 240;
  if(DISCOURAGED_RHYME_WORDS.has(word)) score -= 340;
  if(OBSCURE_RHYME_PATTERNS.some(rx => rx.test(word)) && !COMMON_SONGWORDS.has(word)) score -= 95;
  if(/(?:tion|sion|ment|tude|ness|less|ship|gram|graph|scope|crat|zyme|tron)$/i.test(word)) score -= 26;
  if(/^[qxz]/i.test(word) && !LYRIC_FRIENDLY_WORDS.has(word)) score -= 18;

  // Hard demotion for words that feel technical/academic or overly dictionary-ish.
  if(/(?:electro|gram|graph|metric|nomic|ology|scope|tron|zyme)$/i.test(word) && !COMMON_SONGWORDS.has(word)) score -= 55;

  // Special boost for the most singable/common rhyme family members for short seed words like "man".
  if(seed.length <= 4 && /^(can|ran|hand|land|band|sand|stand|plan|than|fan|van)$/.test(word)) score += 120;

  if(candidate && candidate.kind === "near"){
    if(Math.abs((Number(candidate.numSyllables)||0) - seedSyll) <= 1) score += 22;
    if(word.slice(-2) === seed.slice(-2)) score += 12;
  }
...
function estimateSyllables(word){
  ...
  if(!w) return 0;
  const parts = w.match(/[aeiouy]+/g);
  const count = parts ? parts.length : 1;
  return Math.max(1, count - (/e$/.test(w) && !/(le|ye)$/.test(w) ? 1 : 0));
}

function rankRhymeCandidates(seedWord, perfectList, nearList){
  ...
  const push = (entry) => {
    ...
    if(existing.kind !== "perfect" && entry.kind === "perfect"){
      merged.set(entry.word, { ...existing, ...entry, kind: "perfect" });
      return;
    }
    merged.set(entry.word, {
      ...existing,
      score: Math.max(existing.score || 0, entry.score || 0),
      numSyllables: existing.numSyllables || entry.numSyllables || 0,
      tags: [...new Set([...(existing.tags || []), ...(entry.tags || [])])]
    });
  };

  (perfectList || []).forEach(push);
  (nearList || []).forEach(push);

  const all = Array.from(merged.values()).map(entry => ({
    ...entry,
    rankScore: scoreLyricFriendliness(entry, seedWord)
  })).filter(entry => entry.rankScore > -260);

  const perfect = all
    .filter(entry => entry.kind === "perfect")
    .sort((a,b) => (b.rankScore - a.rankScore) || (b.score - a.score) || a.word.localeCompare(b.word))
    .slice(0, 22);

  const used = new Set(perfect.map(x => x.word));
  const near = all
...
function buildRhymeChip(word, kindLabel){
  const b = document.createElement("div");
  b.className = "rWord";
  b.style.flex = "0 0 auto";
  b.style.whiteSpace = "nowrap";
  b.textContent = word;
  b.dataset.kind = kindLabel || "";
  if(kindLabel === "near"){
    b.style.background = "#ffd666";
  }
...
function appendRhymeGroup(parent, title, items, kindLabel){
  ...
  items.forEach(item => {
    group.appendChild(buildRhymeChip(item.word, kindLabel));
  });
...
async function renderRhymes(seed){
  ...
  try{
    ...
    el.rhymeWords.style.flexWrap = "wrap";
    el.rhymeWords.style.alignContent = "flex-start";
    el.rhymeWords.style.gap = "10px";
    el.rhymeWords.style.overflowX = "hidden";
    el.rhymeWords.style.overflowY = "auto";
    ...
    el.rhymeWords.style.touchAction = "pan-y";
    el.rhymeWords.style.height = "132px";
    el.rhymeWords.style.maxHeight = "132px";
  }catch{}
  ...
  const [perfectRaw, nearRaw] = await Promise.all([
    fetchDatamuseRhymes(word, 64),
    fetchDatamuseNearRhymes(word, 64)
  ]);
  const ranked = rankRhymeCandidates(word, perfectRaw, nearRaw);

  el.rhymeWords.innerHTML = "";

  if((ranked.perfect.length + ranked.near.length) === 0){
    status.textContent = "No good rhymes found (try another word).";
    el.rhymeWords.appendChild(status);
    return;
  }

  appendRhymeGroup(el.rhymeWords, "Perfect Rhymes", ranked.perfect, "perfect");
  appendRhymeGroup(el.rhymeWords, "Near Rhymes", ranked.near, "near");
}
...
function updateAudioButtonsUI(){
  // Keep Beat 1 hidden so no extra phantom button appears beside Upload.
  if(el.beat1Btn){
    el.beat1Btn.style.display = "none";
  }

  // In Show mode, hide Upload while AutoScroll is active/paused so the row stays clean.
  // In Hide mode, keep Upload out of the mini bar entirely (same as current layout).
  if(el.uploadAudioBtn){
    const panelHidden = !!el.panelBody?.classList.contains("hidden");
    const hideUpload = (!panelHidden) && !!state.autoScrollOn;
    el.uploadAudioBtn.style.display = hideUpload ? "none" : "inline-flex";
    el.uploadAudioBtn.textContent = "⬆️";
    el.uploadAudioBtn.title = "Upload audio or PDF";
    el.uploadAudioBtn.setAttribute("aria-label", "Upload audio or PDF");
  }
}
  function renderAll(){
  renderProjectsDropdown();
...
function getProjectNotesImages(project){
  return Array.isArray(project?.notesImages) ? project.notesImages.map(x => ({ ...(x || {}) })) : [];
}

function setProjectNotesImages(project, manifest){
  if(!project || typeof project !== "object") return project;
  project.notesImages = Array.isArray(manifest)
    ? manifest.filter(Boolean).map((entry) => ({
        id: String(entry.id || ""),
        projectId: String(entry.projectId || project.id || ""),
        title: String(entry.title || ""),
        createdAt: Number(entry.createdAt) || now(),
        ...
        size: Number(entry.size) || 0,
        deleted: !!entry.deleted
      })).filter((entry) => entry.id).sort((a,b) => (Number(b.updatedAt)||0) - (Number(a.updatedAt)||0))
    : [];
  return project;
}

function getCloudNoteImagePath(projectId, imageId, blobOrType){
  const userId = String(getAuthUserId() || "").trim();
  const pid = String(projectId || "").trim();
  ...
  else if(type.includes("heic")) ext = ".heic";
  return `${userId}/${pid}/notes/${iid}${ext}`;
}

function noteImageToManifestEntry(rec){
  if(!rec || !rec.id) return null;
  const path = String(rec.cloudPath || getCloudNoteImagePath(rec.projectId, rec.id, rec.blob || rec.contentType || "image/jpeg"));
  return {
    id: String(rec.id),
    projectId: String(rec.projectId || ""),
    title: String(rec.title || ""),
    ...
    updatedAt: Number(rec.updatedAt) || Number(rec.createdAt) || now(),
    path,
    contentType: String(rec.blob?.type || rec.contentType || "image/jpeg"),
    size: Number(rec.blob?.size || rec.size || 0),
    deleted: false
  };
}

async function uploadNoteImageToCloud(rec){
  if(!cloud.client || !cloud.user || !rec || !rec.id || !rec.blob) return null;
  const entry = noteImageToManifestEntry(rec);
  const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).upload(entry.path, rec.blob, {
    upsert: true,
    contentType: entry.contentType || undefined,
    cacheControl: "3600"
  });
  if(error) throw error;
  rec.cloudPath = entry.path;
  rec.contentType = entry.contentType;
  rec.size = entry.size;
  return { ...entry };
}

async function downloadNoteImageFromCloud(entry){
  if(!cloud.client || !cloud.user || !entry?.path) return null;
  const { data, error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).download(entry.path);
  if(error) throw error;
  const blob = data instanceof Blob ? data : new Blob([data], { type: entry.contentType || "image/jpeg" });
  return {
    id: String(entry.id),
    projectId: String(entry.projectId || ""),
    title: String(entry.title || ""),
    ...
    blob,
    cloudPath: String(entry.path || ""),
    contentType: String(entry.contentType || blob.type || "image/jpeg"),
    size: Number(entry.size) || Number(blob.size) || 0
  };
}

async function removeNoteImageFromCloud(entry){
  if(!cloud.client || !cloud.user || !entry?.path) return;
  try{
    const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).remove([entry.path]);
    if(error) throw error;
  }catch(err){
    console.warn("Cloud note image remove failed", err);
  }
}

async function saveProjectNotesImageManifest(project, manifest){
  if(!project || !project.id) return;
  const next = normalizeProject(deepClone(project));
  setProjectNotesImages(next, manifest);
  if(state.project && String(state.project.id || "") === String(next.id || "")) state.project = next;
  cloud.suspendQueue = true;
  try{
    upsertProject(next);
...
async function syncCloudNoteImagesForAllProjects(reason=""){
  if(!cloud.client || !cloud.user || !getAuthUserId()) return;
  const allProjects = loadAllProjects().map(normalizeProject).filter(Boolean);
  const allImages = await dbGetAllNoteImages().catch(() => []);
  const imageMap = new Map((Array.isArray(allImages) ? allImages : []).map(r => [String(r.id || ""), r]));
  let anyManifestChanged = false;
  let currentProjectTouched = false;
  ...
  for(const project of allProjects){
    const projectId = String(project.id || "");
    if(!projectId) continue;
    const manifestMap = new Map(getProjectNotesImages(project).map(e => [String(e.id || ""), { ...(e || {}) }]));
    const localProjectImages = (Array.isArray(allImages) ? allImages : []).filter(r => String(r.projectId || "") === projectId);
    let manifestChanged = false;
    let projectTouched = false;

    for(const rec of localProjectImages){
      if(!rec || !rec.id || !rec.blob) continue;
      const localUpdated = Number(rec.updatedAt) || Number(rec.createdAt) || now();
      const remoteEntry = manifestMap.get(String(rec.id));
      const remoteUpdated = Number(remoteEntry?.updatedAt) || 0;
      if(!remoteEntry || remoteEntry.deleted || localUpdated > remoteUpdated){
        const uploadedEntry = await uploadNoteImageToCloud(rec);
        uploadedEntry.updatedAt = localUpdated;
        uploadedEntry.createdAt = Number(rec.createdAt) || uploadedEntry.createdAt;
        uploadedEntry.title = String(rec.title || "");
        manifestMap.set(String(rec.id), uploadedEntry);
        ...
        const remoteRec = await downloadNoteImageFromCloud(remoteEntry);
        await dbPutNoteImage(remoteRec);
        imageMap.set(String(remoteRec.id), remoteRec);
        projectTouched = true;
      }
    }

    for(const [imageId, entry] of manifestMap.entries()){
      const localRec = imageMap.get(String(imageId));
      const localUpdated = Number(localRec?.updatedAt) || Number(localRec?.createdAt) || 0;
      const remoteUpdated = Number(entry?.updatedAt) || 0;
      if(entry?.deleted){
        if(localRec && remoteUpdated >= localUpdated){
          await dbDeleteNoteImage(imageId);
          imageMap.delete(imageId);
          projectTouched = true;
        }
        await removeNoteImageFromCloud(entry);
        continue;
      }
      if(!localRec){
        const remoteRec = await downloadNoteImageFromCloud(entry);
        await dbPutNoteImage(remoteRec);
        imageMap.set(String(remoteRec.id), remoteRec);
        projectTouched = true;
      }
    ...
    const nextManifest = Array.from(manifestMap.values()).filter(Boolean).sort((a,b) => (Number(b.updatedAt)||0) - (Number(a.updatedAt)||0));
    if(JSON.stringify(getProjectNotesImages(project)) !== JSON.stringify(nextManifest)){
      await saveProjectNotesImageManifest(project, nextManifest);
      manifestChanged = true;
    }
    if(manifestChanged) anyManifestChanged = true;
    if(projectTouched) currentProjectTouched = currentProjectTouched || (state.project && String(state.project.id||"") === projectId);
  ...
  if(anyManifestChanged) queueCloudSync(`notes-image-manifest-${reason || "sync"}`);
  if(currentProjectTouched && state.notesOpen) resolveNotesImageElementSources();
}
...
function findNotesTopLevelBlock(node){
  let cur = node;
  while(cur && cur !== el?.notesEditor){
    if(cur.parentNode === el.notesEditor && cur.nodeType === 1) return cur;
    cur = cur.parentNode;
  }
  return null;
}
...
function createNotesTodoLine(html="", checked=false){
  ...
  text.className = NOTES_TODO_TEXT_CLASS;
  text.innerHTML = html && String(html).trim() ? html : "<br>";
...
function createNotesPlainLine(html="<br>"){
  const line = document.createElement("div");
  line.innerHTML = html && String(html).trim() ? html : "<br>";
  return line;
}
...
function notesTodoTextValue(line){
  const text = ensureNotesTodoText(line);
  return normalizeLineBreaks(String(text?.innerText || text?.textContent || "").replace(/\u00A0/g, " ")).trim();
}
...
function insertNotesTodoLineAfter(line, html="<br>"){
  if(!el.notesEditor) return null;
  const todo = createNotesTodoLine(html, false);
  if(line && line.parentNode === el.notesEditor){
    line.parentNode.insertBefore(todo, line.nextSibling);
  }else{
    el.notesEditor.appendChild(todo);
  }
  return todo;
}
...
function insertChecklistIntoNotes(){
  ...
  const todo = (() => {
    ...
    if(el.notesEditor.childNodes.length){
      el.notesEditor.appendChild(newTodo);
    }else{
      el.notesEditor.appendChild(newTodo);
    }
    return newTodo;
  ...
  saveNotesDoc();
  pushNotesHistorySnapshot(true);
  if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync('notes-image-move');
  return true;
}

function normalizeNotesTodoStructure(root = el?.notesEditor){
  if(!root) return;
  ...
  todos.forEach((line) => {
    ...
    const text = ensureNotesTodoText(line);
    if(!String(text.innerHTML || "").trim()) text.innerHTML = "<br>";
  });
}
...
function serializeNotesEditorHtml(){
  if(!el?.notesEditor) return "";
  const clone = el.notesEditor.cloneNode(true);
  clone.querySelectorAll('.notesImageBlock').forEach((node) => {
    node.classList.remove('is-selected','is-loading');
  });
  clone.querySelectorAll('.notesImageResizeHandle,.notesImageDeleteBtn,.notesImageCropBtn,.notesImageMoveUpBtn,.notesImageMoveDownBtn').forEach((node) => node.remove());
  clone.querySelectorAll('img[data-notes-image-id]').forEach((img) => {
    img.removeAttribute('src');
    img.removeAttribute('srcset');
    img.removeAttribute('data-notes-image-loaded');
...
function currentNotesHtml(){
  return normalizeNotesHtml(stripNotesRhymeMarkers(serializeNotesEditorHtml()));
}

function getUsedNotesImageIds(root = el?.notesEditor){
  const out = new Set();
  try{
    root?.querySelectorAll?.('[data-notes-image-id]').forEach((node) => {
      const id = String(node.getAttribute('data-notes-image-id') || "").trim();
      if(id) out.add(id);
    });
  }catch{}
  return out;
...
function selectNotesImageBlockById(imageId){
  clearNotesImageSelection();
  const id = String(imageId || "").trim();
  if(!id || !el?.notesEditor) return;
  const block = el.notesEditor.querySelector(`.notesImageBlock[data-notes-image-id="${id}"]`);
  if(block){
    block.classList.add('is-selected');
    state.notesSelectedImageId = id;
  }
}

async function moveNotesImageBlockBy(id, direction){
  const imageId = String(id || '').trim();
  if(!imageId || !el?.notesEditor) return false;
  const block = el.notesEditor.querySelector(`.notesImageBlock[data-notes-image-id="${imageId}"]`);
  if(!block || !block.parentNode) return false;

  const isSpacer = (node) => !!(node && node.nodeType === 1 && String(node.innerHTML || '').trim().toLowerCase() === '<br>');
  const prevElem = (node) => {
  ...
  let spacerAfter = nextElem(block);
  if(!isSpacer(spacerAfter)) spacerAfter = null;

  if(direction < 0){
    let target = prevElem(block);
    if(isSpacer(target)) target = prevElem(target);
    if(!target) return false;
    block.parentNode.insertBefore(block, target);
    if(spacerAfter) block.parentNode.insertBefore(spacerAfter, block.nextSibling);
  }else{
    let after = nextElem(block);
    if(isSpacer(after)) after = nextElem(after);
    if(!after) return false;
    const insertBefore = nextElem(after);
    if(insertBefore) block.parentNode.insertBefore(block, insertBefore);
    else block.parentNode.appendChild(block);
    if(spacerAfter) block.parentNode.insertBefore(spacerAfter, block.nextSibling);
  }

  selectNotesImageBlockById(imageId);
  saveNotesDoc();
  pushNotesHistorySnapshot(true);
  if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync('notes-image-delete');
  return true;
}

function startNotesImageResize(block, startEvent){
  if(!block) return;
  const rect = block.getBoundingClientRect();
  state.notesImageResize = {
    block,
    startX: Number(startEvent?.clientX || 0),
    startWidth: Math.max(140, Math.round(rect.width || block.offsetWidth || 420))
  };
  try{ block.setPointerCapture?.(startEvent.pointerId); }catch{}
}

function moveNotesImageResize(ev){
  const rz = state.notesImageResize;
  if(!rz?.block) return;
  const dx = Number(ev?.clientX || 0) - Number(rz.startX || 0);
  const hostW = Math.max(160, Math.round(el?.notesEditor?.clientWidth || window.innerWidth || 420) - 8);
  const nextW = clamp(Math.round((rz.startWidth || 420) + dx), 140, hostW);
  rz.block.style.width = `min(100%, ${nextW}px)`;
}

function stopNotesImageResize(commit=false){
  const rz = state.notesImageResize;
  if(!rz?.block){ state.notesImageResize = null; return; }
  const block = rz.block;
  try{ block.releasePointerCapture?.(rz.pointerId); }catch{}
  state.notesImageResize = null;
  if(commit){
    saveNotesDoc();
    pushNotesHistorySnapshot(true);
    if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync('notes-image-resize');
  }
}

function startNotesImageDrag(block, startEvent){
  if(!block) return;
  const id = String(block.getAttribute('data-notes-image-id') || '').trim();
  if(!id) return;
  try{ clearTimeout(state.notesImageDrag?.holdTimer); }catch{}
  ...
  state.notesImageDrag = {
    id,
    block,
    pointerId: Number.isFinite(startEvent?.pointerId) ? startEvent.pointerId : null,
    startX: Number(startEvent?.clientX || 0),
    startY: Number(startEvent?.clientY || 0),
    lastX: Number(startEvent?.clientX || 0),
    lastY: Number(startEvent?.clientY || 0),
    active: false,
    holdTimer: null,
    baseX,
    baseY,
  ...
  drag.docMove = (ev) => { moveNotesImageDrag(ev); };
  drag.docUp = () => { if(state.notesImageDrag?.block) stopNotesImageDrag(); };

  document.addEventListener('pointermove', drag.docMove, { passive:false });
  document.addEventListener('pointerup', drag.docUp, true);
  document.addEventListener('pointercancel', drag.docUp, true);

  try{ block.setPointerCapture?.(startEvent.pointerId); }catch{}

  drag.holdTimer = setTimeout(() => {
    const live = state.notesImageDrag;
    if(!live || live.id !== id || live.block !== block) return;
    live.active = true;
    selectNotesImageBlockById(id);
    block.classList.add('is-selected');
...
function moveNotesImageDrag(ev){
  const drag = state.notesImageDrag;
  if(!drag?.block) return;
  if(drag.pointerId != null && Number.isFinite(ev?.pointerId) && ev.pointerId !== drag.pointerId) return;
  ...
  const dxFromStart = x - Number(drag.startX || 0);
  const dyFromStart = y - Number(drag.startY || 0);

  if(!drag.active){
    if(Math.abs(dxFromStart) > 18 || Math.abs(dyFromStart) > 18){
      try{ clearTimeout(drag.holdTimer); }catch{}
      drag.holdTimer = null;
    }
    return;
  }
  ...
  drag.moved = true;
  drag.freeX = Math.round(Number(drag.baseX || 0) + dxFromStart);
  drag.freeY = Math.round(Number(drag.baseY || 0) + dyFromStart);

  drag.block.dataset.freeX = String(drag.freeX);
  drag.block.dataset.freeY = String(drag.freeY);
  drag.block.style.transform = `translate(${drag.freeX}px, ${drag.freeY}px)`;
}

function stopNotesImageDrag(){
  const drag = state.notesImageDrag;
  if(drag?.holdTimer){
    try{ clearTimeout(drag.holdTimer); }catch{}
  }
  if(drag?.docMove) document.removeEventListener('pointermove', drag.docMove, { passive:false });
  if(drag?.docUp){
    document.removeEventListener('pointerup', drag.docUp, true);
    document.removeEventListener('pointercancel', drag.docUp, true);
  }
  if(drag?.block){
    try{ drag.block.releasePointerCapture?.(drag.pointerId); }catch{}
    drag.block.style.opacity = '';
    drag.block.style.zIndex = '';
    drag.block.style.transition = '';
    if(drag.active){
      const freeX = Number(drag.freeX || 0) || 0;
      const freeY = Number(drag.freeY || 0) || 0;
      drag.block.dataset.freeX = String(freeX);
      drag.block.dataset.freeY = String(freeY);
      drag.block.style.transform = `translate(${freeX}px, ${freeY}px)`;
    }
  }
...
function removeSelectedNotesImageBlock(){
  const id = String(state.notesSelectedImageId || "").trim();
  if(!id || !el?.notesEditor) return false;
  const block = el.notesEditor.querySelector(`.notesImageBlock[data-notes-image-id="${id}"]`);
...
function ensureNotesImageBlockChrome(block){
  if(!block) return;
  block.classList.add('is-loading');

  const makeBtn = (cls, label, title) => {
    let btn = block.querySelector('.' + cls);
    if(btn) return btn;
    btn = document.createElement('button');
    btn.type = 'button';
    btn.className = cls;
  ...
  makeBtn('notesImageDeleteBtn', '×', 'Delete photo');
  makeBtn('notesImageCropBtn', '✂', 'Crop photo');
...
async function getSignedOrPublicNoteImageUrl(path){
  try{
    const p = String(path || '').trim();
    if(!p) return '';
    if(cloud?.client?.storage?.from){
      try{
        const signed = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).createSignedUrl(p, 60 * 60 * 24 * 7);
        const signedUrl = String(signed?.data?.signedUrl || '').trim();
        if(signedUrl) return signedUrl;
      }catch(err){
        console.warn('Notes signed url failed', err);
      }
      try{
        const pub = cloud.client.storage.from(CLOUD_AUDIO_BUCKET).getPublicUrl(p);
        const publicUrl = String(pub?.data?.publicUrl || '').trim();
        if(publicUrl) return publicUrl;
      }catch(err){
        console.warn('Notes public url failed', err);
      }
    }
    const base = String(SUPABASE_URL || '').replace(/\/$/, '');
    return base && p ? `${base}/storage/v1/object/public/${CLOUD_AUDIO_BUCKET}/${p}` : '';
  }catch{
    return '';
  }
}
...
function getPublicNoteImageUrl(path){
  try{
    const p = String(path || '').trim();
    if(!p) return '';
    if(cloud?.client?.storage?.from){
...
async function resolveNotesImageElementSources(){
  if(!el?.notesEditor) return;
  const imgs = Array.from(el.notesEditor.querySelectorAll('img[data-notes-image-id]'));
  const manifest = getProjectNotesImages(state.project);
  const manifestMap = new Map(manifest.map((entry) => [String(entry.id || ''), entry]));
  for(const img of imgs){
    const id = String(img.getAttribute('data-notes-image-id') || '').trim();
    if(!id) continue;
    const block = img.closest('.notesImageBlock');
    ensureNotesImageBlockChrome(block);

    if(block){
      const fx = Number(block.dataset.freeX || 0) || 0;
      const fy = Number(block.dataset.freeY || 0) || 0;
      block.style.transform = `translate(${fx}px, ${fy}px)`;
    }

    try{
      const rec = await dbGetNoteImage(id);
      if(rec?.blob){
        try{ if(img.dataset.objectUrl) URL.revokeObjectURL(img.dataset.objectUrl); }catch{}
        const url = URL.createObjectURL(rec.blob);
        img.onload = () => { block?.classList.remove('is-loading'); };
        img.onerror = () => { block?.classList.remove('is-loading'); };
        img.src = url;
        img.dataset.objectUrl = url;
        img.setAttribute('data-notes-image-loaded', '1');
        if(img.complete) block?.classList.remove('is-loading');
        continue;
      }
    }catch(err){
      console.warn('Notes image resolve failed', err);
    }
    ...
    const publicUrl = getPublicNoteImageUrl(entry.path);
    if(publicUrl){
      img.onload = () => { block?.classList.remove('is-loading'); };
      img.onerror = async () => {
        try{
          if(entry?.path && cloud?.client && cloud?.user){
            const remoteRec = await downloadNoteImageFromCloud(entry);
            if(remoteRec?.blob){
              await dbPutNoteImage(remoteRec);
              try{ if(img.dataset.objectUrl) URL.revokeObjectURL(img.dataset.objectUrl); }catch{}
              const url = URL.createObjectURL(remoteRec.blob);
              img.src = url;
              img.dataset.objectUrl = url;
              block?.classList.remove('is-loading');
            }else{
          ...
          console.warn('Notes image fallback load failed', err);
          block?.classList.remove('is-loading');
        }
      };
      img.src = publicUrl;
...
function injectNotesCropStyles(){
  const old = document.getElementById('notesCropStyles');
  if(old) return;
  const style = document.createElement('style');
  style.id = 'notesCropStyles';
  style.textContent = `
    .notesImageCropBtn{
      position:absolute;
      top:8px;
    ...
    .notesCropModal.show{ display:flex; }
    .notesCropCard{
    ...
    .notesCropTop{
      ...
      font-weight:1100;
      background:linear-gradient(#fafafa,#f1f1f2);
    }
    .notesCropBtns{
      display:flex;
      gap:8px;
    ...
    .notesCropHandle.nw{ left:-9px; top:-9px; cursor:nwse-resize; }
    .notesCropHandle.ne{ right:-9px; top:-9px; cursor:nesw-resize; }
    .notesCropHandle.sw{ left:-9px; bottom:-9px; cursor:nesw-resize; }
    .notesCropHandle.se{ right:-9px; bottom:-9px; cursor:nwse-resize; }
    .notesCropHint{
      padding:0 14px 14px;
      font-size:12px;
      font-weight:900;
...
function ensureNotesCropModal(){
  injectNotesCropStyles();
  let modal = document.getElementById('notesCropModal');
  if(!modal){
    modal = document.createElement('div');
    modal.id = 'notesCropModal';
    modal.className = 'notesCropModal';
    modal.innerHTML = `
      <div class="notesCropCard">
        <div class="notesCropTop">
          <div>Crop photo</div>
          <div class="notesCropBtns">
            <button type="button" class="notesCropBtn" id="notesCropResetBtn">Reset</button>
            <button type="button" class="notesCropBtn" id="notesCropCancelBtn">Cancel</button>
            <button type="button" class="notesCropBtn primary" id="notesCropApplyBtn">Apply</button>
          </div>
        </div>
        <div class="notesCropBody">
          <div class="notesCropStage" id="notesCropStage">
            <img id="notesCropImage" alt="Crop preview" />
            <div class="notesCropSelection" id="notesCropSelection">
              <span class="notesCropHandle nw" data-handle="nw"></span>
              <span class="notesCropHandle ne" data-handle="ne"></span>
              <span class="notesCropHandle sw" data-handle="sw"></span>
              <span class="notesCropHandle se" data-handle="se"></span>
            </div>
          </div>
        </div>
        <div class="notesCropHint">Drag the box to move it. Drag the corner circles to resize.</div>
      </div>
    `;
    document.body.appendChild(modal);

    modal.addEventListener('click', (e) => {
      if(e.target === modal) closeNotesCropModal();
    });
    modal.querySelector('#notesCropCancelBtn')?.addEventListener('click', closeNotesCropModal);
    modal.querySelector('#notesCropResetBtn')?.addEventListener('click', resetNotesCropSelection);
    modal.querySelector('#notesCropApplyBtn')?.addEventListener('click', () => { applyNotesCrop().catch(err => console.error(err)); });

    const selection = modal.querySelector('#notesCropSelection');
    selection?.addEventListener('pointerdown', startNotesCropPointer);
    window.addEventListener('resize', () => {
      if(state.notesCrop?.open) requestAnimationFrame(resetNotesCropSelection);
    }, { passive:true });
  }
  return {
    modal,
    stage: modal.querySelector('#notesCropStage'),
    img: modal.querySelector('#notesCropImage'),
    selection: modal.querySelector('#notesCropSelection'),
    applyBtn: modal.querySelector('#notesCropApplyBtn')
  };
}

function getNotesCropBounds(){
  const ui = ensureNotesCropModal();
  const w = Math.max(1, Math.round(ui.stage?.clientWidth || ui.img?.clientWidth || 1));
  const h = Math.max(1, Math.round(ui.stage?.clientHeight || ui.img?.clientHeight || 1));
  return { w, h };
}

function renderNotesCropSelection(){
  const ui = ensureNotesCropModal();
  const sel = state.notesCrop?.sel;
  if(!sel || !ui.selection) return;
  ui.selection.style.left = `${Math.round(sel.x)}px`;
  ui.selection.style.top = `${Math.round(sel.y)}px`;
  ui.selection.style.width = `${Math.round(sel.w)}px`;
  ui.selection.style.height = `${Math.round(sel.h)}px`;
}

function resetNotesCropSelection(){
  const crop = state.notesCrop;
  if(!crop?.open) return;
  const { w, h } = getNotesCropBounds();
  const insetX = Math.max(12, Math.round(w * 0.10));
  const insetY = Math.max(12, Math.round(h * 0.10));
  crop.sel = {
    x: insetX,
    y: insetY,
    w: Math.max(40, w - insetX * 2),
    h: Math.max(40, h - insetY * 2)
  };
  renderNotesCropSelection();
}

function closeNotesCropModal(){
  const ui = ensureNotesCropModal();
  ui.modal.classList.remove('show');
  if(state.notesCrop?.objectUrl){
    try{ URL.revokeObjectURL(state.notesCrop.objectUrl); }catch{}
  }
  state.notesCrop = null;
}

async function getNoteImageRecordForCrop(imageId){
  const id = String(imageId || '').trim();
  if(!id || !state.project) return null;
  let rec = null;
  try{ rec = await dbGetNoteImage(id); }catch{}
  if(rec?.blob) return rec;

  const manifestEntry = getProjectNotesImages(state.project).find((entry) => String(entry.id || '') === id && !entry.deleted);
  if(manifestEntry?.path && cloud?.client && cloud?.user){
    try{
      rec = await downloadNoteImageFromCloud(manifestEntry);
      if(rec?.blob){
        await dbPutNoteImage(rec);
        return rec;
      }
    }catch(err){
      console.warn('Crop image cloud download failed', err);
    }
  }

  const img = el?.notesEditor?.querySelector?.(`img[data-notes-image-id="${id}"]`);
  ...
  if(src){
    try{
      ...
      rec = {
        ...
        title: String(manifestEntry?.title || img?.alt || 'Photo'),
        createdAt: Number(manifestEntry?.createdAt) || now(),
        updatedAt: Number(manifestEntry?.updatedAt) || now(),
        blob,
        contentType: String(blob.type || manifestEntry?.contentType || 'image/jpeg'),
        size: Number(blob.size) || 0,
        cloudPath: String(manifestEntry?.path || '')
      };
      await dbPutNoteImage(rec);
      return rec;
    }catch(err){
      console.warn('Crop image fetch fallback failed', err);
    }
  }
  return null;
}

async function openNotesCropForImage(imageId){
  const id = String(imageId || '').trim();
  if(!id) return false;
  const rec = await getNoteImageRecordForCrop(id);
  ...
  ui.img.onload = () => {
    requestAnimationFrame(() => {
      resetNotesCropSelection();
    });
  };
  ui.img.src = url;
  if(ui.img.complete) requestAnimationFrame(() => resetNotesCropSelection());
  return true;
}

function startNotesCropPointer(ev){
  const crop = state.notesCrop;
  const ui = ensureNotesCropModal();
  if(!crop?.open || !ui.selection) return;
  ev.preventDefault();
  ev.stopPropagation();

  const rect = ui.selection.getBoundingClientRect();
  ...
  crop.drag = {
    ...
    startSel: { ...(crop.sel || { x:0, y:0, w:rect.width, h:rect.height }) },
    bounds: { w: Math.round(stageRect.width || 1), h: Math.round(stageRect.height || 1) }
  };
  try{ ui.selection.setPointerCapture?.(ev.pointerId); }catch{}

  const move = (e2) => { moveNotesCropPointer(e2); };
  const up = (e2) => { stopNotesCropPointer(e2); };
  crop.drag.docMove = move;
  crop.drag.docUp = up;
...
function moveNotesCropPointer(ev){
  const crop = state.notesCrop;
  ...
  const b = drag.bounds || { w:1, h:1 };
  let { x, y, w, h } = drag.startSel || crop.sel;

  if(drag.mode === 'move'){
    x = clamp(Math.round(drag.startSel.x + dx), 0, Math.max(0, b.w - drag.startSel.w));
    y = clamp(Math.round(drag.startSel.y + dy), 0, Math.max(0, b.h - drag.startSel.h));
    w = drag.startSel.w;
    h = drag.startSel.h;
  }else{
    let left = drag.startSel.x;
    let top = drag.startSel.y;
    let right = drag.startSel.x + drag.startSel.w;
    let bottom = drag.startSel.y + drag.startSel.h;
    const handle = String(drag.mode || '').replace('resize-','');
    if(handle.includes('n')) top = clamp(Math.round(drag.startSel.y + dy), 0, bottom - minSize);
    if(handle.includes('s')) bottom = clamp(Math.round(drag.startSel.y + drag.startSel.h + dy), top + minSize, b.h);
    if(handle.includes('w')) left = clamp(Math.round(drag.startSel.x + dx), 0, right - minSize);
    if(handle.includes('e')) right = clamp(Math.round(drag.startSel.x + drag.startSel.w + dx), left + minSize, b.w);
...
function stopNotesCropPointer(ev){
  const crop = state.notesCrop;
  const drag = crop?.drag;
  if(!drag) return;
  if(drag.docMove) document.removeEventListener('pointermove', drag.docMove, { passive:false });
  ...
  try{ ensureNotesCropModal().selection.releasePointerCapture?.(drag.pointerId); }catch{}
  crop.drag = null;
}

async function refreshNotesImageElementFromBlob(imageId, blob){
  const id = String(imageId || '').trim();
  if(!id || !blob) return;
  const img = el?.notesEditor?.querySelector?.(`img[data-notes-image-id="${id}"]`);
  const block = img?.closest?.('.notesImageBlock');
...
async function applyNotesCrop(){
  const crop = state.notesCrop;
  if(!crop?.open || !crop.rec?.blob || !state.project) return false;
  const ui = ensureNotesCropModal();
  const sel = crop.sel;
  if(!sel || !ui.img?.naturalWidth || !ui.img?.naturalHeight) return false;

  ui.applyBtn.disabled = true;
  ui.applyBtn.textContent = 'Applying…';
  try{
    const renderW = Math.max(1, ui.stage.clientWidth || ui.img.clientWidth || 1);
    const renderH = Math.max(1, ui.stage.clientHeight || ui.img.clientHeight || 1);
    const scaleX = ui.img.naturalWidth / renderW;
    const scaleY = ui.img.naturalHeight / renderH;
    ...
    ctx.drawImage(ui.img, sx, sy, sw, sh, 0, 0, sw, sh);

    const origType = String(crop.rec.blob.type || crop.rec.contentType || 'image/jpeg').toLowerCase();
    const outType = (origType.includes('png') || origType.includes('webp')) ? origType : 'image/jpeg';

    const newBlob = await new Promise((resolve, reject) => {
      canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('Crop failed')), outType, 0.92);
    });

    const oldEntry = getProjectNotesImages(state.project).find((entry) => String(entry.id || '') === String(crop.imageId || ''));
    const updatedRec = {
      ...(crop.rec || {}),
      id: String(crop.imageId || ''),
      projectId: String(state.project.id || ''),
      title: String(crop.rec.title || oldEntry?.title || 'Photo'),
      createdAt: Number(crop.rec.createdAt) || Number(oldEntry?.createdAt) || now(),
      updatedAt: now(),
      blob: newBlob,
      contentType: String(newBlob.type || outType || 'image/jpeg'),
      size: Number(newBlob.size) || 0,
      cloudPath: ''
    };

    await dbPutNoteImage(updatedRec);

    editProject('notesImageCrop', () => {
      const manifest = getProjectNotesImages(state.project).filter((item) => String(item.id || '') !== String(updatedRec.id || ''));
      manifest.unshift(noteImageToManifestEntry(updatedRec));
      setProjectNotesImages(state.project, manifest);
    });

    if(cloud.ready && cloud.client && getAuthUserId()){
      try{
        const uploaded = await uploadNoteImageToCloud(updatedRec);
        if(uploaded){
          editProject('notesImageCropCloud', () => {
            const manifest = getProjectNotesImages(state.project).filter((item) => String(item.id || '') !== String(updatedRec.id || ''));
            manifest.unshift(uploaded);
            setProjectNotesImages(state.project, manifest);
          });
          const oldPath = String(oldEntry?.path || '').trim();
          const newPath = String(uploaded?.path || '').trim();
          if(oldPath && newPath && oldPath !== newPath){
            removeNoteImageFromCloud({ path: oldPath }).catch(()=>{});
          }
        }
      }catch(err){
        console.warn('Notes image crop upload failed', err);
      }
    }

    await refreshNotesImageElementFromBlob(updatedRec.id, newBlob);
    saveNotesDoc();
    pushNotesHistorySnapshot(true);
    if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync('notes-image-crop');
    closeNotesCropModal();
    selectNotesImageBlockById(updatedRec.id);
    return true;
  }catch(err){
    console.error(err);
    return false;
    ...
    ui.applyBtn.textContent = 'Apply';
  }
}


function syncProjectNotesImagesFromEditor(){
  if(!state.project || !el?.notesEditor) return;
  const used = getUsedNotesImageIds(el.notesEditor);
  const manifest = getProjectNotesImages(state.project);
  let changed = false;
  const next = manifest.map((entry) => {
    const id = String(entry.id || '');
    if(!id) return entry;
    if(!used.has(id) && !entry.deleted){
      changed = true;
      dbDeleteNoteImage(id).catch(()=>{});
      return { ...entry, deleted:true, updatedAt: now() };
    }
    return entry;
  });
  if(changed) setProjectNotesImages(state.project, next);
}

async function insertNotesImageFromFile(file){
  if(!file || !state.project || !el?.notesEditor) return false;
  const imageId = uuid();
  const rec = {
    id: imageId,
    projectId: String(state.project.id || ''),
    title: String(file.name || 'Photo'),
    createdAt: now(),
    updatedAt: now(),
    blob: file,
  ...
  await dbPutNoteImage(rec);
  const entry = noteImageToManifestEntry(rec);
  editProject('notesImageAdd', () => {
    const manifest = getProjectNotesImages(state.project).filter((item) => String(item.id || '') !== imageId);
  ...
  if(cloud.ready && cloud.client && getAuthUserId()){
    try{
      const uploaded = await uploadNoteImageToCloud(rec);
      if(uploaded){
        editProject('notesImageCloudPath', () => {
          const manifest = getProjectNotesImages(state.project).filter((item) => String(item.id || '') !== imageId);
          manifest.unshift(uploaded);
          setProjectNotesImages(state.project, manifest);
        });
      ...
      console.warn('Immediate notes image upload failed', err);
    }
  }

  restoreNotesSelection();
  const html = `<div class="notesImageBlock" contenteditable="false" data-notes-image-id="${imageId}" style="width:min(100%, 420px); transform:translate(0px, 0px);"><img class="notesImage" data-notes-image-id="${imageId}" alt="${escapeHtml(file.name || 'Photo')}" loading="lazy" decoding="async"></div><div><br></div>`;
  try{
  ...
  clearNotesImageSelection();
  await resolveNotesImageElementSources();
  saveNotesDoc();
  pushNotesHistorySnapshot(true);
  if(cloud.ready && cloud.client && getAuthUserId()) triggerImmediateCloudSync('notes-image-add');
  return true;
}
...
function applyNotesHistoryAt(index){
  ...
  resolveNotesImageElementSources();
  applyNotesEditorDefaults();
...
function hasExplicitNotesColor(node){
  try{
    ...
    while(cur && cur !== el.notesEditor){
      if(cur.nodeType === 1){
        const inline = (cur.style?.color || "").trim();
        const attr = (cur.getAttribute?.("color") || "").trim();
...
function normalizeNotesExplicitColors(fallbackColor){
  if(!el.notesEditor) return;
  const color = fallbackColor || state?.project?.notesColor || state.notesDefaultColor || "#151515";
  const walker = document.createTreeWalker(el.notesEditor, NodeFilter.SHOW_TEXT, null);
  const targets = [];
  let node;
...
function saveNotesDoc(){
  if(!state.project || !el.notesEditor) return;
  normalizeNotesExplicitColors(state.project.notesColor || state.notesDefaultColor || "#151515");
  syncProjectNotesImagesFromEditor();
  state.project.notesDoc = normalizeNotesHtml(sanitizeNotesHtml(serializeNotesEditorHtml()));
...
function persistActiveProjectState(opts={}){
  if(!state.project) return;

  const saveNotes = (opts && typeof opts.saveNotes === "boolean")
    ? opts.saveNotes
    : !!(state.notesOpen && el.notesEditor);

  if(saveNotes && el.notesEditor){
    saveNotesDoc();
    return;
  }

  // Keep a fresh notes backup even when the notes modal is closed.
  writeNotesBackup(state.project);
  upsertProject(state.project);
}
...
function refreshNotesRhymesFromCaret(){
  try{
    if(!state.notesOpen || !el.notesEditor) return;
...
function applyNotesColor(color){
  ...
  // Keep picker/default in sync, but do not recolor the whole editor container.
  applyNotesEditorDefaults();
  cacheNotesSelection();
  saveNotesDoc();
  pushNotesHistorySnapshot();
...
function openNotesModal(){
  ...
  const backup = readNotesBackup(state?.project?.id);
  if(state.project && backup){
    const bestNotes = pickBestNotesBackup([
      { notesDoc: state.project.notesDoc, notesColor: state.project.notesColor, updatedAt: Number(state.project.updatedAt) || 0 },
      backup
    ]);
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener("click", (e) => {
      ...
      const imageDeleteBtn = e.target?.closest?.('.notesImageDeleteBtn');
      if(imageDeleteBtn){
        e.preventDefault();
        e.stopPropagation();
        const block = imageDeleteBtn.closest('.notesImageBlock');
        if(block){
          selectNotesImageBlockById(block.getAttribute('data-notes-image-id'));
          removeSelectedNotesImageBlock();
        }
        return;
      }
      const imageCropBtn = e.target?.closest?.('.notesImageCropBtn');
      if(imageCropBtn){
        e.preventDefault();
        e.stopPropagation();
        const block = imageCropBtn.closest('.notesImageBlock');
        if(block){
          const imageId = block.getAttribute('data-notes-image-id');
          selectNotesImageBlockById(imageId);
          openNotesCropForImage(imageId).catch(err => console.error(err));
        }
        return;
      }
      const imageUpBtn = e.target?.closest?.('.notesImageMoveUpBtn');
      if(imageUpBtn){
        e.preventDefault();
        e.stopPropagation();
        const block = imageUpBtn.closest('.notesImageBlock');
        if(block){
          selectNotesImageBlockById(block.getAttribute('data-notes-image-id'));
          moveNotesImageBlockBy(block.getAttribute('data-notes-image-id'), -1);
        }
        return;
      }
      const imageDownBtn = e.target?.closest?.('.notesImageMoveDownBtn');
      if(imageDownBtn){
        e.preventDefault();
        e.stopPropagation();
        const block = imageDownBtn.closest('.notesImageBlock');
        if(block){
          selectNotesImageBlockById(block.getAttribute('data-notes-image-id'));
          moveNotesImageBlockBy(block.getAttribute('data-notes-image-id'), 1);
        }
      ...
      const imageBlock = e.target?.closest?.('.notesImageBlock');
      if(imageBlock){
        e.preventDefault();
        e.stopPropagation();
        selectNotesImageBlockById(imageBlock.getAttribute('data-notes-image-id'));
        cacheNotesSelection();
        return;
      }
      clearNotesImageSelection();
    ...
    el.notesEditor.addEventListener('pointerdown', (e) => {
      ...
      if(resizeHandle){
        ...
        if(block){
          ...
          e.stopPropagation();
          selectNotesImageBlockById(block.getAttribute('data-notes-image-id'));
          startNotesImageResize(block, e);
        }
        return;
      }
      const moveBtn = e.target?.closest?.('.notesImageDeleteBtn, .notesImageCropBtn');
      if(moveBtn) return;
      const imageBlock = e.target?.closest?.('.notesImageBlock');
      if(imageBlock){
        e.preventDefault();
        ...
        selectNotesImageBlockById(imageBlock.getAttribute('data-notes-image-id'));
        startNotesImageDrag(imageBlock, e);
      }
    });
    ...
    el.notesEditor.addEventListener('pointermove', (e) => {
      if(state.notesImageResize?.block){
        e.preventDefault();
        moveNotesImageResize(e);
        return;
      }
      if(state.notesImageDrag?.block){
        moveNotesImageDrag(e);
      }
    });

    const stopNotesImagePointerWork = () => {
      if(state.notesImageResize?.block) stopNotesImageResize(true);
      if(state.notesImageDrag?.block) stopNotesImageDrag();
    };
    el.notesEditor.addEventListener('pointerup', stopNotesImagePointerWork);
    el.notesEditor.addEventListener('pointercancel', stopNotesImagePointerWork);
    el.notesEditor.addEventListener('lostpointercapture', stopNotesImagePointerWork);

    el.notesEditor.addEventListener("keydown", async (e) => {
      if((e.key === "Backspace" || e.key === "Delete") && state.notesSelectedImageId){
        e.preventDefault();
        e.stopPropagation();
        await removeSelectedNotesImageBlock();
        return;
      }
      ...
      if(e.key === "Enter"){
        ...
        if(value){
          ...
          setCaretAtStartOfNode(nextText);
        }else{
    ...
    el.notesEditor.addEventListener("input", () => {
      clearNotesImageSelection();
      normalizeNotesTodoStructure();
      // Keep the picker/default synced without repainting older text.
      applyNotesEditorDefaults();
      refreshNotesRhymesFromCaret();
      saveNotesDoc();
      pushNotesHistorySnapshot();
    });
    el.notesEditor.addEventListener("keyup", refreshNotesRhymesFromCaret);
    el.notesEditor.addEventListener("click", refreshNotesRhymesFromCaret);
    el.notesEditor.addEventListener("touchend", refreshNotesRhymesFromCaret, { passive:true });
  ...
  const flushNotesPersistence = () => {
    try{
      if(state.project){
        if(state.notesOpen && el.notesEditor) saveNotesDoc();
        else writeNotesBackup(state.project);
      }
    }catch{}
  };
  window.addEventListener("beforeunload", flushNotesPersistence);
  window.addEventListener("pagehide", flushNotesPersistence);
  document.addEventListener("visibilitychange", () => {
    if(document.visibilityState === "hidden") flushNotesPersistence();
  });
  ...
  if(el.notesChecklistBtn){
    el.notesChecklistBtn.addEventListener("click", () => insertChecklistIntoNotes());
  }

  if(el.notesAttachBtn){
    el.notesAttachBtn.addEventListener("click", () => {
      restoreNotesSelection();
      el.notesImageInput?.click();
    });
  }

  if(el.notesImageInput){
    el.notesImageInput.addEventListener("change", async () => {
      const files = Array.from(el.notesImageInput.files || []).filter((file) => /^image\//i.test(String(file.type || '')));
      for(const file of files){
        try{ await insertNotesImageFromFile(file); }catch(err){ console.error(err); }
      }
  ...
  document.addEventListener("selectionchange", () => {
    if(state.notesOpen) refreshNotesRhymesFromCaret();
  });
...
const v = Number.isFinite(raw) ? raw : 0; 
  ...
  function commitInstrumentSelect(value){
    const which = String(value || "off");
    if(which === "off"){
      stopInstrument();
    }else{
      ensureCtx();
  ...
  if(el.instrumentSelect) el.instrumentSelect.addEventListener("change", () => commitInstrumentSelect(el.instrumentSelect.value));
  if(el.mInstrumentSelect) el.mInstrumentSelect.addEventListener("change", () => commitInstrumentSelect(el.mInstrumentSelect.value));

  function commitNoteLenSelect(value){
    state.noteLenMode = (["half","eighth","bar"].includes(String(value || ""))) ? String(value) : "half";
    renderNoteLenUI();
  }

  if(el.noteLenSelect) el.noteLenSelect.addEventListener("change", () => commitNoteLenSelect(el.noteLenSelect.value));
  if(el.mNoteLenSelect) el.mNoteLenSelect.addEventListener("change", () => commitNoteLenSelect(el.mNoteLenSelect.value));
  ...
  function commitDrumSelect(value){
    const which = String(value || "off");
    if(which === "off"){
      stopDrums();
    }else{
  ...
  if(el.drumSelect) el.drumSelect.addEventListener("change", () => commitDrumSelect(el.drumSelect.value));
  if(el.mDrumSelect) el.mDrumSelect.addEventListener("change", () => commitDrumSelect(el.mDrumSelect.value));

  if(el.autoPlayBtn) el.autoPlayBtn.addEventListener("click", handleAutoScrollPrimaryAction);
  if(el.autoStopBtn) el.autoStopBtn.addEventListener("click", () => setAutoScroll(false));
  if(el.mScrollBtn) el.mScrollBtn.addEventListener("click", handleAutoScrollPrimaryAction);
  if(el.mAutoStopBtn) el.mAutoStopBtn.addEventListener("click", () => setAutoScroll(false));
  updateAutoScrollButtons();
  ...
  if(el.sortSelect) el.sortSelect.addEventListener("change", () => {
    saveProjectSort(el.sortSelect.value);
    renderProjectsDropdown();
  });
  if(el.projectSortSelect) el.projectSortSelect.addEventListener("change", () => {
    saveProjectSort(el.projectSortSelect.value);
    renderProjectsDropdown();
    renderProjectModal();
  });
  if(el.projectPickerBtn) el.projectPickerBtn.addEventListener("click", openProjectModal);
  if(el.projectCloseBtn) el.projectCloseBtn.addEventListener("click", closeProjectModal);
  if(el.projectModal) el.projectModal.addEventListener("click", (e) => {
  ...
  if(el.archiveBtn) el.archiveBtn.addEventListener("click", openArchiveModal);
  if(el.archiveCloseBtn) el.archiveCloseBtn.addEventListener("click", closeArchiveModal);
  if(el.archiveModal) el.archiveModal.addEventListener("click", (e) => {
    if(e.target === el.archiveModal) closeArchiveModal();
  });
  if(el.archiveSortSelect) el.archiveSortSelect.addEventListener("change", () => {
    saveArchiveSort(el.archiveSortSelect.value);
    renderArchiveModal();
  });
  if(el.archiveAddBtn) el.archiveAddBtn.addEventListener("click", () => setArchiveAddPanel(!archiveUi.addOpen));
  if(el.archiveMoveCancelBtn) el.archiveMoveCancelBtn.addEventListener("click", closeArchiveMoveModal);
  if(el.archiveMoveModal) el.archiveMoveModal.addEventListener("click", (e) => {
    if(e.target === el.archiveMoveModal) closeArchiveMoveModal();
  });
  if(el.archiveMoveConfirmBtn) el.archiveMoveConfirmBtn.addEventListener("click", () => {
    if(archiveUi.moveProjectId) moveProjectToProjects(archiveUi.moveProjectId);
  });
  ...
  if(el.deleteProjectBtn) el.deleteProjectBtn.addEventListener("click", () => {
    ...
    // Close Notes first so its autosave cannot immediately re-create
    // the project we are trying to delete.
    closeNotesModal?.();
...
async function init(){
  ...
  if(el.sortSelect) el.sortSelect.value = getSavedProjectSort();
  if(el.projectSortSelect) el.projectSortSelect.value = getSavedProjectSort();
  if(el.archiveSortSelect) el.archiveSortSelect.value = getSavedArchiveSort();
  ...
  updateUndoRedoUI();
  installSectionSwipe();
  cloud.appMounted = true;
  updateAuthModalUI();
  if(cloud.user) scheduleDeferredCloudAudioSync("post-init");
...
/***********************
KEEP RHYME UI ABOVE KEYBOARD
- simple layout-viewport anchoring for Android Chrome
- portals rhyme dock + floating R to body so parent transforms cannot trap them
- when a text editor is focused, pin the R and dock to the bottom of the resized viewport
- no visualViewport top math (that was lifting the R too high on this device)
***********************/
{
  const dock = document.getElementById("rhymeDock");
  const rBtn = document.getElementById("rBtn");
  const hideBtn = document.getElementById("hideRhymeBtn");
  const notesBtn = document.getElementById("notesBtn");
  const notesCloseBtn = document.getElementById("notesCloseBtn");
  const notesModal = document.getElementById("notesModal");

  const BASE_BTN_BOTTOM = 12;
  const BASE_DOCK_BOTTOM = 8;
  const KB_GAP = 8;
  ...
  function isEditableTarget(node){
    ...
    if(node.matches && node.matches('textarea, input[type="text"], input:not([type]), [contenteditable="true"]')) return true;
    if(node.closest){
      return !!node.closest('textarea, input[type="text"], input:not([type]), [contenteditable="true"], #notesBody, .lyricsTa, .fullLyricsTa');
    }
    return false;
  }
  ...
  function keyboardLikelyOpen(){
    const a = document.activeElement;
    const vv = window.visualViewport;
    const focusedEditable = isEditableTarget(a) || !!(state && state.notesOpen);
    const vvShort = !!(vv && (window.innerHeight - vv.height > 120 || vv.offsetTop > 0));
    return focusedEditable || vvShort;
  }
  ...
  function isNotesModalOpen(){
    return !!(notesModal && notesModal.classList && notesModal.classList.contains('show'));
  }
  ...
  function ensureOverlayRoots(){
    ...
    if(dock){
      forceStyle(dock, 'position', 'fixed');
      forceStyle(dock, 'left', '12px');
      forceStyle(dock, 'right', '12px');
      forceStyle(dock, 'width', 'auto');
      forceStyle(dock, 'max-width', 'none');
      forceStyle(dock, 'margin', '0');
      forceStyle(dock, 'transform', 'none');
      forceStyle(dock, 'z-index', '2147483645');
    }

    if(rBtn){
      forceStyle(rBtn, 'position', 'fixed');
      forceStyle(rBtn, 'right', '12px');
      forceStyle(rBtn, 'left', 'auto');
      forceStyle(rBtn, 'margin', '0');
      forceStyle(rBtn, 'transform', 'none');
      forceStyle(rBtn, 'z-index', '2147483646');
    }
  }

  function applyBottomAnchors(){
    rafId = 0;
    ensureOverlayRoots();

    const kbOpen = keyboardLikelyOpen();
    const dockShown = !!(dock && dock.style.display === 'block');
    const vvBottom = getVisibleViewportBottom();
    const keyboardInset = getKeyboardInset();
    const notesOpenNow = isNotesModalOpen();

    if(dock){
      const dockH = Math.max(48, Math.ceil(dock.getBoundingClientRect().height || 0));
      if(kbOpen){
        if(notesOpenNow){
          forceStyle(dock, 'top', 'auto');
          forceStyle(dock, 'bottom', Math.max(BASE_DOCK_BOTTOM, keyboardInset + KB_GAP) + 'px');
          forceStyle(dock, 'max-height', Math.max(180, Math.round((vvBottom - Math.max(0, keyboardInset)) * 0.42)) + 'px');
        }else{
          const dockTop = Math.max(8, Math.round(vvBottom - dockH - KB_GAP));
          forceStyle(dock, 'bottom', 'auto');
          forceStyle(dock, 'top', dockTop + 'px');
          forceStyle(dock, 'max-height', Math.max(180, Math.round(vvBottom * 0.42)) + 'px');
        }
      }else{
        forceStyle(dock, 'top', 'auto');
        forceStyle(dock, 'bottom', BASE_DOCK_BOTTOM + 'px');
        try{ dock.style.removeProperty('max-height'); }catch{}
      }
    }
    ...
    if(rBtn){
      if(kbOpen){
        if(notesOpenNow){
          forceStyle(rBtn, 'top', 'auto');
          forceStyle(rBtn, 'bottom', Math.max(BASE_BTN_BOTTOM, keyboardInset + KB_GAP) + 'px');
        }else{
          const btnH = Math.max(52, Math.ceil(rBtn.getBoundingClientRect().height || 52));
          const btnTop = Math.max(8, Math.round(vvBottom - btnH - KB_GAP));
          forceStyle(rBtn, 'bottom', 'auto');
          forceStyle(rBtn, 'top', btnTop + 'px');
        }
      }else{
        forceStyle(rBtn, 'top', 'auto');
        forceStyle(rBtn, 'bottom', BASE_BTN_BOTTOM + 'px');
      }
      if(dockShown){
        forceStyle(rBtn, 'opacity', '0');
        forceStyle(rBtn, 'pointer-events', 'none');
      }else{
        forceStyle(rBtn, 'opacity', '1');
        forceStyle(rBtn, 'pointer-events', 'auto');
      }
    }
  }
  ...
  function queueApply(){
    ...
    rafId = requestAnimationFrame(applyBottomAnchors);
  }

  function pulse(){
    clearTimeout(settleTimer);
    if(Date.now() > pulseUntil) return;
    queueApply();
    settleTimer = setTimeout(pulse, 50);
  }
  ...
  function settle(ms = 700){
    ...
    setTimeout(queueApply, 140);
    setTimeout(queueApply, 260);
    setTimeout(queueApply, 420);
    pulse();
  }
  ...
  if(rBtn){
    rBtn.addEventListener('click', settle);
    rBtn.addEventListener('touchstart', settle, { passive:true });
    rBtn.addEventListener('pointerdown', settle, { passive:true });
  }
  if(hideBtn) hideBtn.addEventListener('click', settle);
  if(notesBtn) notesBtn.addEventListener('click', settle);
  if(notesCloseBtn) notesCloseBtn.addEventListener('click', settle);

  if(dock && window.MutationObserver){
    new MutationObserver(settle).observe(dock, { attributes:true, attributeFilter:['style','class'] });
  }
  if(notesModal && window.MutationObserver){
    new MutationObserver(settle).observe(notesModal, { attributes:true, attributeFilter:['style','class','aria-hidden'] });
  }
}
