...
async function tombstoneRecordingInProject(projectId, rec){
  ...
  if(cloud.ready && cloud.user) queueCloudSync("recording-delete");
}
...
applyTick();
  ...
  try{
    ...
    if(rec){
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      if(cloud.ready && cloud.user) queueCloudSync("recording-offset");
      renderRecordings();
    }
  }catch{}
}
...
async function uploadAudioFile(){
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      ...
      await dbPut(item);
      if(cloud.ready && cloud.user) queueCloudSync("recording-upload");
...
async function renderRecordings(){
  ...
  mine.forEach(rec => {
    ...
    edit.addEventListener("click", async () => {
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      if(cloud.ready && cloud.user) queueCloudSync("recording-rename");
      renderRecordings();
    });
...
await dbPut(item);
if(cloud.ready && cloud.user) queueCloudSync("recording-new");
await renderRecordings();
