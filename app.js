...
function parseFullTextToSectionCards(fullText){
  ...
  function isSectionHeadingLine(line){
    ...
    if(!raw) return null;
    const t = raw.replace(/^[\[\(\{]+/,"").replace(/[\]\)\}]+$/,"").trim().toUpperCase();

    // normalize: "VERSE1" -> "VERSE 1"
    const mVerse = t.match(/^VERSE\s*([0-9]+)$/);
    if(mVerse) return `VERSE ${mVerse[1]}`;

    const mCh = t.match(/^CHORUS\s*([0-9]+)$/);
    if(mCh) return `CHORUS ${mCh[1]}`;

    if(t === "CHORUS") return "CHORUS 1";
    if(t === "INTRO") return "INTRO";
    if(t === "OUTRO") return "OUTRO";
    if(t === "BRIDGE") return "BRIDGE";
    if(t === "PRECHORUS" || t === "PRE-CHORUS") return "BRIDGE"; // fallback bucket
