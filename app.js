...
async function renderRhymes(seed){
  ...
  list.forEach(w => {
    ...
    const insertFromChip = (e) => {
      ...
      // Avoid double inserts when mobile fires touchend followed by click.
      const nowTs = Date.now();
      if(b.__lastInsertTs && (nowTs - b.__lastInsertTs) < 500) return;
    ...
    b.addEventListener("pointerup", insertFromChip);
    b.addEventListener("touchend", insertFromChip, {passive:false});
    b.addEventListener("click", insertFromChip);
