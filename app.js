...
const LYRIC_FRIENDLY_WORDS = new Set([
  "again","air","alive","alone","always","angel","ash","away","back","believe","best","blue","body","broken",
  "burn","call","change","close","come","cry","dark","day","desire","dream","drive","dust","eyes","face",
  "faith","fall","far","feel","fire","flame","fly","forever","free","friend","game","ghost","glory","go",
  "gold","gone","good","grace","ground","hand","hard","heart","heaven","hell","high","higher","hold","home",
  "hope","hurt","inside","kiss","know","late","leave","less","life","light","lights","live","lonely","long",
  "love","lover","low","mercy","mind","moon","more","morning","move","name","near","need","never","night",
  "now","open","pain","peace","pray","praise","rain","real","red","rest","rise","road","run","save","say",
  "shadow","shine","sky","slow","small","snow","somebody","song","soul","sound","space","sparks","spirit","stand",
  "stay","still","stone","storm","strong","sun","sweet","take","tell","thing","time","tonight","touch","true",
  "turn","under","wait","wake","walk","want","warm","water","way","whole","wild","wind","wish","world","worth",
  "you","your"
]);
const OBSCURE_RHYME_PATTERNS = [
  /phobia$/i, /logy$/i, /graphy$/i, /nomy$/i, /esque$/i, /oid$/i,
  /aceous$/i, /arium$/i, /orium$/i, /ette$/i, /ling$/i
];
const OBSCURE_RHYME_WORDS = new Set([
  "abye","anigh","briar","brier","clime","direful","fen","glade","lisle","mire","nigh","oer","pyre","quire","sere","thee","thine","thy","twyer","ware","whilom","yon","yore"
]);
...
async function fetchDatamuseRhymes(word, max = 36){
  const w = normalizeWord(word);
  if(!w) return [];
  const url = `https://api.datamuse.com/words?rel_rhy=${encodeURIComponent(w)}&md=sf&max=${max}`;
  try{
...
async function fetchDatamuseNearRhymes(word, max = 36){
  const w = normalizeWord(word);
  if(!w) return [];
  const url = `https://api.datamuse.com/words?rel_nry=${encodeURIComponent(w)}&md=sf&max=${max}`;
  try{
    const res = await fetch(url, { cache: "no-store" });
    if(!res.ok) return [];
    const data = await res.json();
...
function scoreLyricFriendliness(candidate, seedWord){
  ...
  if(candidate && candidate.kind === "perfect") score += 200;
  else if(candidate && candidate.kind === "near") score += 120;

  score += Math.min(80, rawScore / 12);

  if(LYRIC_FRIENDLY_WORDS.has(word)) score += 90;
  if(/^(love|heart|night|light|fire|rain|home|alone|time|grace|faith|higher|desire|pain|dream|stay|gone|strong|song|soul)$/i.test(word)) score += 40;

  if(len >= 3 && len <= 8) score += 28;
  else if(len <= 10) score += 12;
  else score -= 18;

  if(syll === 1) score += 18;
  else if(syll === 2) score += 22;
  else if(syll === 3) score += 10;
  else if(syll >= 5) score -= 14;

  if(/^[a-z]+(?:'[a-z]+)?$/i.test(word)) score += 16;
  if(/ing$|ed$|er$|est$|in'$|in$/i.test(word)) score += 8;
  ...
  if(OBSCURE_RHYME_WORDS.has(word)) score -= 120;
  if(OBSCURE_RHYME_PATTERNS.some(rx => rx.test(word))) score -= 55;
  if(/(?:tion|sion|ment|tude|ness|less|ship|gram|graph|scope|crat|zyme|tron)$/i.test(word)) score -= 22;
  if(/^[qxz]/i.test(word) && !LYRIC_FRIENDLY_WORDS.has(word)) score -= 12;

  if(candidate && candidate.kind === "near"){
    if(Math.abs((Number(candidate.numSyllables)||0) - estimateSyllables(seed)) <= 1) score += 18;
...
function rankRhymeCandidates(seedWord, perfectList, nearList){
  ...
  })).filter(entry => entry.rankScore > -200);
...
async function renderRhymes(seed){
  ...
  const [perfectRaw, nearRaw] = await Promise.all([
    fetchDatamuseRhymes(word, 36),
    fetchDatamuseNearRhymes(word, 36)
  ]);
...
- rhyme dock stays above keyboard
- floating R button also lifts above keyboard for easy access
***********************/
if (window.visualViewport) {
  const dock = document.getElementById("rhymeDock");
  const rBtn = document.getElementById("rBtn");
  let rafId = 0;
  let baselineBottom = Math.max(window.innerHeight, Math.round(window.visualViewport.height + window.visualViewport.offsetTop));
  let lastOverlap = 0;
  ...
  const measureOverlap = () => {
    ...
    const viewportBottom = Math.round(vv.height + vv.offsetTop);
    const rawOverlap = Math.max(0, baselineBottom - viewportBottom);
    const keyboardOpen = rawOverlap > 120;

    // Refresh the baseline only when the keyboard is closed so transient viewport changes
    // during keyboard animation do not make the R button jump too high.
    if(!keyboardOpen){
      baselineBottom = Math.max(baselineBottom, viewportBottom, window.innerHeight);
      lastOverlap = 0;
      return 0;
    }

    // Clamp sudden spikes during the keyboard-open animation.
    const clamped = Math.min(rawOverlap, Math.max(rawOverlap, lastOverlap + 120));
    lastOverlap = clamped;
    return clamped;
  };

  const applyRhymeViewportUI = () => {
    rafId = 0;
    ...
    if(rBtn){
      // While the rhyme dock is open, hide the floating R button entirely.
      if(dockShown){
        rBtn.style.transform = "translateY(0px)";
        rBtn.style.transition = "transform 140ms ease, opacity 120ms ease";
        rBtn.style.opacity = "0";
        rBtn.style.pointerEvents = "none";
        return;
      }

      rBtn.style.transform = `translateY(-${overlap}px)`;
      rBtn.style.transition = "transform 140ms ease, opacity 120ms ease";
      rBtn.style.opacity = keyboardOpen ? "1" : "0.96";
      rBtn.style.pointerEvents = "auto";
    }
  };

  const updateRhymeViewportUI = () => {
    if(rafId) cancelAnimationFrame(rafId);
    rafId = requestAnimationFrame(applyRhymeViewportUI);
  };

  visualViewport.addEventListener("resize", updateRhymeViewportUI);
  visualViewport.addEventListener("scroll", updateRhymeViewportUI);
  window.addEventListener("resize", updateRhymeViewportUI);

  // Re-run after the dock is toggled so the R button visibility/position stays correct.
  if(rBtn){
    rBtn.addEventListener("click", () => setTimeout(updateRhymeViewportUI, 0));
  }
  const hideBtn = document.getElementById("hideRhymeBtn");
  if(hideBtn){
    hideBtn.addEventListener("click", () => setTimeout(updateRhymeViewportUI, 0));
  }

  updateRhymeViewportUI();
}
})();
