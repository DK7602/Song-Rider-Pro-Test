...
function parseFullTextToSectionCards(fullText){
  ...
  function flushCard(){
    ...
    // We return chords as 8 slots so we can preserve placement as much as possible.
    const chordsSlots = Array(8).fill("");
    const seqFallback = []; // used when we can't place (fills left->right)
    const lyricParts = [];
    ...
    function placeChordAtSlot(ch, slot){
      ...
      // If slot already taken, slide right, then left.
      if(!chordsSlots[s]){ chordsSlots[s] = c; return; }
      for(let k=s+1;k<8;k++){ if(!chordsSlots[k]){ chordsSlots[k]=c; return; } }
      for(let k=s-1;k>=0;k--){ if(!chordsSlots[k]){ chordsSlots[k]=c; return; } }
      // All full: drop.
    }
    ...
    function placeAlignedChordLine(chordLineRaw, lyricLineRaw){
      ...
      // Walk visible tokens and keep their starting index (alignment signal).
      const toks = [];
      for(const m of chordLine.matchAll(/\S+/g)){
        const tok = String(m[0] || "").replace(/[\[\]\(\)\{\}]/g, "").replace(/[|]/g, "").trim();
        if(!tok) continue;
      ...
      // Map token X-position -> one of 8 chord slots.
      for(const t of toks){
        const ratio = t.idx / denom;
        const slot = Math.round(ratio * 7);
        placeChordAtSlot(t.chord, slot);
      }
      // If nothing landed (unlikely), fallback to sequential.
      if(chordsSlots.every(x => !String(x||"").trim())){
        toks.forEach(t => seqFallback.push(t.chord));
      }
      return true;
    }
    ...
    for(let i=0;i<blockLines.length;i++){
      ...
      if(lineIsMostlyChords(L) && i+1 < blockLines.length && !lineIsMostlyChords(blockLines[i+1])){
        ...
        // Try to preserve horizontal placement from chord sheet spacing.
        const placed = placeAlignedChordLine(L, lyricLine);
        if(!placed){
          const chordLine = L
            .replace(/[\[\]\(\)\{\}]/g," ")
            .replace(/[|]/g," ")
        ...
        lyricParts.push(String(lyricLine || "").trim());
        i++;
        continue;
      }
    ...
    // Fill any remaining empty chord slots left->right from sequential fallback.
