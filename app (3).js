...
function parseFullTextToSectionCards(fullText){
  ...
  // ✅ Capture bracketed/parenthesized chord tokens and remove them from lyrics
...
function lineIsMostlyChords(line){
  ...
  // quick allow: single-token chord lines
  ...
  // If the line contains many letters outside A-G, it's probably lyrics.
...
function buildSlotsFromAlignedChordLine(chordLineRaw, lyricLineRaw){
  ...
  // If chord line likely collapsed (no multi-space runs), fall back to word-anchoring against lyric words.
...
function normalizeProject(p){
  ...
  if(!p.enabledSections.includes("VERSE 1")) p.enabledSections.unshift("VERSE 1");
...
function renderSheetTitleBar(){
  ...
  // Title input ONLY for EXTRA pages (pre-named pages should not have an editable title box)
  let titleInput = null;
  const isExtraPage = (state.project && Array.isArray(state.project.extraSections) && state.project.extraSections.includes(state.currentSection)) || /^EXTRA/i.test(String(state.currentSection||""));
  if(isExtraPage){
  ...
  if(state.currentSection === "Full"){
    ...
    addBtn.addEventListener("click", () => {
      ...
      const last = vis.length ? vis[vis.length-1] : "VERSE 1";
    ...
    delBtn.addEventListener("click", () => {
      ...
      const candidates = pages.filter(s => s !== "VERSE 1");
      const sec = candidates.length ? candidates[candidates.length-1] : null;
      if(!sec){
        alert("Nothing to delete (VERSE 1 is always kept).");
        return;
      }
...
function deleteSectionPage(sec){
  ...
  // ✅ track manual deletion for BASE pages
  if(isBase && sec !== "Full"){
    const arr = state.project.deletedBaseSections || (state.project.deletedBaseSections = []);
    if(!arr.includes(sec)) arr.push(sec);
  }
