    ...
    if(sel && sel.rangeCount){
      ...
      if(el.notesEditor.contains(range.commonAncestorContainer)){
        ...
        try{
          ...
          prefix = probe.toString();
        }catch{}
        try{
          const probe2 = range.cloneRange();
          probe2.collapse(false);
          const maxLen = (range.endContainer && range.endContainer.nodeType === Node.TEXT_NODE) ? range.endContainer.textContent.length : range.endOffset;
          probe2.setEnd(range.endContainer, Math.min(maxLen, range.endOffset + 1));
          suffix = probe2.toString();
        }catch{}
        const text = `${(/\s$/.test(prefix) || !prefix) ? "" : " "}${insert}${(/^\s/.test(suffix) || !suffix) ? "" : " "}`;
...
function currentNotesHtml(){
  return normalizeNotesHtml(el?.notesEditor?.innerHTML || "");
}
