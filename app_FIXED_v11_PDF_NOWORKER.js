...
async function importPdfToFull(file){
  try{
    ...
    console.error(err);
    alert("PDF import failed. This is usually because PDF.js didn\'t load (blocked CDN/cache) or the PDF has no selectable text (scanned image). Try a different PDF, or export your PDF as text-based, or refresh and try again.");
  }
}

async function extractTextFromPdfFile(file){
  // ✅ MOST-RELIABLE PDF IMPORT (Mobile + GitHub Pages)
  // We avoid the PDF.js worker entirely (disableWorker:true) because mobile browsers
  // and CDNs can sometimes block worker fetches / MIME issues.
  // We also load the *legacy* build via <script> (works in classic scripts).
  ...
  async function ensurePdfJsLegacy(){
    if(window.pdfjsLib && window.pdfjsLib.getDocument) return window.pdfjsLib;

    // Try local first (if you decide to bundle later), then multiple CDNs.
    const libs = [
      "./pdf.min.js",
    ...
    throw lastErr || new Error("pdfjsLib not loaded");
  }

  const pdfjsLib = await ensurePdfJsLegacy();
