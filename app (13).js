...
function setAutoScroll(on){
  ...
  if(state.autoScrollOn){
    ...
    // ✅ Always restart from the FIRST card on the active page.
    // (User expectation: tapping ⇅ starts over at beat 1 / card 1.)
    state.tick8 = 0;
    state.lastAutoBar = -1;
    ...
    if(state.currentSection !== "Full"){
      const cards = getCards();
      state.playCardIndex = cards.length ? 0 : null;
      if(cards[0]) scrollCardIntoView(cards[0]);
      clearTick();
      applyTick();
    }
