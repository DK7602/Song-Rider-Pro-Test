...
function parseFullTextToSectionCards(fullText){
  ...
  const CHORD_TOKEN_RE = /^[A-G](?:#|b)?(?:(?:m|maj|min|dim|aug|sus|add)\d*)?(?:\d+)?(?:\/[A-G](?:#|b)?)?$/i;
  ...
  function lineIsMostlyChords(line){
    ...
    // strip obvious non-chord characters to gauge "other" content
    const bad = raw.replace(/[A-Ga-g#b0-9\/\s\-\|\.\(\)\[\]\{\}'"“”‘’,:;!?\t]+/g, "").trim();
  ...
  for(let i=0; i<lines.length; i++){
    ...
    if(line.trim() === ""){
      ...
      const lastAcc = acc.length ? String(acc[acc.length-1] || "") : "";
      if(lastAcc && lineIsMostlyChords(lastAcc)){
        let k = i + 1;
...
async function extractTextFromPdfFile(file){
  // ✅ Robust PDF.js loader for GitHub Pages + Mobile:
  // - prefers already-loaded global
  // - tries local ESM (pdf.mjs/pdf.worker.mjs)
  // - tries CDN ESM
  // - FINAL fallback: CDN legacy (pdf.min.js) which is the most compatible
  // Also retries parsing with disableWorker:true if worker fetch is blocked.
  ...
  async function tryLoadPdfJs(){
    // 1) already present (from index preload / previous import)
    if(pdfjsLib && pdfjsLib.getDocument) return;

    // 2) local ESM bundled files (best for offline)
    try{
      const mod = await import("./pdf.mjs");
      pdfjsLib = (mod && mod.getDocument) ? mod
        : (mod?.default && mod.default.getDocument ? mod.default : mod);
    ...
    // 4) CDN legacy build (most reliable on mobile + odd MIME/caching situations)
    try{
      await loadScript("https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/legacy/build/pdf.min.js");
  ...
  // PDF.js worker
  // - ESM: *.mjs worker
  // - legacy: *.min.js worker
  try{
    const workerLocalEsm = "./pdf.worker.mjs";
    const workerCdnEsm   = "https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/build/pdf.worker.mjs";
    const workerLocalLeg = "./pdf.worker.min.js";
    const workerCdnLeg   = "https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/legacy/build/pdf.worker.min.js";
    if(pdfjsLib.GlobalWorkerOptions){
      if(usingLegacy){
        pdfjsLib.GlobalWorkerOptions.workerSrc = usingCdn ? workerCdnLeg : workerLocalLeg;
      }else{
        pdfjsLib.GlobalWorkerOptions.workerSrc = usingCdn ? workerCdnEsm : workerLocalEsm;
      }
    }
  }catch(_e){}
  ...
  // Try normal (worker) first, then retry without worker if it fails.
  let pdf;
  try{
    pdf = await pdfjsLib.getDocument({ data: ab }).promise;
  }catch(err){
    console.warn("PDF.js worker load failed; retrying with disableWorker:true", err);
