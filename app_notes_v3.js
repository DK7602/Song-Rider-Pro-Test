...
function getNotesEditorText(ed){
  if(!ed) return "";
  return normalizeLineBreaks(String(ed.innerText || ed.textContent || "")).replace(/ /g, " ");
}
...
function getCaretCharOffsetInEditor(ed){
  try{
    ...
    return normalizeLineBreaks(String(pre.toString() || "")).length;
  }catch{
...
function insertWordIntoLyrics(word){
  ...
  if(active && active === el.notesEditor){
    cacheNotesSelection();
